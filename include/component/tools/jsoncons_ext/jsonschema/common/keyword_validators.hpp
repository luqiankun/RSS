...
struct collecting_error_reporter : public error_reporter {
 ...
 private:
  void do_error(const validation_output& o) final { errors.push_back(o); }
};
...
class ref_validator : public keyword_validator_base<Json> {
 ...
 public:
  ...
  ref_validator(const uri& schema_path,
                const schema_validator<Json>* referred_schema)
      : keyword_validator_base<Json>("$ref", schema_path),
        referred_schema_(referred_schema) {}

  void set_referred_schema(const schema_validator<Json>* target) {
    referred_schema_ = target;
  }

  uri get_base_uri() const { return this->schema_path(); }
  ...
  void do_validate(const evaluation_context<Json>& eval_context,
    ...
    if (!referred_schema_) {
      reporter.error(validation_output(
          this->keyword_name(), this_context.eval_path(), this->schema_path(),
          instance_location.to_string(),
          "Unresolved schema reference " + this->schema_path().string()));
      return;
    }

    referred_schema_->validate(this_context, instance, instance_location,
                               evaluated_properties, reporter, patch);
  }
};
...
class recursive_ref_validator : public keyword_validator_base<Json> {
 ...
 private:
  void do_validate(const evaluation_context<Json>& eval_context,
    ...
    // << " << this->schema_path().string() << ", instanceLocation:" <<
    // instance_location.to_string() << "\n";
    ...
    schema_ptr->validate(this_context, instance, instance_location,
                         evaluated_properties, reporter, patch);
  }
};
...
class content_encoding_validator : public keyword_validator_base<Json> {
 ...
 public:
  content_encoding_validator(const uri& schema_path,
 ...
 private:
  void do_validate(const evaluation_context<Json>& eval_context,
                   const Json& instance,
                   const jsonpointer::json_pointer& instance_location,
                   std::unordered_set<std::string>&, error_reporter& reporter,
                   Json&) const final {
    ...
    if (content_encoding_ == "base64") {
      ...
      if (retval.ec != jsoncons::conv_errc::success) {
        reporter.error(validation_output(
            this->keyword_name(), this_context.eval_path(), this->schema_path(),
            instance_location.to_string(), "Content is not a base64 string"));
...
class content_media_type_validator : public keyword_validator_base<Json> {
 ...
 public:
  content_media_type_validator(const uri& schema_path,
...
class format_validator : public keyword_validator_base<Json> {
 ...
 public:
  format_validator(const uri& schema_path, const format_checker& format_check)
      : keyword_validator_base<Json>("format", schema_path),
 ...
 private:
  void do_validate(const evaluation_context<Json>& eval_context,
    ...
    if (format_check_ != nullptr) {
      ...
      format_check_(this_context.eval_path(), this->schema_path(),
...
class pattern_validator : public keyword_validator_base<Json> {
 ...
 public:
  pattern_validator(const uri& schema_path, const std::string& pattern_string,
 ...
 private:
  void do_validate(const evaluation_context<Json>& eval_context,
    ...
    if (!std::regex_search(s, regex_)) {
      ...
      reporter.error(validation_output(
          this->keyword_name(), this_context.eval_path(), this->schema_path(),
          instance_location.to_string(), std::move(message)));
      if (reporter.fail_early()) {
        return;
      }
    }
...
class pattern_validator : public keyword_validator_base<Json> {
 ...
 private:
  void do_validate(const Json&, const jsonpointer::json_pointer&,
                   std::unordered_set<std::string>&, error_reporter&,
                   Json&) const final {}
};
#endif
...
class max_length_validator : public keyword_validator_base<Json> {
 ...
 public:
  max_length_validator(const uri& schema_path, std::size_t max_length)
      : keyword_validator_base<Json>("maxLength", schema_path),
...
class max_items_validator : public keyword_validator_base<Json> {
 ...
 public:
  max_items_validator(const uri& schema_path, std::size_t max_items)
      : keyword_validator_base<Json>("maxItems", schema_path),
 ...
 private:
  void do_validate(const evaluation_context<Json>& eval_context,
    ...
    if (instance.size() > max_items_) {
      std::string message("Expected maximum item count: " +
...
class min_items_validator : public keyword_validator_base<Json> {
 ...
 public:
  min_items_validator(const uri& schema_path, std::size_t min_items)
      : keyword_validator_base<Json>("minItems", schema_path),
...
class items_array_validator : public keyword_validator_base<Json> {
 ...
 private:
  void do_validate(const evaluation_context<Json>& eval_context,
                   ...
                   const jsonpointer::json_pointer& instance_location,
                   std::unordered_set<std::string>& evaluated_properties,
                   error_reporter& reporter, Json& patch) const final {
    if (!instance.is_array()) {
      return;
    }
    ...
    for (const auto& item : instance.array_range()) {
      ...
      if (validator_it != item_validators_.cend()) {
        ...
        pointer /= index++;
        (*validator_it)
            ->validate(item_context, item, pointer, evaluated_properties,
                       reporter, patch);
        ++validator_it;
      } else if (additional_items_validator_ != nullptr) {
        pointer /= index++;
        additional_items_validator_->validate(
            eval_context, item, pointer, evaluated_properties, reporter, patch);
      } else
        break;
    }
  }
  ...
  void do_validate(const evaluation_context<Json>& eval_context,
    ...
    if (items_validator_) {
      for (const auto& item : instance.array_range()) {
        jsonpointer::json_pointer pointer(instance_location);
        pointer /= index;
        items_validator_->validate(this_context, item, pointer,
                                   evaluated_properties, reporter, patch);
        index++;
      }
    }
...
class unique_items_validator : public keyword_validator_base<Json> {
 ...
 public:
  unique_items_validator(const uri& schema_path, bool are_unique)
      : keyword_validator_base<Json>("uniqueItems", schema_path),
 ...
 private:
  void do_validate(const evaluation_context<Json>& eval_context,
    ...
    if (are_unique_ && !array_has_unique_items(instance)) {
      reporter.error(validation_output(
          this->keyword_name(), this_context.eval_path(), this->schema_path(),
          instance_location.to_string(), "Array items are not unique"));
      if (reporter.fail_early()) {
        return;
      }
    }
...
class min_length_validator : public keyword_validator_base<Json> {
 ...
 public:
  min_length_validator(const uri& schema_path, std::size_t min_length)
      : keyword_validator_base<Json>("minLength", schema_path),
...
class not_validator : public keyword_validator_base<Json> {
 ...
 public:
  not_validator(const uri& schema_path, schema_validator_type&& rule)
 ...
 private:
  void do_validate(const evaluation_context<Json>& eval_context,
    ...
    rule_->validate(this_context, instance, instance_location,
                    evaluated_properties, local_reporter, patch);

    if (local_reporter.errors.empty()) {
      reporter.error(
          validation_output(this->keyword_name(), this_context.eval_path(),
                            this->schema_path(), instance_location.to_string(),
...
struct all_of_criterion {
  ...
  static bool is_complete(const evaluation_context<Json>& eval_context,
                          ...
                          std::size_t) {
    if (!local_reporter.errors.empty())
      reporter.error(validation_output(key(), eval_context.eval_path(), uri(""),
                                       instance_location.to_string(),
                                       "At least one schema failed to match, "
                                       "but all are required to match. ",
...
struct any_of_criterion {
  ...
  static bool is_complete(const evaluation_context<Json>&, const Json&,
                          const jsonpointer::json_pointer&, error_reporter&,
                          const collecting_error_reporter&, std::size_t count) {
...
struct one_of_criterion {
  ...
  static bool is_complete(const evaluation_context<Json>& eval_context,
    ...
    if (count > 1) {
      ...
      reporter.error(
          validation_output(key(), eval_context.eval_path(), uri("XXX"),
                            instance_location.to_string(), std::move(message)));
    }
    return count > 1;
  }
...
class combining_validator : public keyword_validator_base<Json> {
 ...
 private:
  void do_validate(const evaluation_context<Json>& eval_context,
                   ...
                   std::unordered_set<std::string>& evaluated_properties,
                   error_reporter& reporter, Json& patch) const final {
    // std::cout << "combining_validator.do_validate " << "keywordLocation: << "
    // << this->schema_path().string() << ", instanceLocation:" <<
    // instance_location.to_string() << "\n";
    ...
    for (std::size_t i = 0; i < validators_.size(); ++i) {
      ...
      validators_[i]->validate(item_context, instance, instance_location,
                               evaluated_properties, local_reporter, patch);
      if (!is_complete) {
        if (mark == local_reporter.errors.size()) count++;
        if (Criterion::is_complete(item_context, instance, instance_location,
                                   reporter, local_reporter, count))
          is_complete = true;
      }
    }
    ...
    if (count == 0) {
      reporter.error(validation_output(
          Criterion::key(), this_context.eval_path(), this->schema_path(),
          instance_location.to_string(),
...
class any_of_validator : public keyword_validator_base<Json> {
 ...
 private:
  void do_validate(const evaluation_context<Json>& eval_context,
    ...
    // eval_context.eval_path().to_string() << ", " << instance << "\n";
...
class maximum_validator : public keyword_validator_base<Json> {
 ...
 public:
  maximum_validator(const uri& schema_path, const Json& value)
      : keyword_validator_base<Json>("maximum", schema_path),
        value_(value),
        message_{" is greater than maximum " +
...
class exclusive_maximum_validator : public keyword_validator_base<Json> {
 ...
 public:
  exclusive_maximum_validator(const uri& schema_path, const Json& value)
      : keyword_validator_base<Json>("exclusiveMaximum", schema_path),
        value_(value),
        message_{" is no less than exclusiveMaximum " +
                 value.template as<std::string>()} {}
...
class minimum_validator : public keyword_validator_base<Json> {
 ...
 public:
  minimum_validator(const uri& schema_path, const Json& value)
      : keyword_validator_base<Json>("minimum", schema_path),
        value_(value),
...
class exclusive_minimum_validator : public keyword_validator_base<Json> {
 ...
 public:
  exclusive_minimum_validator(const uri& schema_path, const Json& value)
      : keyword_validator_base<Json>("exclusiveMinimum", schema_path),
        value_(value),
        message_{" is no greater than exclusiveMinimum " +
                 value.template as<std::string>()} {}
...
class multiple_of_validator : public keyword_validator_base<Json> {
 ...
 public:
  multiple_of_validator(const uri& schema_path, double value)
      : keyword_validator_base<Json>("multipleOf", schema_path),
        value_(value) {}
...
class max_properties_validator : public keyword_validator_base<Json> {
 ...
 public:
  max_properties_validator(const uri& schema_path, std::size_t max_properties)
      : keyword_validator_base<Json>("maxProperties", schema_path),
 ...
 private:
  void do_validate(const evaluation_context<Json>& eval_context,
    ...
    if (!instance.is_object()) {
      ...
      std::string message("Maximum properties: " +
...
class min_properties_validator : public keyword_validator_base<Json> {
 ...
 public:
  min_properties_validator(const uri& schema_path, std::size_t min_properties)
      : keyword_validator_base<Json>("minProperties", schema_path),
...
class unevaluated_properties_validator : public keyword_validator_base<Json> {
 ...
 public:
  unevaluated_properties_validator(const uri& schema_path,
      ...
      : keyword_validator_base<Json>("unevaluatedProperties",
                                     std::move(schema_path)),
    ...
    if (validator_) {
      ...
      for (const auto& prop : instance.object_range()) {
        ...
        if (prop_it == evaluated_properties.end()) {
          ...
          validator_->validate(this_context, prop.value(), instance_location,
                               evaluated_properties, reporter, patch);
 ...
 private:
  void do_validate(const evaluation_context<Json>& eval_context,
    ...
    if (if_validator_) {
      ...
      if_validator_->validate(this_context, instance, instance_location,
                              evaluated_properties, local_reporter, patch);
      if (local_reporter.errors.empty()) {
        if (then_validator_)
          then_validator_->validate(this_context, instance, instance_location,
                                    evaluated_properties, reporter, patch);
      } else {
        if (else_validator_)
          else_validator_->validate(this_context, instance, instance_location,
                                    evaluated_properties, reporter, patch);
      }
    }
  }
 ...
 private:
  void do_validate(const evaluation_context<Json>& eval_context,
    ...
    if (value_ != instance) {
      ...
      reporter.error(validation_output(
          this->keyword_name(), this_context.eval_path(), this->schema_path(),
          instance_location.to_string(), "Instance is not const"));
    }
  }
};
  ...
  void do_validate(const evaluation_context<Json>& eval_context,
    ...
    for (auto it = expected_types_.begin(); it != end && !is_type_found; ++it) {
      switch (*it) {
        ...
        case json_schema_type::integer:
          if (instance.is_number()) {
            if (!(instance.template is_integer<int64_t>() ||
                  (instance.is_double() &&
    ...
    if (!is_type_found) {
      ...
      for (std::size_t i = 0; i < expected_types_.size(); ++i) {
        ...
        // std::cout << ", " << i << ". expected " <<
        // to_string(expected_types_[i]);
      }
      ss << ", found " << instance.type();

      reporter.error(validation_output(
          this->keyword_name(), this_context.eval_path(), this->schema_path(),
          instance_location.to_string(), ss.str()));
      if (reporter.fail_early()) {
        return;
      }
    }
...
class properties_validator : public keyword_base<Json> {
 ...
 public:
  ...
  void validate(const evaluation_context<Json>& eval_context,
                ...
                const jsonpointer::json_pointer& instance_location,
                std::unordered_set<std::string>& evaluated_properties,
                error_reporter& reporter, Json& patch,
    ...
    for (const auto& prop : instance.object_range()) {
      ...
      if (properties_it != properties_.end()) {
        std::size_t error_count = reporter.error_count();
        properties_it->second->validate(prop_context, prop.value(), pointer,
                                        evaluated_properties, reporter, patch);
    ...
    for (const auto& prop : instance.object_range()) {
      ...
      for (auto& schema_pp : pattern_properties_)
        if (std::regex_search(prop.key(), schema_pp.first)) {
          all_properties.insert(prop.key());
          std::size_t error_count = reporter.error_count();
          schema_pp.second->validate(prop_context, prop.value(), pointer,
                                     evaluated_properties, reporter, patch);
          if (reporter.error_count() == error_count) {
            evaluated_properties.insert(prop.key());
          }
...
class additional_properties_validator : public keyword_validator_base<Json> {
 ...
 public:
  additional_properties_validator(
      ...
      : keyword_validator_base<Json>("additionalProperties",
                                     std::move(schema_path)),
        properties_(std::move(properties)),
        pattern_properties_(std::move(pattern_properties)),
 ...
 private:
  void do_validate(const evaluation_context<Json>& eval_context,
    ...
    if (properties_) {
      properties_->validate(this_context, instance, instance_location,
                            evaluated_properties, reporter, patch,
                            all_properties);
      if (reporter.fail_early()) {
        return;
    ...
    if (pattern_properties_) {
      pattern_properties_->validate(this_context, instance, instance_location,
                                    evaluated_properties, reporter, patch,
                                    all_properties);
      if (reporter.fail_early()) {
        return;
    ...
    for (const auto& prop : instance.object_range()) {
      ...
      if (properties_it == all_properties.end()) {
        ...
        if (additional_properties_) {
          ...
          additional_properties_->validate(this_context, prop.value(), pointer,
                                           evaluated_properties, local_reporter,
                                           patch);
          if (!local_reporter.errors.empty()) {
            reporter.error(validation_output(
                this->keyword_name(), this_context.eval_path(),
...
class dependent_required_validator : public keyword_validator_base<Json> {
 ...
 public:
  dependent_required_validator(
      ...
      : keyword_validator_base<Json>("dependentRequired",
                                     std::move(schema_path)),
  ...
  void do_validate(const evaluation_context<Json>& eval_context,
    ...
    for (const auto& dep : dependent_required_) {
      ...
      if (prop != instance.object_range().end()) {
        ...
        jsonpointer::json_pointer pointer(instance_location);
        pointer /= dep.first;
        dep.second->validate(this_context, instance, pointer,
                             evaluated_properties, reporter,
                             patch);  // validate
      }
    }
  }
...
class dependent_schemas_validator : public keyword_validator_base<Json> {
 ...
 public:
  dependent_schemas_validator(
      ...
      : keyword_validator_base<Json>("dependentSchemas",
                                     std::move(schema_path)),
  ...
  void do_validate(const evaluation_context<Json>& eval_context,
    ...
    for (const auto& prop : instance.object_range()) {
      ...
      if (property_names_schema_validator_) {
        property_names_schema_validator_->validate(
            this_context, prop.key(), instance_location, evaluated_properties,
            reporter, patch);
      }
      // Any property that doesn't match any of the property names in the
      // properties keyword is ignored by this keyword.
...
class max_contains_keyword : public keyword_base<Json> {
 ...
 public:
  ...
  void validate(const evaluation_context<Json>& eval_context,
    ...
    if (count > max_value_) {
      std::string message("Expected maxContains: " +
...
class min_contains_keyword : public keyword_base<Json> {
 ...
 public:
  ...
  void validate(const evaluation_context<Json>& eval_context,
    ...
    if (count < min_value_) {
      std::string message("Expected minContains: " +
...
class contains_validator : public keyword_validator_base<Json> {
 ...
 private:
  void do_validate(const evaluation_context<Json>& eval_context,
    ...
    for (const auto& item : instance.array_range()) {
      std::size_t mark = local_reporter.errors.size();
      schema_validator_->validate(this_context, item, instance_location,
                                  evaluated_properties, local_reporter, patch);
      if (mark == local_reporter.errors.size()) {
        ++contains_count;
      }
    ...
    if (max_contains_ && min_contains_) {
      max_contains_->validate(this_context, instance_location, contains_count,
                              reporter);
      min_contains_->validate(this_context, instance_location, contains_count,
                              reporter);

...
struct decode_traits {
  ...
  static T decode(basic_staj_cursor<CharT>& cursor,
    ...
    if (ec) {
      ...
      JSONCONS_THROW(ser_error(conv_errc::conversion_failed,
                               cursor.context().line(),
                               cursor.context().column()));
    }
...
struct decode_traits<T, CharT,
  ...
  static T decode(basic_staj_cursor<CharT>& cursor,
    ...
    while (cursor.current().event_type() != staj_event_type::end_array && !ec) {
      v.push_back(
          decode_traits<value_type, CharT>::decode(cursor, decoder, ec));
      if (ec) {
        return T{};
      }
      cursor.next(ec);
...
struct typed_array_visitor : public default_json_visitor {
 ...
 private:
  ...
  bool visit_begin_array(std::size_t size, semantic_tag, const ser_context&,
                         std::error_code& ec) override {
    if (++level_ != 1) {
      ec = conv_errc::not_vector;
      return false;
  ...
  bool visit_half(uint16_t value, semantic_tag, const ser_context&,
                  std::error_code&) override {
    return visit_half_(typename std::integral_constant<
                           bool, std::is_integral<value_type>::value>::type(),
                       value);
  }
  ...
  bool visit_double(double value, semantic_tag, const ser_context&,
                    std::error_code&) override {
    v_.push_back(static_cast<value_type>(value));
    return true;
  }
  ...
  bool visit_typed_array(const jsoncons::span<const value_type>& data,
                         semantic_tag, const ser_context&,
                         std::error_code&) override {
...
struct decode_traits<std::array<T, N>, CharT> {
  ...
  static std::array<T, N> decode(basic_staj_cursor<CharT>& cursor,
    ...
    cursor.next(ec);
    for (std::size_t i = 0;
         i < N && cursor.current().event_type() != staj_event_type::end_array &&
         !ec;
         ++i) {
  ...
  static T decode(basic_staj_cursor<CharT>& cursor,
    ...
    if (cursor.current().size() > 0) {
      reserve_storage(
          typename std::integral_constant<
              bool, extension_traits::has_reserve<T>::value>::type(),
          val, cursor.current().size());
    }
    cursor.next(ec);

    while (cursor.current().event_type() != staj_event_type::end_object &&
           !ec) {
      ...
      cursor.next(ec);
      if (ec) return val;
      val.emplace(std::move(key), decode_traits<mapped_type, CharT>::decode(
                                      cursor, decoder, ec));
      if (ec) {
        return val;
      }
  ...
  static T decode(basic_staj_cursor<CharT>& cursor,
    ...
    while (cursor.current().event_type() != staj_event_type::end_object &&
      ...
      auto s =
          cursor.current()
              .template get<
                  jsoncons::basic_string_view<typename Json::char_type>>(ec);
      if (ec) return val;
      key_type n{0};
      ...
      if (ec) return val;
      val.emplace(
          n, decode_traits<mapped_type, CharT>::decode(cursor, decoder, ec));
      if (ec) {
        return val;
      }
      cursor.next(ec);

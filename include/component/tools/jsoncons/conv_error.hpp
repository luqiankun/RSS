 ...
 public:
  ...
  const char* what() const noexcept override {
    if (what_.empty()) {
      ...
      JSONCONS_CATCH(...) { return std::system_error::what(); }
    } else {
      return what_.c_str();
    }
  }
...
JSONCONS_DEPRECATED_MSG("Instead, use conv_error")
typedef conv_error convert_error;
#endif
...
namespace detail {
class conv_error_category_impl : public std::error_category {
 public:
  const char* name() const noexcept override { return "jsoncons/convert"; }
...
extern inline const std::error_category& conv_error_category() {

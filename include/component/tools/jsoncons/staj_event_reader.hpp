...
#include "./sink.hpp"
#include "./staj_event.hpp"
#include "./typed_array_view.hpp"
...
class basic_item_event {
 ...
 public:
  ...
  basic_item_event(staj_event_type event_kind,
                   semantic_tag tag = semantic_tag::none)
      : event_kind_(event_kind), tag_(tag), ext_tag_(0), value_(), length_(0) {}

  basic_item_event(staj_event_type event_kind, std::size_t length,
                   semantic_tag tag = semantic_tag::none)
  ...
  basic_item_event(null_type, semantic_tag tag)
      : event_kind_(staj_event_type::null_value),
        tag_(tag),
        ext_tag_(0),
        value_(),
        length_(0) {}
  ...
  basic_item_event(bool value, semantic_tag tag)
      : event_kind_(staj_event_type::bool_value),
        tag_(tag),
        ext_tag_(0),
        length_(0) {
  ...
  basic_item_event(int64_t value, semantic_tag tag)
      : event_kind_(staj_event_type::int64_value),
        tag_(tag),
        ext_tag_(0),
        length_(0) {
  ...
  basic_item_event(uint64_t value, semantic_tag tag)
      : event_kind_(staj_event_type::uint64_value),
        tag_(tag),
        ext_tag_(0),
        length_(0) {
  ...
  basic_item_event(half_arg_t, uint16_t value, semantic_tag tag)
      : event_kind_(staj_event_type::half_value),
        tag_(tag),
        ext_tag_(0),
        length_(0) {
  ...
  basic_item_event(double value, semantic_tag tag)
      : event_kind_(staj_event_type::double_value),
        tag_(tag),
        ext_tag_(0),
        length_(0) {
  ...
  basic_item_event(const string_view_type& s, staj_event_type event_kind,
                   semantic_tag tag = semantic_tag::none)
      : event_kind_(event_kind), tag_(tag), ext_tag_(0), length_(s.length()) {
    value_.string_data_ = s.data();
  }

  basic_item_event(const byte_string_view& s, staj_event_type event_kind,
                   semantic_tag tag = semantic_tag::none)
    ...
    switch (event_kind_) {
      case staj_event_type::string_value: {
        ...
        return converter.convert(
            jsoncons::basic_string_view<CharT>(value_.string_data_, length_),
            tag(), ec);
      }
      case staj_event_type::byte_string_value: {
        value_converter<byte_string_view, T> converter;
        return converter.convert(
            byte_string_view(value_.byte_string_data_, length_), tag(), ec);
      }
  ...
  get(std::error_code& ec) const {
    switch (event_kind_) {
      case staj_event_type::string_value: {
        IntegerType val;
        auto result =
            jsoncons::detail::to_integer(value_.string_data_, length_, val);
        if (!result) {
  ...
  template <class T>
  typename std::enable_if<extension_traits::is_bool<T>::value, T>::type get(
      std::error_code& ec) const {
    return as_bool(ec);
  }
...
enum class item_event_reader_state { typed_array = 1, multi_dim, shape };
...
class basic_item_event_receiver : public basic_item_event_visitor<CharT> {
 ...
 public:
  ...
  basic_item_event_receiver(
      std::function<bool(const basic_item_event<CharT>&, const ser_context&)>
          pred)
      : pred_(pred),
        event_(staj_event_type::null_value),
        state_(),
  ...
  const basic_item_event<CharT>& event() const { return event_; }

  bool in_available() const { return state_ != item_event_reader_state(); }
  ...
  bool is_typed_array() const { return data_.type() != typed_array_type(); }

  item_event_reader_state state() const { return state_; }
        ...
        switch (data_.type()) {
          case typed_array_type::uint8_value: {
            this->uint64_value(data_.data(uint8_array_arg)[index_],
                               semantic_tag::none, ser_context(), ec);
            break;
          }
          ...
          case typed_array_type::half_value: {
            this->half_value(data_.data(half_array_arg)[index_],
                             semantic_tag::none, ser_context(), ec);
            break;
          }
  ...
  void advance_multi_dim(std::error_code& ec) {
    if (shape_.size() != 0) {
      if (state_ == item_event_reader_state::multi_dim) {
        ...
        this->uint64_value(shape_[index_], semantic_tag::none, ser_context(),
                           ec);
        ++index_;
      } else {
        state_ = item_event_reader_state();
  ...
  bool dump(basic_item_event_visitor<CharT>& visitor,
 ...
 private:
  static constexpr bool accept(const basic_item_event<CharT>&,
                               const ser_context&) {
    return true;
  }
  ...
  bool visit_begin_object(std::size_t length, semantic_tag tag,
                          ...
                          std::error_code&) override {
    event_ =
        basic_item_event<CharT>(staj_event_type::begin_object, length, tag);
    return !pred_(event_, context);
  }
  ...
  bool visit_string(const string_view_type& s, semantic_tag tag,
                    const ser_context& context, std::error_code&) override {
    event_ = basic_item_event<CharT>(s, staj_event_type::string_value, tag);
    return !pred_(event_, context);
  }

  bool visit_byte_string(const byte_string_view& s, semantic_tag tag,
                         const ser_context& context,
                         std::error_code&) override {
    event_ =
        basic_item_event<CharT>(s, staj_event_type::byte_string_value, tag);
    return !pred_(event_, context);
  }

  bool visit_byte_string(const byte_string_view& s, uint64_t ext_tag,
                         const ser_context& context,
                         std::error_code&) override {
    event_ =
        basic_item_event<CharT>(s, staj_event_type::byte_string_value, ext_tag);
    return !pred_(event_, context);
  }
...
class basic_staj_event_reader {
 public:
  ...
  virtual void array_expected(std::error_code& ec) {
    if (!(current().event_kind() == staj_event_type::begin_array ||
          current().event_kind() == staj_event_type::byte_string_value)) {
...
class basic_staj2_filter_view : basic_staj_event_reader<CharT> {
 ...
 public:
  basic_staj2_filter_view(
      basic_staj_event_reader<CharT>& cursor,
      std::function<bool(const basic_item_event<CharT>&, const ser_context&)>
          pred)
  ...
  bool done() const override { return cursor_->done(); }
  ...
  const ser_context& context() const override { return cursor_->context(); }

  friend basic_staj2_filter_view<CharT> operator|(
      basic_staj2_filter_view& cursor,
      std::function<bool(const basic_item_event<CharT>&, const ser_context&)>
          pred) {

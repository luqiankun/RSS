...
class staj_array_iterator {
 ...
 public:
  ...
  staj_array_iterator() noexcept : view_(nullptr) {}

  staj_array_iterator(staj_array_view<T, Json>& view)
      : view_(std::addressof(view)) {
    if (view_->cursor_->current().event_type() ==
        staj_event_type::begin_array) {
      next();
    } else {
  ...
  bool has_value() const { return !eptr_; }
  ...
  friend bool operator==(const staj_array_iterator& a,
                         const staj_array_iterator& b) {
    return (!a.view_ && !b.view_) || (!a.view_ && b.done()) ||
           (!b.view_ && a.done());
  }
 ...
 private:
  bool done() const {
    return view_->cursor_->done() ||
           view_->cursor_->current().event_type() == staj_event_type::end_array;
  }
  ...
  void next() {
    ...
    next(ec);
    if (ec) {
      JSONCONS_THROW(ser_error(ec, view_->cursor_->context().line(),
                               view_->cursor_->context().column()));
    }
  }
  ...
  void next(std::error_code& ec) {
    if (!done()) {
      ...
      if (!done()) {
        ...
        JSONCONS_TRY {
          view_->value_ = decode_traits<T, char_type>::decode(
              *view_->cursor_, view_->decoder_, ec);
        }
        JSONCONS_CATCH(const conv_error&) { eptr_ = std::current_exception(); }
      }
    }
  }
};
...
class staj_object_iterator {
 ...
 public:
  staj_object_iterator() noexcept : view_(nullptr) {}
  ...
  friend bool operator==(const staj_object_iterator& a,
                         const staj_object_iterator& b) {
    return (!a.view_ && !b.view_) || (!a.view_ && b.done()) ||
           (!b.view_ && a.done());
  }
 ...
 private:
  bool done() const {
    return view_->cursor_->done() || view_->cursor_->current().event_type() ==
                                         staj_event_type::end_object;
  }
  ...
  void next(std::error_code& ec) {
    ...
    if (!done()) {
      JSONCONS_ASSERT(view_->cursor_->current().event_type() ==
                      staj_event_type::key);
      ...
      if (!done()) {
        eptr_ = std::exception_ptr();
        JSONCONS_TRY {
          view_->key_value_ = value_type(
              std::move(key), decode_traits<T, char_type>::decode(
                                  *view_->cursor_, view_->decoder_, ec));
        }
        JSONCONS_CATCH(const conv_error&) { eptr_ = std::current_exception(); }
      }
...
class staj_array_view {
 ...
 public:
  ...
  iterator begin() { return staj_array_iterator<T, Json>(*this); }

  iterator end() { return staj_array_iterator<T, Json>(); }
};
...
class staj_object_view {
 ...
 public:
  ...
  iterator begin() { return staj_object_iterator<Key, T, Json>(*this); }

  iterator end() { return staj_object_iterator<Key, T, Json>(); }
};

template <
    class T, class CharT,
    class Json = typename std::conditional<
        extension_traits::is_basic_json<T>::value, T, basic_json<CharT>>::type>
staj_array_view<T, Json> staj_array(basic_staj_cursor<CharT>& cursor) {
  return staj_array_view<T, Json>(cursor);
}

template <
    class Key, class T, class CharT,
    class Json = typename std::conditional<
        extension_traits::is_basic_json<T>::value, T, basic_json<CharT>>::type>
...
staj_object_view<std::basic_string<CharT>, T, Json> make_object_iterator(

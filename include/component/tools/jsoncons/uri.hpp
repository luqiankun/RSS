...
class uri {
 ...
 public:
  ...
  uri(jsoncons::string_view scheme, jsoncons::string_view userinfo,
      ...
      jsoncons::string_view path, jsoncons::string_view query,
    ...
    if (!userinfo.empty() || !host.empty() || !port.empty()) {
      ...
      if (!scheme.empty()) {
        if (!path.empty() || !query.empty() || !fragment.empty()) {
          ...
          JSONCONS_THROW(
              json_runtime_error<std::invalid_argument>("uri error."));
        }
      }
    }
  ...
  const std::string& string() const { return uri_string_; }

  bool is_absolute() const noexcept { return scheme_.second > scheme_.first; }
  ...
  string_view encoded_userinfo() const noexcept {
    return string_view(uri_string_.data() + userinfo_.first,
                       (userinfo_.second - userinfo_.first));
  }
  ...
  string_view encoded_authority() const noexcept {
    return string_view(uri_string_.data() + userinfo_.first,
                       (port_.second - userinfo_.first));
  }

  std::string path() const { return decode_part(encoded_path()); }

  string_view encoded_path() const noexcept {
    return string_view(uri_string_.data() + path_.first,
                       (path_.second - path_.first));
  }

  std::string query() const { return decode_part(encoded_query()); }

  string_view encoded_query() const noexcept {
    return string_view(uri_string_.data() + query_.first,
                       (query_.second - query_.first));
  }

  std::string fragment() const { return decode_part(encoded_fragment()); }

  string_view encoded_fragment() const noexcept {
    return string_view(uri_string_.data() + fragment_.first,
                       (fragment_.second - fragment_.first));
  }

  std::string authority() const { return decode_part(encoded_authority()); }
  ...
  uri resolve(const uri& base) const {
    ...
    return uri(std::string(base.scheme()), userinfo, host, port, path, query,
               fragment);
  }
 ...
 private:
    ...
    /*
                ...
                while (pos < input.size())
                {
                  if (input.compare(0, 3, "../"))
                  {
                    ...
                    network_boost::replace_head(input, 3, "/");
                    remove_last_segment(result);
                  } else if (network_boost::algorithm::all(input, [](char ch) {
       return ch == '.'; })) { input.clear();
                  }
                  else {
  ...
  static std::string merge_paths(const uri& base, const uri& relative) {
    ...
    if (!relative.encoded_path().empty()) {
      result.append(relative.encoded_path().begin(),
                    relative.encoded_path().end());
    }
 ...
 public:
  // Any character not in the unreserved, punct or escaped categories, and not
  // equal to the slash character ('/') or the  commercial-at character ('@'),
  // is quoted.

  static void encode_path(const jsoncons::string_view& sv,
                          std::string& encoded) {
  ...
  // Any character not in the unreserved, punct, or escaped categories is
  // quoted.

  static void encode_userinfo(const jsoncons::string_view& sv,
                              std::string& encoded) {
    const std::size_t length1 = sv.size() <= 2 ? 0 : sv.size() - 2;
  ...
  static void encode_illegal_characters(const jsoncons::string_view& sv,
                                        std::string& encoded) {
    const std::size_t length1 = sv.size() <= 2 ? 0 : sv.size() - 2;

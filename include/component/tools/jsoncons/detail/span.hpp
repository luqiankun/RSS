 ...
 public:
  ...
  constexpr span(
      ...
      typename std::enable_if<
          !is_span<C>::value && !extension_traits::is_std_array<C>::value &&
          extension_traits::is_compatible_element<C, element_type>::value &&
  ...
  span<element_type, dynamic_extent> subspan(
    ...
    // JSONCONS_ASSERT((offset <= size() && (count == dynamic_extent || (offset
    // + count <= size()))));

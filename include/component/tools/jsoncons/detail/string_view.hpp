...
class basic_string_view {
 ...
 public:
  ...
  JSONCONS_CPP14_CONSTEXPR basic_string_view& operator=(
  ...
  int compare(const basic_string_view& s) const noexcept {
    const int rc =
        Traits::compare(data_, s.data_, (std::min)(length_, s.length_));
    return rc != 0 ? rc
                   : (length_ == s.length_  ? 0
                      : length_ < s.length_ ? -1
                                            : 1);
  ...
  int compare(
    ...
    const int rc =
        Traits::compare(data_, s.data(), (std::min)(length_, s.length()));
    return rc != 0 ? rc
                   : (length_ == s.length()  ? 0
                      : length_ < s.length() ? -1
                                             : 1);
  ...
  size_type find(basic_string_view s, size_type pos = 0) const noexcept {
    ...
    const_iterator it =
        std::search(cbegin() + pos, cend(), s.cbegin(), s.cend(), Traits::eq);
  ...
  size_type find_first_of(basic_string_view s,
    ...
    const_iterator it = std::find_first_of(cbegin() + pos, cend(), s.cbegin(),
                                           s.cend(), Traits::eq);
    return it == cend() ? npos : std::distance(cbegin(), it);
  }

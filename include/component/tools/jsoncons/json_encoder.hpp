...
inline bool is_control_character(uint32_t c) { return c <= 0x1F || c == 0x7f; }
    ...
    switch (c) {
      ...
      default:
        if (escape_solidus && c == '/') {
          ...
          uint32_t cp;
          auto r = unicode_traits::to_codepoint(
              it, end, cp, unicode_traits::conv_flags::strict);
          ...
          if (is_non_ascii_codepoint(cp) || is_control_character(c)) {
            if (cp > 0xFFFF) {
              ...
              sink.push_back('u');
              sink.push_back(
                  jsoncons::detail::to_hex_character(first >> 12 & 0x000F));
              sink.push_back(
                  jsoncons::detail::to_hex_character(first >> 8 & 0x000F));
              sink.push_back(
                  jsoncons::detail::to_hex_character(first >> 4 & 0x000F));
              sink.push_back(
                  jsoncons::detail::to_hex_character(first & 0x000F));
              sink.push_back('\\');
              sink.push_back('u');
              sink.push_back(
                  jsoncons::detail::to_hex_character(second >> 12 & 0x000F));
              sink.push_back(
                  jsoncons::detail::to_hex_character(second >> 8 & 0x000F));
              sink.push_back(
                  jsoncons::detail::to_hex_character(second >> 4 & 0x000F));
              sink.push_back(
                  jsoncons::detail::to_hex_character(second & 0x000F));
              count += 12;
            } else {
              sink.push_back('\\');
              sink.push_back('u');
              sink.push_back(
                  jsoncons::detail::to_hex_character(cp >> 12 & 0x000F));
              sink.push_back(
                  jsoncons::detail::to_hex_character(cp >> 8 & 0x000F));
              sink.push_back(
                  jsoncons::detail::to_hex_character(cp >> 4 & 0x000F));
...
inline byte_string_chars_format resolve_byte_string_chars_format(
    byte_string_chars_format format1, byte_string_chars_format format2,
    byte_string_chars_format default_format =
...
class basic_json_encoder final : public basic_json_visitor<CharT> {
  static const jsoncons::basic_string_view<CharT> null_constant() {
    static const jsoncons::basic_string_view<CharT> k =
        JSONCONS_STRING_VIEW_CONSTANT(CharT, "null");
    return k;
  }
  static const jsoncons::basic_string_view<CharT> true_constant() {
    static const jsoncons::basic_string_view<CharT> k =
        JSONCONS_STRING_VIEW_CONSTANT(CharT, "true");
    return k;
  }
  static const jsoncons::basic_string_view<CharT> false_constant() {
    static const jsoncons::basic_string_view<CharT> k =
        JSONCONS_STRING_VIEW_CONSTANT(CharT, "false");
    return k;
  }
 ...
 private:
  ...
  class encoding_context {
   ...
   public:
    encoding_context(container_type type, line_split_kind split_lines,
                     bool indent_once, std::size_t begin_pos,
                     std::size_t data_pos) noexcept
        : type_(type),
          count_(0),
          line_splits_(split_lines),
          indent_before_(indent_once),
          new_line_after_(false),
    ...
    void set_position(std::size_t pos) { data_pos_ = pos; }

    std::size_t begin_pos() const { return begin_pos_; }

    std::size_t data_pos() const { return data_pos_; }

    std::size_t count() const { return count_; }

    void increment_count() { ++count_; }

    bool new_line_after() const { return new_line_after_; }

    void new_line_after(bool value) { new_line_after_ = value; }

    bool is_object() const { return type_ == container_type::object; }

    bool is_array() const { return type_ == container_type::array; }
    ...
    bool is_indent_once() const { return count_ == 0 ? indent_before_ : false; }
  };
  using encoding_context_allocator_type = typename std::allocator_traits<
 ...
 public:
  basic_json_encoder(Sink&& sink, const Allocator& alloc = Allocator())
      : basic_json_encoder(std::forward<Sink>(sink),
                           basic_json_encode_options<CharT>(), alloc) {}
 ...
 private:
  ...
  bool visit_begin_object(semantic_tag, const ser_context&,
    ...
    if (!stack_.empty())  // object or array
    {
      if (stack_.back().is_object()) {
        ...
        stack_.emplace_back(container_type::object,
                            options_.object_object_line_splits(), false,
      ...
      } else  // array
      {
        ...
        stack_.emplace_back(container_type::object,
                            options_.array_object_line_splits(), false, column_,
    ...
    sink_.append(open_object_brace_str_.data(),
                 open_object_brace_str_.length());
    column_ += open_object_brace_str_.length();
    return true;
  }
  ...
  bool visit_end_object(const ser_context&, std::error_code&) override {
    ...
    stack_.pop_back();
    sink_.append(close_object_brace_str_.data(),
                 close_object_brace_str_.length());
  ...
  bool visit_begin_array(semantic_tag, const ser_context&,
    ...
    if (!stack_.empty()) {
      if (stack_.back().is_object()) {
        switch (options_.object_array_line_splits()) {
          case line_split_kind::same_line:
            stack_.emplace_back(
                container_type::array, options_.object_array_line_splits(),
                false, column_, column_ + open_array_bracket_str_.length());
            break;
          case line_split_kind::new_line: {
            stack_.emplace_back(
                container_type::array, options_.object_array_line_splits(),
                true, column_, column_ + open_array_bracket_str_.length());
            break;
          }
          default:  // multi_line
            stack_.emplace_back(
      ...
      } else  // array
      {
        switch (options_.array_array_line_splits()) {
          case line_split_kind::same_line:
            ...
            stack_.emplace_back(
                container_type::array, options_.array_array_line_splits(),
                false, column_, column_ + open_array_bracket_str_.length());
            break;
          case line_split_kind::new_line:
            stack_.back().new_line_after(true);
    ...
    sink_.append(open_array_bracket_str_.data(),
                 open_array_bracket_str_.length());
    column_ += open_array_bracket_str_.length();
    return true;
  }
  ...
  bool visit_end_array(const ser_context&, std::error_code&) override {
    ...
    stack_.pop_back();
    sink_.append(close_array_bracket_str_.data(),
                 close_array_bracket_str_.length());
  ...
  bool visit_key(const string_view_type& name, const ser_context&,
    ...
    if (stack_.back().is_multi_line()) {
      stack_.back().new_line_after(true);
      new_line();
    } else if (stack_.back().count() > 0 &&
               column_ >= options_.line_length_limit()) {
      // stack_.back().new_line_after(true);
    ...
    sink_.push_back('\"');
    std::size_t length = jsoncons::detail::escape_string(
        name.data(), name.length(), options_.escape_all_non_ascii(),
  ...
  bool visit_string(const string_view_type& sv, semantic_tag tag,
    ...
    switch (tag) {
      ...
      default: {
        sink_.push_back('\"');
        std::size_t length = jsoncons::detail::escape_string(
            sv.data(), sv.length(), options_.escape_all_non_ascii(),
            options_.escape_solidus(), sink_);
        sink_.push_back('\"');
        column_ += (length + 2);
        break;
  ...
  bool visit_byte_string(const byte_string_view& b, semantic_tag tag,
    ...
    byte_string_chars_format format =
        jsoncons::detail::resolve_byte_string_chars_format(
            options_.byte_string_format(), encoding_hint,
            byte_string_chars_format::base64url);
    switch (format) {
      case byte_string_chars_format::base16: {
        sink_.push_back('\"');
  ...
  void indent() { indent_amount_ += static_cast<int>(options_.indent_size()); }
  ...
  void new_line() {
    sink_.append(options_.new_line_chars().data(),
                 options_.new_line_chars().length());
...
class basic_compact_json_encoder final : public basic_json_visitor<CharT> {
 ...
 private:
  ...
  class encoding_context {
   ...
   public:
    encoding_context(container_type type) noexcept : type_(type), count_(0) {}
  ...
  basic_compact_json_encoder& operator=(const basic_compact_json_encoder&) =
      delete;

 public:
  basic_compact_json_encoder(Sink&& sink, const Allocator& alloc = Allocator())
      : basic_compact_json_encoder(std::forward<Sink>(sink),
                                   basic_json_encode_options<CharT>(), alloc) {}
 ...
 private:
  ...
  bool visit_key(const string_view_type& name, const ser_context&,
    ...
    sink_.push_back('\"');
    jsoncons::detail::escape_string(name.data(), name.length(),
...
using json_stream_encoder =
    basic_json_encoder<char, jsoncons::stream_sink<char>>;
using wjson_stream_encoder =
    basic_json_encoder<wchar_t, jsoncons::stream_sink<wchar_t>>;
using compact_json_stream_encoder =
    basic_compact_json_encoder<char, jsoncons::stream_sink<char>>;
using compact_wjson_stream_encoder =
    basic_compact_json_encoder<wchar_t, jsoncons::stream_sink<wchar_t>>;

using json_string_encoder =
    basic_json_encoder<char, jsoncons::string_sink<std::string>>;
using wjson_string_encoder =
    basic_json_encoder<wchar_t, jsoncons::string_sink<std::wstring>>;
using compact_json_string_encoder =

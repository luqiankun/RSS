...
template <>
struct utest_type_deducer<signed char, false> {
...
template <>
struct utest_type_deducer<unsigned char, false> {
...
template <>
struct utest_type_deducer<short, false> {
  static void _(const short s) { UTEST_PRINTF("%d", static_cast<int>(s)); }
};

template <>
struct utest_type_deducer<unsigned short, false> {
...
template <>
struct utest_type_deducer<float, false> {
  static void _(const float f) { UTEST_PRINTF("%f", static_cast<double>(f)); }
};

template <>
struct utest_type_deducer<double, false> {
  static void _(const double d) { UTEST_PRINTF("%f", d); }
};

template <>
struct utest_type_deducer<long double, false> {
...
template <typename T>
struct utest_type_deducer<T, true> {
  static void _(const T t) {
...
template <>
struct utest_type_deducer<std::nullptr_t, false> {
...
UTEST_WEAK UTEST_OVERLOADABLE void utest_type_printer(
...
static UTEST_INLINE char *utest_strncpy_gcc(char *const dst,
...
#define UTEST_COND(x, y, cond, msg, is_assert)                              \
  ...
  while (0) UTEST_SURPRESS_WARNING_END
...
#define UTEST(SET, NAME)                                                    \
  ...
  UTEST_INITIALIZER(utest_register_##SET##_##NAME) {                        \
    ...
    utest_state.tests = UTEST_PTR_CAST(                                     \
        struct utest_test_state_s *,                                        \
        utest_realloc(                                                      \
            UTEST_PTR_CAST(void *, utest_state.tests),                      \
            sizeof(struct utest_test_state_s) * utest_state.tests_length)); \
...
UTEST_WEAK inline int utest_should_filter_test(const char *filter,
                                               const char *testcase) {
  if (filter) {

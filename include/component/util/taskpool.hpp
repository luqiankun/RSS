...
class future_t {
 public:
  ...
  future_t(std::shared_ptr<_future_temp_data_t<T>> _data) noexcept
      : m_data(_data) {}
  future_t<T> &operator=(const future_t<T> &_o) noexcept {
    m_data = _o.m_data;
    return *this;
  }
  static future_t<T> from_value(const T &t) {
    return future_t<T>(std::make_shared<_future_temp_data_t<T>>(t));
  }
  static future_t<T> from_value(T &&t) {
    return future_t<T>(std::make_shared<_future_temp_data_t<T>>(std::move(t)));
  }
...
class future_t<void> {
 public:
  ...
  future_t(std::shared_ptr<_future_temp_data_t<void>> _data) noexcept
      : m_data(_data) {}
  future_t<void> &operator=(const future_t<void> &_o) noexcept {
    m_data = _o.m_data;
    return *this;
  }
...
class promise_t {
 ...
 private:
  std::shared_ptr<_future_temp_data_t<T>> m_data =
      std::make_shared<_future_temp_data_t<T>>();
};
...
class promise_t<void> {
 ...
 private:
  std::shared_ptr<_future_temp_data_t<void>> m_data =
      std::make_shared<_future_temp_data_t<void>>();
};
...
class futures_t {
 public:
  futures_t(std::vector<future_t<T>> &&_futures)
      : m_futures(std::move(_futures)) {}
  bool peek() noexcept {
    for (future_t<T> &_future : m_futures) {
      if (!_future.peek()) return false;
    }
    return true;
  }
  std::vector<T> get() {
    ...
    for (future_t<T> &_future : m_futures) _v.emplace_back(_future.get());
    return _v;
  }
...
class futures_t<void> {
 public:
  futures_t(std::vector<future_t<void>> &&_futures)
      : m_futures(std::move(_futures)) {}
  bool peek() noexcept {
  ...
  void get() {
    for (future_t<void> &_future : m_futures) _future.get();
  }
...
class taskpool_t {
 public:
  ...
  auto async_after_run(future_t<void> &&_future, F &&f)
    ...
    std::shared_ptr<promise_t<TRet>> _promise =
        std::make_shared<promise_t<TRet>>();
    ...
    [&]() {
      ...
      for (size_t i = 0; i < m_timed_tasks.size(); ++i) {
        if (std::get<0>(m_timed_tasks[i]) > _tp) {
          m_timed_tasks.insert(
              m_timed_tasks.begin() + i,
              std::make_tuple(_tp, [_task]() { _task->invoke(); }));
          return;
        }
      }
      m_timed_tasks.push_back(
          std::make_tuple(_tp, [_task]() { _task->invoke(); }));
    }();
 ...
 private:
  ...
  future_t<T> async_wait(T &&t,
                         const std::chrono::duration<_Rep, _Period> &_chr) {
    auto _tp = std::chrono::system_clock::now() + _chr;
    auto _task = std::make_shared<promise_pack_t<T>>(
        std::bind([](const T &t) { return std::move(t); }, std::move(t)));
    [&]() {
      std::unique_lock<std::recursive_mutex> ul(m_mutex);
      for (size_t i = 0; i < m_timed_tasks.size(); ++i) {
        if (std::get<0>(m_timed_tasks[i]) > _tp) {
 ...
 public:
  template <typename _Rep, typename _Period>
  future_t<void> async_after_wait(
      future_t<void> &&_future,
      const std::chrono::duration<_Rep, _Period> &_chr) {
    auto _future_wrap = std::make_shared<future_t<void>>(std::move(_future));
    std::shared_ptr<promise_t<void>> _promise =
        std::make_shared<promise_t<void>>();
    auto _f = [this, _chr]() -> future_t<void> {
      return async_wait<_Rep, _Period>(_chr);
    };
  ...
  template <typename _Rep, typename _Period, typename T>
  future_t<T> async_after_wait(
      future_t<T> &&_future, const std::chrono::duration<_Rep, _Period> &_chr) {
    auto _future_wrap = std::make_shared<future_t<T>>(std::move(_future));
    std::shared_ptr<promise_t<T>> _promise = std::make_shared<promise_t<T>>();
    auto _f = [this, _chr](T &&t) -> future_t<T> {
      return async_wait<_Rep, _Period, T>(std::move(t), _chr);
    };
  ...
  template <typename F, typename... Args>
  auto sync_run(std::shared_ptr<std::recursive_mutex> _mutex, F &&f,
                Args &&...args) -> future_t<decltype(f(args...))> {
    using TRet = decltype(f(args...));
    std::shared_ptr<promise_t<TRet>> _promise =
        std::make_shared<promise_t<TRet>>();
    std::function<TRet()> _f =
        std::bind(std::forward<F>(f), std::forward<Args>(args)...);
    if constexpr (std::is_void<TRet>::value) {
  ...
  auto sync_after_run(future_t<void> &&_future,
    ...
    if constexpr (std::is_void<TRet>::value) {
      ...
      _wait_sync_run2_noret<decltype(_f)>(std::move(_f), _future_wrap,
                                          _promise);
    } else {
      auto _f = [=]() { return sync_run(_mutex, f); };
  ...
  auto sync_after_run(future_t<T> &&_future,
    ...
    if constexpr (std::is_void<decltype(f(_future.get()))>::value) {
      ...
      _wait_sync_run2_noret<decltype(_f), T>(std::move(_f), _future_wrap,
                                             _promise);
    } else {
      auto _f = [=](T &&t) { return sync_run(_mutex, f, std::move(t)); };
 ...
 private:
  ...
  auto _run_timed_task_until(std::chrono::system_clock::time_point _tp, F &&f)
    ...
    std::unique_lock<std::recursive_mutex> ul(m_mutex, std::defer_lock);
    auto _task =
        std::make_shared<promise_pack_t<TRet>>(std::bind(std::forward<F>(f)));
    ul.lock();
    for (size_t i = 0; i < m_timed_tasks.size(); ++i) {
      if (std::get<0>(m_timed_tasks[i]) > _tp) {
        m_timed_tasks.insert(
  ...
  bool _timed_trigger() {
    if (m_timed_tasks.empty()) return false;
    return std::chrono::system_clock::now() >= std::get<0>(m_timed_tasks[0]);
  }

  void _wait_all0(std::shared_ptr<futures_t<void>> _futures_wrap,
                  std::shared_ptr<promise_t<void>> _promise) {
    if (_futures_wrap->peek()) {
      ...
      _run_timed_task(
          std::bind(&taskpool_t::_wait_all0, this, _futures_wrap, _promise));
    }
  }

  template <typename T>
  void _wait_all(std::shared_ptr<futures_t<T>> _futures_wrap,
                 std::shared_ptr<promise_t<std::vector<T>>> _promise) {
    if (_futures_wrap->peek()) {
      _promise->set_value(std::move(_futures_wrap->get()));
    } else {
      _run_timed_task(
          std::bind(&taskpool_t::_wait_all<T>, this, _futures_wrap, _promise));
    }
  }

  template <typename F>
  void _wait_forward(const F &f, std::shared_ptr<future_t<void>> _future_wrap,
                     std::shared_ptr<promise_t<decltype(f())>> _promise) {
    using TRet = decltype(f());
    if (_future_wrap->peek()) {
      _future_wrap->get();
      _promise->set_value(std::forward<TRet>((f)()));
    } else {
      _run_timed_task(std::bind(&taskpool_t::_wait_forward<F>, this,
                                std::move(f), _future_wrap, _promise));
    }
  }

  template <typename F>
  void _wait_forward_noret(const F &f,
                           std::shared_ptr<future_t<void>> _future_wrap,
                           std::shared_ptr<promise_t<void>> _promise) {
    if (_future_wrap->peek()) {
      _future_wrap->get();
      (f)();
      ...
      _run_timed_task(std::bind(&taskpool_t::_wait_forward_noret<F>, this,
                                std::move(f), _future_wrap, _promise));
    }
  }
  ...
  template <typename F, typename T>
  void _wait_forward(
      const F &f, std::shared_ptr<future_t<T>> _future_wrap,
      std::shared_ptr<promise_t<decltype(f(_future_wrap->get()))>> _promise) {
    using TRet = decltype(f(_future_wrap->get()));
    if (_future_wrap->peek()) {
      _promise->set_value(std::forward<TRet>((f)(_future_wrap->get())));
    } else {
      _run_timed_task(std::bind(&taskpool_t::_wait_forward<F, T>, this,
                                std::move(f), _future_wrap, _promise));
    }
  }
  ...
  template <typename F, typename T>
  void _wait_forward_noret(const F &f,
                           std::shared_ptr<future_t<T>> _future_wrap,
                           std::shared_ptr<promise_t<void>> _promise) {
    if (_future_wrap->peek()) {
      (f)(_future_wrap->get());
      _promise->set_value();
    } else {
      _run_timed_task(std::bind(&taskpool_t::_wait_forward_noret<F, T>, this,
                                std::move(f), _future_wrap, _promise));
    }
  }
  ...
  template <typename F>
  void _wait_forward2(
      const F &f, std::shared_ptr<future_t<void>> _future_wrap,
      std::shared_ptr<promise_t<decltype(f().get())>> _promise) {
    using TRet = decltype(f().get());
    if (_future_wrap->peek()) {
      _future_wrap->get();
      future_t<TRet> _future2 = (f)();
      auto _future2_wrap =
          std::make_shared<future_t<TRet>>(std::move(_future2));
      auto _f2 = [](TRet &&tret) { return std::forward<TRet>(tret); };
      _wait_forward<decltype(_f2)>(std::move(_f2), _future2_wrap, _promise);
    } else {
      _run_timed_task(std::bind(&taskpool_t::_wait_forward2<F>, this,
                                std::move(f), _future_wrap, _promise));
    }
  }

  template <typename F>
  void _wait_forward2_noret(const F &f,
                            std::shared_ptr<future_t<void>> _future_wrap,
                            std::shared_ptr<promise_t<void>> _promise) {
    if (_future_wrap->peek()) {
      _future_wrap->get();
      ...
      auto _future2_wrap =
          std::make_shared<future_t<void>>(std::move(_future2));
      auto _f2 = []() {};
      _wait_forward_noret<decltype(_f2)>(std::move(_f2), _future2_wrap,
                                         _promise);
    } else {
      _run_timed_task(std::bind(&taskpool_t::_wait_forward2_noret<F>, this,
                                std::move(f), _future_wrap, _promise));
    }
  }

  template <typename F, typename T>
  void _wait_forward2(const F &f, std::shared_ptr<future_t<T>> _future_wrap,
                      std::shared_ptr<promise_t<typename std::remove_reference<
                          decltype(f(_future_wrap->get()).get())>::type>>
                          _promise) {
    using TRet = typename std::remove_reference<
        decltype(f(_future_wrap->get()).get())>::type;
    if (_future_wrap->peek()) {
      ...
      _run_timed_task(std::bind(&taskpool_t::_wait_forward2<F, T>, this,
                                std::move(f), _future_wrap, _promise));
    }
  }
  ...
  void _wait_forward2_noret(const F &f,
    ...
    if (_future_wrap->peek()) {
      ...
      auto _future2_wrap =
          std::make_shared<future_t<void>>(std::move(_future2));
      auto _f2 = []() {};
      _wait_forward_noret<decltype(_f2), T>(std::move(_f2), _future2_wrap,
                                            _promise);
    } else {
      _run_timed_task(std::bind(&taskpool_t::_wait_forward2_noret<F, T>, this,
                                std::move(f), _future_wrap, _promise));
    }
  }

  template <typename F, typename TRet>
  void _wait_sync_run(std::shared_ptr<std::recursive_mutex> _mutex,
                      std::shared_ptr<promise_t<TRet>> _promise, const F &f) {
    if (_mutex->try_lock()) {
      TRet _t = f();
      _promise->set_value(std::move(_t));
      _mutex->unlock();
    } else {
      _run_timed_task(std::bind(&taskpool_t::_wait_sync_run<F, TRet>, this,
                                _mutex, _promise, std::move(f)));
    }
  }

  template <typename F>
  void _wait_sync_run_noret(std::shared_ptr<std::recursive_mutex> _mutex,
                            std::shared_ptr<promise_t<void>> _promise,
                            const F &f) {
    if (_mutex->try_lock()) {
      f();
      ...
      _run_timed_task(std::bind(&taskpool_t::_wait_sync_run_noret<F>, this,
                                _mutex, _promise, std::move(f)));
    }
  }
  ...
  void _wait_sync_run(std::shared_ptr<std::recursive_mutex> _mutex,
    ...
    if (_mutex->try_lock()) {
      ...
      _run_timed_task(std::bind(&taskpool_t::_wait_sync_run<F, T, TRet>, this,
                                std::move(_mutex), std::move(_promise),
  ...
  void _wait_sync_run_noret(std::shared_ptr<std::recursive_mutex> _mutex,
    ...
    if (_mutex->try_lock()) {
      ...
      _run_timed_task(std::bind(&taskpool_t::_wait_sync_run_noret<F, T>, this,
                                std::move(_mutex), std::move(_promise),
                                std::move(f), std::move(t)));
    }
  }
  ...
  void _wait_sync_run2(
    ...
    if (_future_wrap->peek()) {
      ...
      auto _future2_wrap =
          std::make_shared<future_t<TRet>>(std::move(_future2));
      auto _f2 = [](TRet &&tret) { return std::forward<TRet>(tret); };
      _wait_forward<decltype(_f2), TRet>(std::move(_f2), _future2_wrap,
                                         _promise);
    } else {
      _run_timed_task(std::bind(&taskpool_t::_wait_sync_run2<F>, this,
                                std::move(f), _future_wrap, _promise));
    }
  }

  template <typename F>
  void _wait_sync_run2_noret(const F &f,
                             std::shared_ptr<future_t<void>> _future_wrap,
                             std::shared_ptr<promise_t<void>> _promise) {
    using TRet = decltype(f().get());
    if (_future_wrap->peek()) {
      ...
      _run_timed_task(std::bind(&taskpool_t::_wait_sync_run2_noret<F>, this,
                                std::move(f), _future_wrap, _promise));
    }
  }
  ...
  void _wait_sync_run2(
      const F &f, std::shared_ptr<future_t<T>> _future_wrap,
      std::shared_ptr<promise_t<decltype(f(_future_wrap->get()).get())>>
          _promise) {
    ...
    if (_future_wrap->peek()) {
      ...
      _run_timed_task(std::bind(&taskpool_t::_wait_sync_run2<F, T>, this,
                                std::move(f), _future_wrap, _promise));
    }
  }
  ...
  void _wait_sync_run2_noret(const F &f,
    ...
    if (_future_wrap->peek()) {
      ...
      _run_timed_task(std::bind(&taskpool_t::_wait_sync_run2_noret<F, T>, this,
                                std::move(f), _future_wrap, _promise));
    }
  }
  ...
  std::vector<
      std::tuple<std::chrono::system_clock::time_point, std::function<void()>>>
      m_timed_tasks;

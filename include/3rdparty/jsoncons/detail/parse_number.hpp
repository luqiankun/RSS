...
enum class to_integer_errc : uint8_t {
  success = 0,
  overflow,
  invalid_digit,
  invalid_number
...
class to_integer_error_category_impl : public std::error_category {
 public:
...
inline const std::error_category& to_integer_error_category() {
  static to_integer_error_category_impl instance;
  return instance;
}

inline std::error_code make_error_code(to_integer_errc e) {
...
namespace std {
template <>
struct is_error_code_enum<jsoncons::detail::to_integer_errc>
    : public true_type {};
}  // namespace std
...
struct to_integer_result {
  ...
  std::error_code error_code() const { return make_error_code(ec); }
};

enum class integer_chars_format : uint8_t { decimal = 1, hex };
enum class integer_chars_state {
  initial,
  minus,
  integer,
  binary,
...
template <typename T, typename CharT>
typename std::enable_if<extension_traits::integer_limits<T>::is_specialized &&
                            !extension_traits::integer_limits<T>::is_signed,
                        to_integer_result<T, CharT>>::type
decimal_to_integer(const CharT* s, std::size_t length, T& n) {
      ...
      case integer_chars_state::initial: {
        switch (*s) {
          case '0':
            if (++s == end) {
              return (++s == end)
                         ? to_integer_result<T, CharT>(s)
                         : to_integer_result<T, CharT>(s, to_integer_errc());
            } else {
              return to_integer_result<T, CharT>(
                  s, to_integer_errc::invalid_digit);
            }
          case '1':
          case '2':
          case '3':
          ...
          case '7':
          case '8':
          case '9':  // Must be decimal
            state = integer_chars_state::decimal;
            break;
          default:
            return to_integer_result<T, CharT>(s,
                                               to_integer_errc::invalid_digit);
        }
        break;
      }
      case integer_chars_state::decimal: {
        static constexpr T max_value =
            (extension_traits::integer_limits<T>::max)();
  ...
  return (state == integer_chars_state::initial)
             ? to_integer_result<T, CharT>(s, to_integer_errc::invalid_number)
             : to_integer_result<T, CharT>(s, to_integer_errc());
}
...
decimal_to_integer(const CharT* s, std::size_t length, T& n) {
  ...
  if (is_negative) {
    if (u > static_cast<U>(
                -((extension_traits::integer_limits<T>::lowest)() + T(1))) +
                U(1)) {
    ...
    switch (state) {
      case integer_chars_state::initial: {
        switch (*s) {
          case '0':
            state =
                integer_chars_state::integer;  // Could be binary, octal, hex
            ++s;
            break;
          case '1':
          case '2':
...
to_integer_unchecked(const CharT* s, std::size_t length, T& n) {
  static_assert(extension_traits::integer_limits<T>::is_specialized,
                "Integer type not specialized");
  ...
  if (*s == '-') {
    static constexpr T min_value =
        (extension_traits::integer_limits<T>::lowest)();
...
class chars_to {
 public:
  char get_decimal_point() const { return '.'; }
  ...
  operator()(const CharT* s, std::size_t len) const {
    ...
    if (res.ec != std::errc()) {
      JSONCONS_THROW(json_runtime_error<std::invalid_argument>(
          "Convert chars to double failed"));
    }
    return val;
  }
...
class chars_to {
 ...
 public:
  chars_to() { locale_ = _create_locale(LC_NUMERIC, "C"); }
  ~chars_to() noexcept { _free_locale(locale_); }

  chars_to(const chars_to&) { locale_ = _create_locale(LC_NUMERIC, "C"); }
  ...
  operator()(const CharT* s, std::size_t) const {
    ...
    if (s == end) {
      JSONCONS_THROW(json_runtime_error<std::invalid_argument>(
          "Convert string to double failed"));
    }
    return val;
  }
...
class chars_to {
 ...
 public:
  chars_to() { locale_ = newlocale(LC_ALL_MASK, "C", (locale_t)0); }
  ~chars_to() noexcept { freelocale(locale_); }

  chars_to(const chars_to&) {
    locale_ = newlocale(LC_ALL_MASK, "C", (locale_t)0);
  }

  chars_to& operator=(const chars_to&) { return *this; }
...
class chars_to {
 ...
 public:
  ...
  char get_decimal_point() const { return decimal_point_; }

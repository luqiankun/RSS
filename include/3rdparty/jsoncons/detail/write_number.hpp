...
inline char to_hex_character(uint8_t c) {
...
void prettify_string(const char* buffer, std::size_t length, int k, int min_exp,
  ...
  */
  ...
  if (nb_digits <= kk && kk <= max_exp) {
    ...
    /* the max_exp is a personal choice. Only 16 digits could possibly be
     * relevant. Basically we want to print 12340000000 rather than 1234.0e7
     * or 1.234e10 */
    ...
    result.push_back('0');
    result.push_back('.');
    for (int i = 2; i < offset; ++i) result.push_back('0');
    for (int i = 0; i < nb_digits; ++i) {
      result.push_back(buffer[i]);
    }
  } else if (nb_digits == 1) {
...
bool dtoa_general(double v, char decimal_point, Result& result,
  ...
  if (jsoncons::detail::grisu3(u, buffer, &length, &k)) {
    ...
    jsoncons::detail::prettify_string(buffer, length, k, -4,
...
bool dtoa_fixed(double v, char decimal_point, Result& result, std::true_type) {
  ...
  if (jsoncons::detail::grisu3(u, buffer, &length, &k)) {
    ...
    jsoncons::detail::prettify_string(
        buffer, length, k, std::numeric_limits<int>::lowest(),
        (std::numeric_limits<int>::max)(), result);
    return true;
  } else {
  ...
  std::size_t operator()(double val, Result& result) {
    ...
    switch (float_format_) {
      case float_chars_format::fixed: {
        if (precision_ > 0) {
          length = snprintf(number_buffer, sizeof(number_buffer), "%1.*f",
                            precision_, val);
          if (length < 0) {
      ...
      case float_chars_format::scientific: {
        if (precision_ > 0) {
          length = snprintf(number_buffer, sizeof(number_buffer), "%1.*e",
                            precision_, val);
          if (length < 0) {
            JSONCONS_THROW(json_runtime_error<std::invalid_argument>(
                "write_double failed."));
      ...
      case float_chars_format::general: {
        if (precision_ > 0) {
          length = snprintf(number_buffer, sizeof(number_buffer), "%1.*g",
                            precision_, val);
          if (length < 0) {
            JSONCONS_THROW(json_runtime_error<std::invalid_argument>(
                "write_double failed."));
      ...
      default:
        JSONCONS_THROW(
            json_runtime_error<std::invalid_argument>("write_double failed."));
        break;
    }
    return count;
  }

...
class optional {
 ...
 public:
  ...
  // copy constructors
  optional(const optional<T>& other) : has_value_(false), dummy_{} {
    if (other) {
      construct(*other);
    }
  }
  ...
  template <
      ...
      typename std::enable_if<
          !std::is_same<T, U>::value &&
              ...
              std::is_copy_constructible<typename std::decay<U>::type>::value,
          int>::type = 0>
  optional(const optional<U>& other) : has_value_(false), dummy_{} {
    if (other) {
      construct(*other);
    }
  }
  ...
  template <
      ...
      typename std::enable_if<
          !std::is_same<T, U>::value &&
              ...
              std::is_copy_constructible<typename std::decay<U>::type>::value,
          int>::type = 0>
  explicit optional(const optional<U>& other) : has_value_(false), dummy_{} {
    if (other) {
      construct(*other);
    }
  }
  ...
  optional(optional<T>&& other,
           typename std::enable_if<std::is_move_constructible<
               typename std::decay<T2>::type>::value>::type* = 0)
  ...
  optional(
      T2&& value,
      typename std::enable_if<
          !std::is_same<optional<T>, typename std::decay<T2>::type>::value &&
  ...
  typename std::enable_if<
      !std::is_same<optional<T>, U>::value &&
          std::is_constructible<T, const U&>::value &&
          !is_constructible_convertible_or_assignable_from_optional<T,
                                                                    U>::value &&
...
#pragma warning(pop)
  ...
  const T* operator->() const { return std::addressof(this->value_); }

  T* operator->() { return std::addressof(this->value_); }

  JSONCONS_CONSTEXPR const T& operator*() const& { return value(); }
...
template <typename T1, typename T2>
constexpr bool operator==(const optional<T1>& lhs,
                          const optional<T2>& rhs) noexcept {

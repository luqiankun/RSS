...
inline char* align_up(char* ptr, std::size_t alignment) noexcept {
...
struct heap_string : public heap_string_base<Extra, Allocator> {
  ...
  using allocator_type =
      typename std::allocator_traits<Allocator>::template rebind_alloc<CharT>;
  ...
  const char_type* c_str() const {
    return extension_traits::to_plain_pointer(p_);
  }
...
class heap_string_factory {
 ...
 private:
  ...
  using heap_string_allocator_type = typename std::allocator_traits<
      Allocator>::template rebind_alloc<heap_string_type>;

 public:
  using pointer =
      typename std::allocator_traits<heap_string_allocator_type>::pointer;
  ...
  typedef
      typename jsoncons_aligned_storage<sizeof(storage_t),
                                        alignof(storage_t)>::type storage_type;

  static size_t aligned_size(std::size_t n) { return sizeof(storage_type) + n; }

 public:
  static pointer create(const char_type* s, std::size_t length, Extra extra,

...
class basic_item_event_visitor_to_json_visitor
 ...
 private:
  ...
  struct level {
   ...
   public:
    level(target_t state, container_t type) noexcept
        : state_(state),
          type_(type),
          even_odd_(type == container_t::object ? 0 : 1),
          count_(0) {}
    ...
    bool is_key() const { return even_odd_ == 0; }

    bool is_object() const { return type_ == container_t::object; }

    target_t target() const { return state_; }

    std::size_t count() const { return count_; }
  };
  using level_allocator_type =
      typename std::allocator_traits<Allocator>::template rebind_alloc<level>;
  ...
  basic_item_event_visitor_to_json_visitor(
      const basic_item_event_visitor_to_json_visitor&) = delete;
  basic_item_event_visitor_to_json_visitor& operator=(
      const basic_item_event_visitor_to_json_visitor&) = delete;

 public:
  explicit basic_item_event_visitor_to_json_visitor(
      ...
      : default_visitor_(),
        ...
        key_buffer_(alloc),
        level_stack_(alloc) {
    level_stack_.emplace_back(target_t::destination,
                              container_t::root);  // root
  }
 ...
 private:
  ...
  bool visit_begin_object(semantic_tag tag, const ser_context& context,
    ...
    if (level_stack_.back().is_key()) {
      if (level_stack_.back().target() == target_t::buffer &&
          level_stack_.back().count() > 0) {
  ...
  bool visit_begin_object(std::size_t length, semantic_tag tag,
    ...
    if (level_stack_.back().is_key()) {
      ...
      switch (level_stack_.back().target()) {
        case target_t::buffer:
          if (!level_stack_.back().is_object() &&
              level_stack_.back().count() > 0) {
            key_buffer_.push_back(',');
          }
          level_stack_.emplace_back(target_t::buffer, container_t::object);
  ...
  bool visit_byte_string(const byte_string_view& value, semantic_tag tag,
                         const ser_context& context,
                         std::error_code& ec) override {
    if (level_stack_.back().is_key() ||
        level_stack_.back().target() == target_t::buffer) {
      key_.clear();
      switch (tag) {
  ...
  bool visit_byte_string(const byte_string_view& value, uint64_t ext_tag,
                         const ser_context& context,
                         std::error_code& ec) override {
    if (level_stack_.back().is_key() ||
        level_stack_.back().target() == target_t::buffer) {
  ...
  bool visit_typed_array(const jsoncons::span<const uint8_t>& s,
    ...
    if (is_key || level_stack_.back().target() == target_t::buffer) {
      return basic_item_event_visitor<CharT>::visit_typed_array(s, tag, context,
                                                                ec);
    } else {
  ...
  bool visit_typed_array(half_arg_t, const jsoncons::span<const uint16_t>& s,
    ...
    if (is_key || level_stack_.back().target() == target_t::buffer) {
      return basic_item_event_visitor<CharT>::visit_typed_array(
          half_arg, s, tag, context, ec);
    } else {
...
class basic_default_item_event_visitor
 ...
 private:
  ...
  bool visit_begin_object(std::size_t, semantic_tag, const ser_context&,
                          std::error_code& ec) override {
    if (ec_) {
      ec = ec_;
    }
  ...
  bool visit_begin_array(std::size_t, semantic_tag, const ser_context&,
                         std::error_code& ec) override {
    if (ec_) {
      ec = ec_;
    }
...
class basic_json_visitor_to_item_event_visitor
 ...
 private:
  ...
  // noncopyable and nonmoveable
  basic_json_visitor_to_item_event_visitor(
      const basic_json_visitor_to_item_event_visitor&) = delete;
  basic_json_visitor_to_item_event_visitor& operator=(
      const basic_json_visitor_to_item_event_visitor&) = delete;

 public:
  basic_json_visitor_to_item_event_visitor(
      basic_item_event_visitor<char_type>& visitor)
      : destination_(visitor) {}

  basic_item_event_visitor<char_type>& destination() { return destination_; }
...
class diagnostics_visitor2 : public basic_default_item_event_visitor<char> {
  ...
  bool visit_typed_array(const jsoncons::span<const uint16_t>& s,
                         semantic_tag tag, const ser_context&,
                         std::error_code&) override {
...
using item_event_visitor_to_visitor_adaptor =
    basic_item_event_visitor_to_json_visitor<char>;

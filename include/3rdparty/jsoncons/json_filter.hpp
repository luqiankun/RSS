...
class basic_json_filter : public basic_json_visitor<CharT> {
 ...
 private:
  ...
  bool visit_byte_string(const byte_string_view& b, uint64_t ext_tag,
                         const ser_context& context,
                         std::error_code& ec) override {
...
class basic_rename_object_key_filter : public basic_json_filter<CharT> {
 ...
 public:
  basic_rename_object_key_filter(const std::basic_string<CharT>& name,
      ...
      : basic_json_filter<CharT>(visitor), name_(name), new_name_(new_name) {}
...
class json_visitor_adaptor_base : public From {
 ...
 private:
  ...
  json_visitor_adaptor_base& operator=(const json_visitor_adaptor_base&) =
      delete;
 ...
 public:
  ...
  To& destination() { return *destination_; }
...
class json_visitor_adaptor<
    ...
    typename std::enable_if<extension_traits::is_narrow_character<
                                typename From::char_type>::value &&
                            extension_traits::is_narrow_character<
                                typename To::char_type>::value>::type>
 ...
 public:
  json_visitor_adaptor(To& visitor) : supertype(visitor) {}
 ...
 private:
  bool visit_key(const string_view_type& key, const ser_context& context,
                 std::error_code& ec) override {
    return destination().key(
        string_view_type(reinterpret_cast<const to_char_type*>(key.data()),
                         key.size()),
        context, ec);
  }
  ...
  bool visit_string(const string_view_type& value, semantic_tag tag,
    ...
    return destination().string_value(
        string_view_type(reinterpret_cast<const to_char_type*>(value.data()),
                         value.size()),
        tag, context, ec);
  }
};

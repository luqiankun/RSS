...
JSONCONS_DEPRECATED_MSG("Instead, use float_chars_format")
typedef float_chars_format chars_format;
#endif
...
#if !defined(JSONCONS_NO_DEPRECATED)
JSONCONS_DEPRECATED_MSG("Instead, use bigint_chars_format")
...
struct strict_json_parsing {
  bool operator()(json_errc, const ser_context&) noexcept { return false; }
};

struct allow_trailing_commas {
  bool operator()(const std::error_code& ec, const ser_context&) noexcept {
    return ec == json_errc::illegal_comment ||
           ec == jsoncons::json_errc::extra_comma;
  }
};

#if !defined(JSONCONS_NO_DEPRECATED)
JSONCONS_DEPRECATED_MSG("Instead, use default_json_parsing")
typedef default_json_parsing default_parse_error_handler;
JSONCONS_DEPRECATED_MSG("Instead, use strict_json_parsing")
typedef strict_json_parsing strict_parse_error_handler;
#endif
...
class basic_json_options_common {
 ...
 protected:
  ...
  basic_json_options_common& operator=(const basic_json_options_common&) =
      default;
 ...
 public:
  bool enable_nan_to_num() const { return enable_nan_to_num_; }

  bool enable_inf_to_num() const { return enable_inf_to_num_; }

  bool enable_neginf_to_num() const {
    return enable_neginf_to_num_ || enable_inf_to_num_;
  }

  bool enable_nan_to_str() const { return enable_nan_to_str_; }

  bool enable_str_to_nan() const { return enable_str_to_nan_; }

  bool enable_inf_to_str() const { return enable_inf_to_str_; }

  bool enable_str_to_inf() const { return enable_str_to_inf_; }
...
#if !defined(JSONCONS_NO_DEPRECATED)
    else if (can_read_nan_replacement_ &&
             nan_replacement_.size() >= 2)  // string
    {
      return nan_replacement_.substr(
          1, nan_replacement_.size() - 2);  // Remove quotes
    }
#endif
    else {
...
#if !defined(JSONCONS_NO_DEPRECATED)
    else if (can_read_pos_inf_replacement_ &&
             pos_inf_replacement_.size() >= 2)  // string
    {
      return pos_inf_replacement_.substr(
          1, pos_inf_replacement_.size() - 2);  // Strip quotes
    }
#endif
    else {
...
#if !defined(JSONCONS_NO_DEPRECATED)
    else if (can_read_neg_inf_replacement_ &&
             neg_inf_replacement_.size() >= 2)  // string
    {
      return neg_inf_replacement_.substr(
          1, neg_inf_replacement_.size() - 2);  // Strip quotes
    }
#endif
    else {
  ...
  JSONCONS_DEPRECATED_MSG(
      "Instead, use enable_nan_to_num() or enable_nan_to_str()")
  bool can_read_nan_replacement() const { return can_read_nan_replacement_; }

  JSONCONS_DEPRECATED_MSG(
      "Instead, use enable_inf_to_num() or enable_inf_to_str()")
  bool can_read_pos_inf_replacement() const {
    return can_read_pos_inf_replacement_;
  }

  JSONCONS_DEPRECATED_MSG(
      "Instead, use enable_neginf_to_num() or enable_neginf_to_str()")
  bool can_read_neg_inf_replacement() const {
    return can_read_neg_inf_replacement_;
  }

  bool can_write_nan_replacement() const { return !nan_replacement_.empty(); }

  bool can_write_pos_inf_replacement() const {
    return !pos_inf_replacement_.empty();
  }

  bool can_write_neg_inf_replacement() const {
    return !neg_inf_replacement_.empty();
  }

  JSONCONS_DEPRECATED_MSG("Instead, use nan_to_num() or nan_to_str()")
  const string_type& nan_replacement() const { return nan_replacement_; }
...
class basic_json_decode_options
    : public virtual basic_json_options_common<CharT> {
  friend class basic_json_options<CharT>;
 ...
 public:
  ...
  basic_json_decode_options& operator=(const basic_json_decode_options&) =
      default;

  bool lossless_number() const { return lossless_number_; }

  const std::function<bool(json_errc, const ser_context&)>& err_handler()
      const {
    return err_handler_;
  }
  ...
  bool dec_to_str() const { return lossless_number_; }
#endif
};
...
class basic_json_encode_options
 ...
 public:
  ...
  basic_json_encode_options& operator=(const basic_json_encode_options&) =
      default;

  byte_string_chars_format byte_string_format() const {
    return byte_string_format_;
  }

  bigint_chars_format bigint_format() const { return bigint_format_; }

  line_split_kind object_object_line_splits() const {
    return object_object_line_splits_;
  }

  line_split_kind array_object_line_splits() const {
    return array_object_line_splits_;
  }

  line_split_kind object_array_line_splits() const {
    return object_array_line_splits_;
  }

  line_split_kind array_array_line_splits() const {
    return array_array_line_splits_;
  }

  uint8_t indent_size() const { return indent_size_; }

  spaces_option spaces_around_colon() const { return spaces_around_colon_; }

  spaces_option spaces_around_comma() const { return spaces_around_comma_; }

  bool pad_inside_object_braces() const { return pad_inside_object_braces_; }

  bool pad_inside_array_brackets() const { return pad_inside_array_brackets_; }

  string_type new_line_chars() const { return new_line_chars_; }

  std::size_t line_length_limit() const { return line_length_limit_; }

  float_chars_format float_format() const { return float_format_; }

  int8_t precision() const { return precision_; }

  bool escape_all_non_ascii() const { return escape_all_non_ascii_; }

  bool escape_solidus() const { return escape_solidus_; }
...
class basic_json_options final : public basic_json_decode_options<CharT>,
 ...
 public:
  ...
  basic_json_options& nan_to_str(const string_type& value,
                                 bool enable_inverse = true) {
  ...
  basic_json_options& inf_to_str(const string_type& value,
                                 bool enable_inverse = true) {
  ...
  basic_json_options& neginf_to_str(const string_type& value,
                                    bool enable_inverse = true) {
  ...
  JSONCONS_DEPRECATED_MSG(
      "Instead, use inf_to_num(const string_type&) or inf_to_str(const "
      "string_type&)")
  ...
  JSONCONS_DEPRECATED_MSG(
      "Instead, use neginf_to_num(const string_type&) or neginf_to_str(const "
      "string_type&)")
  ...
  JSONCONS_DEPRECATED_MSG(
      "Instead, use nan_to_num(const string_type&) or nan_to_str(const "
      "string_type&)")
  ...
  basic_json_options& err_handler(
      const std::function<bool(json_errc, const ser_context&)>& value) {
  ...
  basic_json_options& big_integer_format(bigint_chars_format value) {
    this->bigint_format_ = value;
    return *this;
  }

  JSONCONS_DEPRECATED_MSG("Instead, use bigint_format(bigint_chars_format)")
  basic_json_options& bignum_format(bigint_chars_format value) {
    this->bigint_format_ = value;
    return *this;
  }
  ...
  basic_json_options& indent(uint8_t value) { return indent_size(value); }

  JSONCONS_DEPRECATED_MSG(
      "Instead, use object_object_line_splits(line_split_kind)")
  basic_json_options& object_object_split_lines(line_split_kind value) {
    this->object_object_line_splits_ = value;
    return *this;
  }

  JSONCONS_DEPRECATED_MSG(
      "Instead, use array_object_line_splits(line_split_kind)")
  basic_json_options& array_object_split_lines(line_split_kind value) {
    this->array_object_line_splits_ = value;
    return *this;
  }

  JSONCONS_DEPRECATED_MSG(
      "Instead, use object_array_line_splits(line_split_kind)")
  basic_json_options& object_array_split_lines(line_split_kind value) {
    this->object_array_line_splits_ = value;
    return *this;
  }

  JSONCONS_DEPRECATED_MSG(
      "Instead, use array_array_line_splits(line_split_kind)")
  basic_json_options& array_array_split_lines(line_split_kind value) {
    this->array_array_line_splits_ = value;
    return *this;
  }
#endif
...
JSONCONS_DEPRECATED_MSG("json_options")
typedef json_options serialization_options;
JSONCONS_DEPRECATED_MSG("wjson_options")
typedef wjson_options wserialization_options;
JSONCONS_DEPRECATED_MSG("json_options")
typedef json_options json_serializing_options;
JSONCONS_DEPRECATED_MSG("wjson_options")
typedef wjson_options wjson_serializing_options;
#endif

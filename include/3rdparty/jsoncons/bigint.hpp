...
class basic_bigint_base {
 public:
  ...
  using basic_type_allocator_type = typename std::allocator_traits<
      allocator_type>::template rebind_alloc<uint64_t>;
 ...
 public:
  using allocator_traits_type =
      std::allocator_traits<basic_type_allocator_type>;
...
class basic_bigint : protected detail::basic_bigint_base<Allocator> {
 ...
 public:
  ...
  static constexpr uint64_t max_basic_type =
      (std::numeric_limits<uint64_t>::max)();
  static constexpr uint64_t basic_type_bits =
      sizeof(uint64_t) * 8;  // Number of bits
  static constexpr uint64_t basic_type_halfBits = basic_type_bits / 2;

  static constexpr uint16_t word_length =
      4;  // Use multiples of word_length words
 ...
 private:
  ...
  struct short_storage {
    ...
    short_storage()
        : is_dynamic_(false), is_negative_(false), length_(0), values_{0, 0} {}
    ...
    short_storage(T n,
        ...
        : is_dynamic_(false), is_negative_(n < 0), length_(n == 0 ? 0 : 1) {
      values_[0] = n < 0 ? (uint64_t(0) - static_cast<uint64_t>(n))
    ...
    short_storage(T n,
                  typename std::enable_if<std::is_integral<T>::value &&
                                          sizeof(T) <= sizeof(int64_t) &&
                                          !std::is_signed<T>::value>::type* = 0)
        : is_dynamic_(false), is_negative_(false), length_(n == 0 ? 0 : 1) {
      values_[0] = n;
      values_[1] = 0;
    }
    ...
    short_storage(T n,
      ...
      auto u = n < 0 ? (unsigned_type(0) - static_cast<unsigned_type>(n))
  ...
  struct dynamic_storage {
    ...
    using pointer =
        typename std::allocator_traits<real_allocator_type>::pointer;
    ...
    dynamic_storage(const dynamic_storage& stor,
      ...
      std::memcpy(data_, stor.data_,
                  size_type(stor.length_ * sizeof(uint64_t)));
    }
    ...
    void create(size_type length, real_allocator_type alloc) {
      ...
      data_ = std::allocator_traits<real_allocator_type>::allocate(alloc,
                                                                   capacity_);
      JSONCONS_TRY {
        std::allocator_traits<real_allocator_type>::construct(
            alloc, extension_traits::to_plain_pointer(data_));
      }
      JSONCONS_CATCH(...) {
        std::allocator_traits<real_allocator_type>::deallocate(alloc, data_,
                                                               capacity_);
        JSONCONS_RETHROW;
      }
    }
    ...
    void destroy(const real_allocator_type& a) noexcept {
      if (data_ != nullptr) {
        ...
        std::allocator_traits<real_allocator_type>::destroy(
            alloc, extension_traits::to_plain_pointer(data_));
        std::allocator_traits<real_allocator_type>::deallocate(alloc, data_,
                                                               capacity_);
      }
    ...
    void reserve(size_type n, const real_allocator_type& a) {
      ...
      data_ = std::allocator_traits<real_allocator_type>::allocate(
          alloc, capacity_new);
      if (length_ > 0) {
        std::memcpy(data_, data_old, size_type(length_ * sizeof(uint64_t)));
      }
      if (capacity_ > 0 && data_ != nullptr) {
        std::allocator_traits<real_allocator_type>::deallocate(alloc, data_old,
                                                               capacity_);
      }
 ...
 public:
  basic_bigint() { ::new (&short_stor_) short_storage(); }

  explicit basic_bigint(const Allocator& alloc) : base_t(alloc) {
    ::new (&short_stor_) short_storage();
  }

  basic_bigint(const basic_bigint<Allocator>& n) : base_t(n.get_allocator()) {
    if (!n.is_dynamic()) {
  ...
  ~basic_bigint() noexcept { destroy(); }

  constexpr bool is_dynamic() const { return common_stor_.is_dynamic_; }

  constexpr size_type length() const { return common_stor_.length_; }

  constexpr size_type capacity() const {
    return is_dynamic() ? dynamic_stor_.capacity_ : max_short_storage_size;
  }

  bool is_negative() const { return common_stor_.is_negative_; }

  void is_negative(bool value) { common_stor_.is_negative_ = value; }
  ...
  static basic_bigint<Allocator> from_string(
  ...
  template <typename CharT>
  static basic_bigint<Allocator> from_string(const CharT* data,
                                             size_type length) {
    bool neg;
    if (*data == '-') {
      neg = true;
    ...
    for (size_type i = 0; i < length; i++) {
      ...
      switch (c) {
        ...
        case '9':
          ...
          break;
        default:
          JSONCONS_THROW(std::runtime_error(std::string("Invalid digit ") +
                                            "\'" + (char)c + "\'"));
      }
    }
  ...
  template <typename CharT>
  static basic_bigint<Allocator> from_string_radix(const CharT* data,
                                                   size_type length,
                                                   uint8_t radix) {
    if (!(radix >= 2 && radix <= 16)) {
    ...
    for (size_type i = 0; i < length; i++) {
      ...
      switch (c) {
        ...
        case 'F':
          ...
          break;
        default:
          JSONCONS_THROW(std::runtime_error(
              std::string("Invalid digit in radix ") + std::to_string(radix) +
              ": \'" + (char)c + "\'"));
      }
      if (d >= radix) {
        JSONCONS_THROW(std::runtime_error(
  ...
  static basic_bigint from_bytes_be(int signum, const uint8_t* str,
                                    std::size_t n) {
  ...
  void resize(size_type new_length) {
    ...
    if (old_length < new_length) {
      if (is_dynamic()) {
        std::memset(dynamic_stor_.data_ + old_length, 0,
                    size_type((new_length - old_length) * sizeof(uint64_t)));
      } else {
  ...
  void reserve(size_type n) {
    if (capacity() < n) {
      if (!is_dynamic()) {
        ...
        uint64_t values[max_short_storage_size] = {short_stor_.values_[0],
                                                   short_stor_.values_[1]};
  ...
  bool operator!() const { return length() == 0 ? true : false; }
  ...
  basic_bigint& operator+=(const basic_bigint<Allocator>& y) {
    if (is_negative() != y.is_negative()) return *this -= -y;
    uint64_t d;
    uint64_t carry = 0;
  ...
  basic_bigint& operator-=(const basic_bigint<Allocator>& y) {
    ...
    uint64_t d;
    for (size_type i = 0; i < length(); i++) {
      if (i >= y.length() && borrow == 0) break;
      d = data()[i] - borrow;
      borrow = d > data()[i];
      if (i < y.length()) {
        data()[i] = d - y.data()[i];
        if (data()[i] > d) borrow = 1;
      } else
        data()[i] = d;
    }
    reduce();
  ...
  basic_bigint& operator*=(int64_t y) {
    *this *= uint64_t(y < 0 ? -y : y);
    if (y < 0) common_stor_.is_negative_ = !is_negative();
    return *this;
  }
  ...
  basic_bigint& operator*=(basic_bigint<Allocator> y) {
    if (length() == 0 || y.length() == 0) return *this = 0;
    bool difSigns = is_negative() != y.is_negative();
    if (length() + y.length() ==
        max_short_storage_size)  // length() = y.length() = 1
    {
      ...
      *this *= digit;
      ...
      if (y.length() == 1)
        *this *= y.data()[0];
        ...
        uint64_t sumHi = 0, sumLo, hi, lo, sumLo_old, sumHi_old, carry = 0;
  ...
  basic_bigint& operator<<=(uint64_t k) {
    ...
    if (k)  // 0 < k < basic_type_bits:
    {
      ...
      for (size_type i = length(); i-- > 0;) {
        data()[i] <<= k;
        if (i > 0) data()[i] |= (data()[i - 1] >> k1) & mask;
      }
    }
    reduce();
    return *this;
  ...
  basic_bigint& operator>>=(uint64_t k) {
    ...
    for (size_type i = 0; i <= n; i++) {
      data()[i] >>= k;
      if (i < n) data()[i] |= ((data()[i + 1] & mask) << k1);
    }
    reduce();
    return *this;
  }
  ...
  basic_bigint& operator&=(const basic_bigint<Allocator>& a) {
    ...
    if (old_length > new_length) {
      if (is_dynamic()) {
        std::memset(dynamic_stor_.data_ + new_length, 0,
                    size_type(old_length - new_length * sizeof(uint64_t)));
      } else {
        JSONCONS_ASSERT(new_length <= max_short_storage_size);
  ...
  explicit operator bool() const { return length() != 0 ? true : false; }
  ...
  void write_string_hex(std::basic_string<Ch, Traits, Alloc>& data) const {
    ...
    std::size_t len =
        (v.length() * basic_bigint<Allocator>::basic_type_bits / 3) + 2;
    data.reserve(len);
    // 1/3 > ln(2)/ln(10)
    static uint64_t p10 = 1;
    static uint64_t ip10 = 0;
  ...
  friend bool operator==(const basic_bigint<Allocator>& x,
                         const basic_bigint<Allocator>& y) noexcept {
  ...
  friend basic_bigint<Allocator> operator+(basic_bigint<Allocator> x,
                                           const basic_bigint<Allocator>& y) {
    return x += y;
  }
  ...
  friend basic_bigint<Allocator> operator*(int64_t x,
                                           const basic_bigint<Allocator>& y) {
  ...
  friend basic_bigint<Allocator> operator<<(basic_bigint<Allocator> u,
                                            unsigned k) {
    return u <<= k;
  }
  ...
  friend std::basic_ostream<CharT>& operator<<(
      std::basic_ostream<CharT>& os, const basic_bigint<Allocator>& v) {
  ...
  void divide(basic_bigint<Allocator> denom, basic_bigint<Allocator>& quot,
              basic_bigint<Allocator>& rem, bool remDesired) const {
    if (denom.length() == 0) {
    ...
    if (denom.length() == 1 && num.length() == 1) {
      ...
      for (size_type i = length(); i-- > 0;) {
        dividend =
            (dHi << basic_type_halfBits) | (data()[i] >> basic_type_halfBits);
        q1 = dividend / divisor;
        r = dividend % divisor;
    ...
    quot.resize(n - l);
    for (size_type i = quot.length(); i-- > 0;) quot.data()[i] = 0;
    rem = num;
 ...
 private:
  ...
  void DDproduct(uint64_t A, uint64_t B, uint64_t& hi, uint64_t& lo) const
  // Multiplying two digits: (hi, lo) = A * B
  {
    uint64_t hiA = A >> basic_type_halfBits, loA = A & r_mask,
             hiB = B >> basic_type_halfBits, loB = B & r_mask, mid1, mid2, old;
  ...
  int normalize(basic_bigint<Allocator>& denom, basic_bigint<Allocator>& num,
                int& x) const {

...
class json_array : public allocator_holder<typename Json::allocator_type> {
 ...
 private:
  using value_allocator_type = typename std::allocator_traits<
      allocator_type>::template rebind_alloc<value_type>;
  using value_container_type =
      SequenceContainer<value_type, value_allocator_type>;
 ...
 public:
  ...
  explicit json_array(std::size_t n, const Json& value,
                      const allocator_type& alloc = allocator_type())
      : allocator_holder<allocator_type>(alloc),
  ...
  ~json_array() noexcept { flatten_and_destroy(); }
  ...
  void swap(json_array& other) noexcept { elements_.swap(other.elements_); }
  ...
  void remove_range(std::size_t from_index, std::size_t to_index) {
    ...
    elements_.erase(elements_.cbegin() + from_index,
                    elements_.cbegin() + to_index);
  }
#endif

...
class basic_item_event_receiver : public basic_item_event_visitor<CharT> {
 ...
 public:
  ...
  basic_item_event_receiver(
      std::function<bool(const basic_staj_event<CharT>&, const ser_context&)>
          pred)
      : pred_(pred),
        event_(staj_event_type::null_value),
        state_(),
  ...
  const basic_staj_event<CharT>& event() const { return event_; }
 ...
 private:
  static constexpr bool accept(const basic_staj_event<CharT>&,
                               const ser_context&) {
    return true;
  }
  ...
  bool visit_begin_object(std::size_t length, semantic_tag tag,
                          ...
                          std::error_code&) override {
    event_ =
        basic_staj_event<CharT>(staj_event_type::begin_object, length, tag);
    return !pred_(event_, context);
  }
  ...
  bool visit_byte_string(const byte_string_view& s, semantic_tag tag,
                         ...
                         std::error_code&) override {
    event_ =
        basic_staj_event<CharT>(s, staj_event_type::byte_string_value, tag);
    return !pred_(event_, context);
  }
  ...
  bool visit_byte_string(const byte_string_view& s, uint64_t ext_tag,
                         ...
                         std::error_code&) override {
    event_ =
        basic_staj_event<CharT>(s, staj_event_type::byte_string_value, ext_tag);
    return !pred_(event_, context);
  }

...
struct json_type_traits {
  ...
  static constexpr bool is(const Json&) noexcept { return false; }
...
using traits_can_convert_t =
    decltype(json_type_traits<Json, T>::can_convert(Json()));

template <typename Json, typename T>
using has_can_convert =
    extension_traits::is_detected<traits_can_convert_t, Json, T>;

template <typename T>
struct invoke_can_convert {
  template <typename Json>
  static typename std::enable_if<has_can_convert<Json, T>::value, bool>::type
...
// is_json_type_traits_unspecialized
template <typename Json, typename T>
struct is_json_type_traits_unspecialized<
    Json, T,
    typename std::enable_if<!std::integral_constant<
        bool, json_type_traits<Json, T>::is_compatible>::value>::type>
    : std::true_type {};
...
struct is_compatible_array_type<
    ...
    typename std::enable_if<
        !std::is_same<T, typename Json::array>::value &&
        extension_traits::is_array_like<T>::value &&
        !is_json_type_traits_unspecialized<
            Json, typename std::iterator_traits<
                      typename T::iterator>::value_type>::value>::type>
    : std::true_type {};
...
template <typename Json, typename T>
struct is_json_type_traits_specialized<
    Json, T,
    typename std::enable_if<
        !jsoncons::detail::is_json_type_traits_unspecialized<Json, T>::value>::
        type> : std::true_type {};

template <typename Json>
struct json_type_traits<
    Json, const typename std::decay<typename Json::char_type>::type*> {
  using char_type = typename Json::char_type;
  using allocator_type = typename Json::allocator_type;

  static bool is(const Json& j) noexcept { return j.is_string(); }
  static const char_type* as(const Json& j) { return j.as_cstring(); }
...
struct json_type_traits<Json,
                        typename std::decay<typename Json::char_type>::type*> {
  using char_type = typename Json::char_type;
  using allocator_type = typename Json::allocator_type;
...
struct json_type_traits<
    Json, T,
    typename std::enable_if<(extension_traits::is_signed_integer<T>::value &&
                             sizeof(T) <= sizeof(int64_t)) ||
                            (extension_traits::is_unsigned_integer<T>::value &&
                             sizeof(T) <= sizeof(uint64_t))>::type> {
  using allocator_type = typename Json::allocator_type;

  static bool is(const Json& j) noexcept { return j.template is_integer<T>(); }
  static T as(const Json& j) { return j.template as_integer<T>(); }

  static Json to_json(T val) { return Json(val, semantic_tag::none); }
...
template <typename Json, typename T>
struct json_type_traits<
    Json, T, typename std::enable_if<std::is_floating_point<T>::value>::type> {
  using allocator_type = typename Json::allocator_type;

  static bool is(const Json& j) noexcept { return j.is_double(); }
  static T as(const Json& j) { return static_cast<T>(j.as_double()); }
  static Json to_json(T val) { return Json(val, semantic_tag::none); }
  static Json to_json(T val, const allocator_type&) {
    return Json(val, semantic_tag::none);
  }
...
struct json_type_traits<Json, typename Json::object> {
  ...
  static bool is(const Json& j) noexcept { return j.is_object(); }
...
struct json_type_traits<Json, Json> {
  ...
  static Json as(Json j) { return j; }
  static Json to_json(const Json& val) { return val; }
  static Json to_json(const Json& val, const allocator_type&) { return val; }
};
...
struct json_type_traits<Json, jsoncons::null_type> {
  ...
  static bool is(const Json& j) noexcept { return j.is_null(); }
...
struct json_type_traits<Json, bool> {
  ...
  static bool is(const Json& j) noexcept { return j.is_bool(); }
  static bool as(const Json& j) { return j.as_bool(); }
  static Json to_json(bool val) { return Json(val, semantic_tag::none); }
...
struct json_type_traits<
    ...
    typename std::enable_if<std::is_same<
        T, std::conditional<
               !std::is_same<bool, std::vector<bool>::const_reference>::value,
               std::vector<bool>::const_reference, void>::type>::value>::type> {
  using allocator_type = typename Json::allocator_type;
...
struct json_type_traits<
    ...
    typename std::enable_if<
        ...
        std::is_same<typename Json::char_type,
                     typename T::value_type>::value>::type> {
  using allocator_type = typename Json::allocator_type;

  static bool is(const Json& j) noexcept { return j.is_string(); }

  static T as(const Json& j) { return T(j.as_string()); }

  static Json to_json(const T& val) { return Json(val, semantic_tag::none); }
...
struct json_type_traits<
  ...
  static bool is(const Json& j) noexcept { return j.is_string_view(); }
...
struct json_type_traits<
    ...
    typename std::enable_if<
        ...
        extension_traits::is_back_insertable<T>::value>::type> {
  ...
  static typename std::enable_if<
      !extension_traits::is_byte<typename Container::value_type>::value,
      Container>::type
  as(const Json& j) {
    if (j.is_array()) {
      T result;
      visit_reserve_(typename std::integral_constant<
                         bool, extension_traits::has_reserve<T>::value>::type(),
                     result, j.size());
  ...
  template <typename Container = T>
  static typename std::enable_if<
      !extension_traits::is_std_byte<typename Container::value_type>::value,
      Json>::type
  to_json(const T& val) {
...
struct json_type_traits<
    ...
    typename std::enable_if<
        ...
        extension_traits::is_constructible_from_const_pointer_and_size<
            typename T::key_type>::value &&
        is_json_type_traits_specialized<Json, typename T::mapped_type>::value>::
        type> {
  ...
  static T as(const Json& j) {
    ...
    for (const auto& item : j.object_range()) {
      result.emplace(key_type(item.key().data(), item.key().size()),
                     item.value().template as<mapped_type>());
    }
...
struct json_type_traits<
  ...
  static Json to_json(const T& val) {
    ...
    for (const auto& item : val) {
      ...
      if (temp.is_string_view()) {
        j.try_emplace(typename Json::key_type(temp.as_string_view()),
                      item.second);
      } else {
  ...
  static Json to_json(const T& val, const allocator_type& alloc) {
    ...
    for (const auto& item : val) {
      ...
      if (temp.is_string_view()) {
        j.try_emplace(typename Json::key_type(temp.as_string_view(), alloc),
                      item.second);
      } else {
...
struct json_tuple_helper {
  ...
  static void to_json(const Tuple& tuple, Json& j) {
    j.push_back(json_type_traits<Json, element_type>::to_json(
        std::get<Size - Pos>(tuple)));
...
struct json_type_traits<Json, std::tuple<E...>> {
 private:
  using helper = tuple_detail::json_tuple_helper<sizeof...(E), sizeof...(E),
                                                 Json, std::tuple<E...>>;

 public:
  using allocator_type = typename Json::allocator_type;

  static bool is(const Json& j) noexcept { return helper::is(j); }
...
struct json_type_traits<Json, std::pair<T1, T2>> {
 public:
  ...
  static std::pair<T1, T2> as(const Json& j) {
    return std::make_pair<T1, T2>(j[0].template as<T1>(),
                                  j[1].template as<T2>());
  }
...
struct json_type_traits<
    Json, T,
    typename std::enable_if<
        extension_traits::is_basic_byte_string<T>::value>::type> {
 public:
  using allocator_type = typename Json::allocator_type;
...
struct json_type_traits<
    Json, std::shared_ptr<ValueType>,
    typename std::enable_if<
        !is_json_type_traits_declared<std::shared_ptr<ValueType>>::value &&
        !std::is_polymorphic<ValueType>::value>::type> {
  static bool is(const Json& j) noexcept {
  ...
  static std::shared_ptr<ValueType> as(const Json& j) {
    return j.is_null()
               ? std::shared_ptr<ValueType>(nullptr)
               : std::make_shared<ValueType>(j.template as<ValueType>());
  }
...
template <typename Json, typename ValueType>
struct json_type_traits<
    Json, std::unique_ptr<ValueType>,
    typename std::enable_if<
        !is_json_type_traits_declared<std::unique_ptr<ValueType>>::value &&
        !std::is_polymorphic<ValueType>::value>::type> {
  static bool is(const Json& j) noexcept {
    return j.is_null() || j.template is<ValueType>();
  }
  ...
  static std::unique_ptr<ValueType> as(const Json& j) {
    return j.is_null()
               ? std::unique_ptr<ValueType>(nullptr)
               : jsoncons::make_unique<ValueType>(j.template as<ValueType>());
  }
...
struct json_type_traits<Json, jsoncons::optional<T>,
                        typename std::enable_if<!is_json_type_traits_declared<
                            jsoncons::optional<T>>::value>::type> {
 public:
  static bool is(const Json& j) noexcept {
  ...
  static jsoncons::optional<T> as(const Json& j) {
    return j.is_null() ? jsoncons::optional<T>()
                       : jsoncons::optional<T>(j.template as<T>());
  }
...
struct json_type_traits<Json, byte_string_view> {
 ...
 public:
  static bool is(const Json& j) noexcept { return j.is_byte_string_view(); }

  static byte_string_view as(const Json& j) { return j.as_byte_string_view(); }
...
struct json_type_traits<Json, basic_bigint<Allocator>> {
 public:
  ...
  static bool is(const Json& j) noexcept {
    switch (j.type()) {
      case json_type::string_value:
        return jsoncons::detail::is_base10(j.as_string_view().data(),
                                           j.as_string_view().length());
  ...
  static basic_bigint<Allocator> as(const Json& j) {
    switch (j.type()) {
      case json_type::string_value:
        if (!jsoncons::detail::is_base10(j.as_string_view().data(),
                                         j.as_string_view().length())) {
          JSONCONS_THROW(conv_error(conv_errc::not_bigint));
        }
        return basic_bigint<Allocator>::from_string(
            j.as_string_view().data(), j.as_string_view().length());
...
template <std::size_t N, typename Json, typename Variant, typename T,
          typename... U>
    typename std::enable_if <
    N<std::variant_size_v<Variant>, bool>::type is_variant(const Json& j) {
  if (j.template is<T>()) {
    return true;
  } else {
...
template <std::size_t N, typename Json, typename Variant, typename T,
          typename... U>
    typename std::enable_if <
    N<std::variant_size_v<Variant>, Variant>::type as_variant(const Json& j) {
  if (j.template is<T>()) {
...
struct json_type_traits<Json, std::variant<VariantTypes...>> {
 public:
  ...
  static bool is(const Json& j) noexcept {
    return variant_detail::is_variant<0, Json, variant_type, VariantTypes...>(
        j);
  }

  static std::variant<VariantTypes...> as(const Json& j) {
    return variant_detail::as_variant<0, Json, variant_type, VariantTypes...>(
        j);
  }
...
struct json_type_traits<Json, std::chrono::duration<Rep, Period>> {
  ...
  static bool is(const Json& j) noexcept {
    return (j.tag() == semantic_tag::epoch_second ||
            j.tag() == semantic_tag::epoch_milli ||
            j.tag() == semantic_tag::epoch_nano);
  }

  static duration_type as(const Json& j) { return from_json_(j); }

  static Json to_json(const duration_type& val,
                      const allocator_type& = allocator_type()) {
    return to_json_(val);
  }

  template <typename PeriodT = Period>
  static typename std::enable_if<std::is_same<PeriodT, std::ratio<1>>::value,
                                 duration_type>::type
  ...
  template <typename PeriodT = Period>
  static typename std::enable_if<std::is_same<PeriodT, std::milli>::value,
                                 duration_type>::type
  from_json_(const Json& j) {
    if (j.is_int64() || j.is_uint64()) {
      ...
      switch (j.tag()) {
        ...
        case semantic_tag::epoch_nano:
          return duration_type(
              count == 0 ? 0 : static_cast<Rep>(count / nanos_in_milli));
        default:
          return duration_type(static_cast<Rep>(count));
      }
    } else if (j.is_string()) {
      switch (j.tag()) {
        ...
        case semantic_tag::epoch_milli: {
          ...
          Rep n{0};
          auto result =
              jsoncons::detail::decimal_to_integer(sv.data(), sv.size(), n);
          if (!result) {
            return duration_type();
          }
  ...
  template <typename PeriodT = Period>
  static typename std::enable_if<std::is_same<PeriodT, std::nano>::value,
                                 duration_type>::type
  from_json_(const Json& j) {
    if (j.is_int64() || j.is_uint64() || j.is_double()) {
      auto count = j.template as<Rep>();
...
struct json_type_traits<Json, std::nullptr_t> {
  ...
  static Json to_json(const std::nullptr_t&,
                      const allocator_type& = allocator_type()) {
    return Json::null();
  }
};

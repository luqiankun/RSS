...
class stream_sink {
 ...
 private:
  std::size_t buffer_length() const { return p_ - begin_buffer_; }
};
...
class string_sink {
 ...
 public:
  string_sink(string_sink&& other) noexcept : buf_ptr(nullptr) {
    std::swap(buf_ptr, other.buf_ptr);
  }

  string_sink(container_type& buf) : buf_ptr(std::addressof(buf)) {}
  ...
  void push_back(value_type ch) { buf_ptr->push_back(ch); }
};
...
class bytes_sink<
    Container,
    typename std::enable_if<extension_traits::is_back_insertable_byte_container<
 ...
 public:
  ...
  bytes_sink(container_type& buf) : buf_ptr(std::addressof(buf)) {}

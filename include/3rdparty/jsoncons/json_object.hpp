...
class key_value {
 ...
 public:
  ...
  key_value(key_value&& member, const allocator_type& alloc) noexcept
      : key_(std::move(member.key_), alloc),
        value_(std::move(member.value_), alloc) {}
  ...
  const key_type& name() const { return key_; }
#endif
...
struct get_key_value {
  ...
  key_value_type operator()(std::pair<T1, T2>&& p) {
    return key_value_type(std::forward<T1>(p.first),
...
template <class KeyT, class Json,
          template <typename, typename> class SequenceContainer = std::vector>
class sorted_json_object
 ...
 private:
  struct Comp {
    bool operator()(const key_value_type& kv, string_view_type k) const {
      return kv.key() < k;
    }
    bool operator()(string_view_type k, const key_value_type& kv) const {
      return k < kv.key();
    }
  };
  ...
  using key_value_container_type =
      SequenceContainer<key_value_type, key_value_allocator_type>;
 ...
 public:
  ...
  sorted_json_object(sorted_json_object&& other, const allocator_type& alloc)
      : allocator_holder<allocator_type>(alloc),
        members_(std::move(other.members_), key_value_allocator_type(alloc)) {}
  ...
  sorted_json_object(InputIt first, InputIt last) {
    ...
    std::stable_sort(
        ...
        [](const key_value_type& a, const key_value_type& b) -> bool {
  ...
  sorted_json_object(const std::initializer_list<
                         std::pair<std::basic_string<char_type>, Json>>& init,
  ...
  ~sorted_json_object() noexcept { flatten_and_destroy(); }
  ...
  iterator find(const string_view_type& name) noexcept {
    auto p = std::equal_range(members_.begin(), members_.end(), name, Comp());
  ...
  static bool compare(const index_key_value<Json>& item1,
                      const index_key_value<Json>& item2) {
  ...
  void uninitialized_init(index_key_value<Json>* items, std::size_t count) {
    ...
    if (count > 0) {
      ...
      members_.emplace_back(key_type(first->name, get_allocator()),
                            std::move(first->value));
      auto prev_it = first;
      for (auto it = first + 1; it != last; ++it) {
        if (it->name != prev_it->name) {
          members_.emplace_back(key_type(it->name, get_allocator()),
                                std::move(it->value));
        }
        ++prev_it;
      }
  ...
  void insert(sorted_unique_range_tag, InputIt first, InputIt last) {
    if (first != last) {
      ...
      if (it != members_.end()) {
        for (auto s = first; s != last; ++s) {
          it = members_.emplace(it, key_type(s->first, get_allocator()),
                                s->second);
        }
      } else {
        for (auto s = first; s != last; ++s) {
  ...
  typename std::enable_if<extension_traits::is_stateless<A>::value,
  ...
  try_emplace(iterator hint, const string_view_type& name, Args&&... args) {
    ...
    if (hint != members_.end() && hint->key() <= name) {
      it = std::lower_bound(hint, members_.end(), name, Comp());
    } else {
      it = std::lower_bound(members_.begin(), members_.end(), name, Comp());
    }
  ...
  insert_or_assign(iterator hint, const string_view_type& name, T&& value) {
    ...
    if (hint != members_.end() && hint->key() <= name) {
      it = std::lower_bound(
          hint, members_.end(), name,
          [](const key_value_type& a, const string_view_type& k) -> bool {
            return string_view_type(a.key()).compare(k) < 0;
          });
    } else {
      it = std::lower_bound(
  ...
  void merge(iterator hint, sorted_json_object&& source) {
    ...
    for (; it != end; ++it) {
      ...
      if (hint != members_.end() && hint->key() <= (*it).key()) {
        pos = std::lower_bound(hint, members_.end(), (*it).key(), Comp());
      } else {
...
template <class KeyT, class Json,
          template <typename, typename> class SequenceContainer = std::vector>
class order_preserving_json_object
    : public allocator_holder<typename Json::allocator_type> {
 public:
  using allocator_type = typename Json::allocator_type;
  using char_type = typename Json::char_type;
 ...
 private:
  ...
  struct Comp {
    ...
    Comp(const key_value_container_type& members_) : members_(members_) {}

    bool operator()(std::size_t i, string_view_type k) const {
      return members_.at(i).key() < k;
    }
    bool operator()(string_view_type k, std::size_t i) const {
      return k < members_.at(i).key();
    }
  };
 ...
 public:
  ...
  order_preserving_json_object(
      std::initializer_list<std::pair<std::basic_string<char_type>, Json>> init,
      const allocator_type& alloc = allocator_type())
      : allocator_holder<allocator_type>(alloc),
        members_(key_value_allocator_type(alloc)) {
    members_.reserve(init.size());
  ...
  ~order_preserving_json_object() noexcept { flatten_and_destroy(); }
  ...
  order_preserving_json_object& operator=(
      const order_preserving_json_object& val) {
  ...
  iterator erase(const_iterator first, const_iterator last) {
    std::size_t pos1 =
        first == members_.end() ? members_.size() : first - members_.begin();
    std::size_t pos2 =
        last == members_.end() ? members_.size() : last - members_.begin();
  ...
  static bool compare1(const index_key_value<Json>& item1,
                       const index_key_value<Json>& item2) {
    int comp = item1.name.compare(item2.name);
    if (comp < 0) return true;
    if (comp == 0) return item1.index < item2.index;
  ...
  static bool compare2(const index_key_value<Json>& item1,
                       const index_key_value<Json>& item2) {
  ...
  void merge_or_update(iterator hint,
                       const order_preserving_json_object& source) {
    std::size_t pos = hint - members_.begin();
    for (auto it = source.begin(); it != source.end(); ++it) {
      hint = insert_or_assign(hint, (*it).key(), (*it).value());
      std::size_t newpos = hint - members_.begin();

...
class basic_json_parser : public ser_context {
 ...
 private:
  ...
  std::basic_string<char_type, std::char_traits<char_type>, char_allocator_type>
      string_buffer_;
  jsoncons::detail::chars_to to_double_;

  std::vector<json_parse_state, parse_state_allocator_type> state_stack_;
  std::vector<std::pair<std::basic_string<char_type>, double>>
      string_double_map_;
 ...
 public:
  ...
  basic_json_parser(
      ...
      : basic_json_parser(basic_json_decode_options<char_type>(), err_handler,
                          temp_alloc) {}
  ...
  basic_json_parser(
    ...
    if (options_.enable_str_to_inf()) {
      string_double_map_.emplace_back(options_.inf_to_str(),
                                      std::numeric_limits<double>::infinity());
    }
    if (options_.enable_str_to_neginf()) {
      string_double_map_.emplace_back(options_.neginf_to_str(),
                                      -std::numeric_limits<double>::infinity());
    }
  }

  bool source_exhausted() const { return input_ptr_ == end_input_; }
  ...
  bool enter() const { return state_ == json_parse_state::start; }

  bool accept() const { return state_ == json_parse_state::accept || done_; }

  bool stopped() const { return !more_; }

  json_parse_state state() const { return state_; }

  bool finished() const { return !more_ && state_ != json_parse_state::accept; }

  const char_type* first() const { return begin_input_; }

  const char_type* current() const { return input_ptr_; }

  const char_type* last() const { return end_input_; }
  ...
  void begin_object(basic_json_visitor<char_type>& visitor,
                    std::error_code& ec) {
  ...
  void begin_array(basic_json_visitor<char_type>& visitor,
                   std::error_code& ec) {
  ...
  void update(const string_view_type sv) { update(sv.data(), sv.length()); }
  ...
  void parse_some_(basic_json_visitor<char_type>& visitor,
                   std::error_code& ec) {
  ...
  void parse_true(basic_json_visitor<char_type>& visitor, std::error_code& ec) {
    ...
    if (JSONCONS_LIKELY(end_input_ - input_ptr_ >= 4)) {
      if (*(input_ptr_ + 1) == 'r' && *(input_ptr_ + 2) == 'u' &&
          *(input_ptr_ + 3) == 'e') {
        input_ptr_ += 4;
        position_ += 4;
        more_ = visitor.bool_value(true, semantic_tag::none, *this, ec);
  ...
  void parse_null(basic_json_visitor<char_type>& visitor, std::error_code& ec) {
    saved_position_ = position_;
    if (JSONCONS_LIKELY(end_input_ - input_ptr_ >= 4)) {
      if (*(input_ptr_ + 1) == 'u' && *(input_ptr_ + 2) == 'l' &&
          *(input_ptr_ + 3) == 'l') {
        input_ptr_ += 4;
        position_ += 4;
        more_ = visitor.null_value(semantic_tag::none, *this, ec);
  ...
  void parse_false(basic_json_visitor<char_type>& visitor,
                   std::error_code& ec) {
    saved_position_ = position_;
    if (JSONCONS_LIKELY(end_input_ - input_ptr_ >= 5)) {
      if (*(input_ptr_ + 1) == 'a' && *(input_ptr_ + 2) == 'l' &&
          *(input_ptr_ + 3) == 's' && *(input_ptr_ + 4) == 'e') {
        input_ptr_ += 5;
        position_ += 5;
        more_ = visitor.bool_value(false, semantic_tag::none, *this, ec);
        if (parent() == json_parse_state::root) {
  ...
  void parse_number(basic_json_visitor<char_type>& visitor,
                    std::error_code& ec) {
    const char_type* local_input_end = end_input_;
  ...
  void parse_string(basic_json_visitor<char_type>& visitor,
                    std::error_code& ec) {
    const char_type* local_input_end = end_input_;
  ...
  void translate_conv_errc(unicode_traits::conv_errc result,
                           std::error_code& ec) {
    switch (result) {
  ...
  std::size_t position() const override { return saved_position_; }

  std::size_t end_position() const override { return position_; }

  std::size_t offset() const { return input_ptr_ - begin_input_; }

 private:
  void end_integer_value(basic_json_visitor<char_type>& visitor,
                         std::error_code& ec) {
    if (string_buffer_[0] == '-') {
  ...
  void end_negative_value(basic_json_visitor<char_type>& visitor,
                          std::error_code& ec) {
    int64_t val;
    auto result = jsoncons::detail::to_integer_unchecked(
        string_buffer_.data(), string_buffer_.length(), val);
    if (result) {
  ...
  void end_positive_value(basic_json_visitor<char_type>& visitor,
                          std::error_code& ec) {
    uint64_t val;
    auto result = jsoncons::detail::to_integer_unchecked(
        string_buffer_.data(), string_buffer_.length(), val);
  ...
  void end_fraction_value(basic_json_visitor<char_type>& visitor,
                          std::error_code& ec) {
    JSONCONS_TRY {
      if (options_.lossless_number()) {
        more_ = visitor.string_value(string_buffer_, semantic_tag::bigdec,
                                     *this, ec);
      } else {
  ...
  void end_string_value(const char_type* s, std::size_t length,
    ...
    switch (parent()) {
      ...
      case json_parse_state::array: {
        auto it =
            std::find_if(string_double_map_.begin(), string_double_map_.end(),
                         string_maps_to_double{sv});

...
#include "./byte_string.hpp"  // jsoncons::byte_traits
...
class basic_null_istream : public std::basic_istream<CharT> {
  class null_buffer : public std::basic_streambuf<CharT> {
   ...
   public:
    ...
    int_type overflow(
        int_type ch =
            typename std::basic_streambuf<CharT>::traits_type::eof()) override {
      return ch;
    }
  } nb_;

 public:
  basic_null_istream() : std::basic_istream<CharT>(&nb_) {}
...
class stream_source {
 ...
 private:
  using char_type = typename std::conditional<sizeof(CharT) == sizeof(char),
                                              char, CharT>::type;
 ...
 public:
  ...
  stream_source(std::basic_istream<char_type>& is,
                std::size_t buf_size = default_max_buffer_size)
      : stream_ptr_(std::addressof(is)),
        sbuf_(is.rdbuf()),
        position_(0),
        buffer_(buf_size),
        buffer_data_(buffer_.data()),
  ...
  stream_source(stream_source&& other) noexcept
    ...
    if (other.stream_ptr_ != &other.null_is_) {
      ...
      buffer_data_ =
          buffer_.data() + (other.buffer_data_ - other.buffer_.data());
  ...
  bool eof() const { return buffer_length_ == 0 && stream_ptr_->eof(); }

  bool is_error() const { return stream_ptr_->bad(); }
    ...
    if (length - len == 0) {
      ...
      JSONCONS_TRY {
        std::streamsize count =
            sbuf_->sgetn(reinterpret_cast<char_type*>(p + len), length - len);
 ...
 private:
  void fill_buffer() {
    ...
    buffer_data_ = buffer_.data();
    JSONCONS_TRY {
      std::streamsize count = sbuf_->sgetn(
...
class string_source {
 ...
 public:
  ...
  char_result<value_type> peek() {
    return current_ < end_ ? char_result<value_type>{*current_, false}
 ...
 public:
  ...
  read(value_type* data, std::size_t length) {
    std::size_t count = (std::min)(
        length, static_cast<std::size_t>(std::distance(current_, end_)));
...
class bytes_source {
 ...
 public:
  ...
  bytes_source(
      ...
      typename std::enable_if<
          extension_traits::is_byte_sequence<Sourceable>::value, int>::type = 0)
  ...
  std::size_t position() const { return current_ - data_; }
 ...
 public:
  ...
  char_result<value_type> peek() {
    return current_ != end_
               ? char_result<value_type>{static_cast<value_type>(*current_),
                                         false}
               : char_result<value_type>{0, true};
  }

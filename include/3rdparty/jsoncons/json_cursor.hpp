...
class basic_json_cursor : public basic_staj_cursor<CharT>,
 ...
 public:
  ...
  bool done() const override { return parser_.done() || done_; }
  ...
  void next(std::error_code& ec) override { read_next(ec); }
  ...
  const ser_context& context() const override { return *this; }
  ...
  std::size_t line() const override { return parser_.line(); }

  std::size_t column() const override { return parser_.column(); }

  friend basic_staj_filter_view<CharT> operator|(
      basic_json_cursor& cursor,
 ...
 private:
  ...
  void read_next(std::error_code& ec) { read_next(cursor_visitor_, ec); }
...
using json_stream_cursor =
    basic_json_cursor<char, jsoncons::stream_source<char>>;
using json_string_cursor =
    basic_json_cursor<char, jsoncons::string_source<char>>;
using wjson_stream_cursor =

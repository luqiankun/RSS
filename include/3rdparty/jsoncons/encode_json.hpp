...
encode_json(const T& val, CharContainer& cont,
            const basic_json_encode_options<
                typename CharContainer::value_type>& options =
...
encode_json(const allocator_set<Allocator, TempAllocator>& alloc_set,
  ...
  if (indent == indenting::no_indent) {
    ...
    basic_json_encoder<char_type, jsoncons::string_sink<CharContainer>,
                       TempAllocator>
        encoder(cont, options, alloc_set.get_temp_allocator());
    val.dump(encoder);
  }
...
encode_json(const allocator_set<Allocator, TempAllocator>& alloc_set,
            ...
            indenting indent = indenting::no_indent) {
  if (indent == indenting::no_indent) {
    basic_compact_json_encoder<CharT, TempAllocator> encoder(
        os, options, alloc_set.get_temp_allocator());
    val.dump(encoder);
  } else {
    basic_json_encoder<CharT, TempAllocator> encoder(
        os, options, alloc_set.get_temp_allocator());
    val.dump(encoder);
  }
}
...
encode_json_pretty(
    const T& val, CharContainer& cont,
    const basic_json_encode_options<typename CharContainer::value_type>&
        options =
            basic_json_encode_options<typename CharContainer::value_type>()) {
  using char_type = typename CharContainer::value_type;

...
class basic_staj_visitor : public basic_json_visitor<CharT> {
 ...
 public:
  ...
  bool in_available() const { return state_ != staj_cursor_state(); }
  ...
  staj_cursor_state state() const { return state_; }
  ...
  bool dump(basic_json_visitor<CharT>& visitor, const ser_context& context,
            std::error_code& ec) {
    bool more = true;
    if (is_typed_array()) {
      if (index_ != 0) {
...
class basic_staj_filter_view : basic_staj_cursor<CharT> {
 ...
 public:
  ...
  friend basic_staj_filter_view<CharT> operator|(
      basic_staj_filter_view& cursor,
      std::function<bool(const basic_staj_event<CharT>&, const ser_context&)>
          pred) {

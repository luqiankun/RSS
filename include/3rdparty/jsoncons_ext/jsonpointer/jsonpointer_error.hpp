...
class jsonpointer_error : public std::system_error,
 ...
 public:
  jsonpointer_error(const std::error_code& ec) : std::system_error(ec) {}
...
class jsonpointer_error_category_impl : public std::error_category {
 public:
  const char* name() const noexcept override { return "jsoncons/jsonpointer"; }
...
inline const std::error_category& jsonpointer_error_category() {
  static jsonpointer_error_category_impl instance;
  return instance;
}

inline std::error_code make_error_code(jsonpointer_errc result) {
  return std::error_code(static_cast<int>(result),
                         jsonpointer_error_category());
}
...
namespace std {
template <>
struct is_error_code_enum<jsoncons::jsonpointer::jsonpointer_errc>
    : public true_type {};
}  // namespace std

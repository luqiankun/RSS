...
enum class pointer_state { start, escaped, new_token, part };
...
class basic_json_pointer {
 public:
  ...
  using const_reverse_iterator =
      typename std::vector<string_type>::const_reverse_iterator;
 ...
 public:
  ...
  static basic_json_pointer parse(const string_view_type& input,
                                  std::error_code& ec) {
    ...
    if (state == jsonpointer::detail::pointer_state::new_token ||
        state == jsonpointer::detail::pointer_state::part) {
      tokens.push_back(buffer);
    }
  ...
  static string_type unescape_uri_string(const string_view_type& s,
                                         std::error_code& ec) {
    if (s.size() < 3) {
    ...
    while (pos < last) {
      if (s[pos] == '%') {
        uint8_t ch;
        auto result =
            jsoncons::detail::to_integer_base16(s.data() + (pos + 1), 2, ch);
        if (!result) {
  ...
  string_type string() const { return to_string(); }
#endif
  ...
  reverse_iterator rbegin() const { return tokens_.rbegin(); }
  reverse_iterator rend() const { return tokens_.rend(); }

  // Non-member functions
  friend basic_json_pointer<CharT> operator/(
      const basic_json_pointer<CharT>& lhs, const string_type& rhs) {
  ...
  friend basic_json_pointer<CharT> operator+(
      const basic_json_pointer<CharT>& lhs,
      const basic_json_pointer<CharT>& rhs) {
    basic_json_pointer<CharT> p(lhs);
    p += rhs;
    return p;
  }

  friend bool operator==(const basic_json_pointer& lhs,
                         const basic_json_pointer& rhs) {
  ...
  friend std::basic_ostream<CharT>& operator<<(
      std::basic_ostream<CharT>& os, const basic_json_pointer<CharT>& p) {
    os << p.to_string();
    return os;
  }
};
...
inline std::string to_string(const json_pointer& ptr) {
  return ptr.to_string();
}

inline std::wstring to_wstring(const wjson_pointer& ptr) {
  return ptr.to_string();
}
...
JSONCONS_DEPRECATED_MSG("Instead, use json_pointer")
typedef json_pointer address;
JSONCONS_DEPRECATED_MSG("Instead, use json_pointer")
typedef json_pointer json_ptr;
JSONCONS_DEPRECATED_MSG("Instead, use wjson_pointer")
typedef wjson_pointer wjson_ptr;
#endif

namespace detail {

template <class Json>
const Json* resolve(const Json* current,
                    const typename Json::string_view_type& buffer,
                    std::error_code& ec) {
  if (current->is_array()) {
    ...
    std::size_t index{0};
    auto result = jsoncons::detail::to_integer_decimal(buffer.data(),
                                                       buffer.length(), index);
    if (!result) {
...
template <class Json>
Json* resolve(Json* current, const typename Json::string_view_type& buffer,
              bool create_if_missing, std::error_code& ec) {
  if (current->is_array()) {
    if (buffer.size() == 1 && buffer[0] == '-') {
      ec = jsonpointer_errc::index_exceeds_array_size;
      return current;
...
Json& get(Json& root,
  ...
  while (it != end) {
    current = jsoncons::jsonpointer::detail::resolve(current, *it,
                                                     create_if_missing, ec);
    if (ec) return *current;
    ++it;
  }
  return *current;
}
...
typename std::enable_if<
    std::is_convertible<StringSource, jsoncons::basic_string_view<
                                          typename Json::char_type>>::value,
    Json&>::type
get(Json& root, const StringSource& location_str, bool create_if_missing,
    std::error_code& ec) {
  auto jsonptr =
      basic_json_pointer<typename Json::char_type>::parse(location_str, ec);
  if (ec) {
    return root;
  }
...
typename std::enable_if<
    std::is_convertible<StringSource, jsoncons::basic_string_view<
                                          typename Json::char_type>>::value,
    const Json&>::type
get(const Json& root, const StringSource& location_str, std::error_code& ec) {
  auto jsonptr =
      basic_json_pointer<typename Json::char_type>::parse(location_str, ec);
  if (ec) {
    return root;
...
typename std::enable_if<
    std::is_convertible<StringSource, jsoncons::basic_string_view<
                                          typename Json::char_type>>::value,
    Json&>::type
get(Json& root, const StringSource& location_str, std::error_code& ec) {
...
typename std::enable_if<
    std::is_convertible<StringSource, jsoncons::basic_string_view<
                                          typename Json::char_type>>::value,
    Json&>::type
get(Json& root, const StringSource& location_str,
    bool create_if_missing = false) {
  std::error_code ec;
...
void add(Json& root,
         const basic_json_pointer<typename Json::char_type>& location,
         T&& value, bool create_if_missing, std::error_code& ec) {
  Json* current = std::addressof(root);
  ...
  while (it != end) {
    ...
    ++it;
    if (it != end) {
      current = jsoncons::jsonpointer::detail::resolve(current, buffer,
                                                       create_if_missing, ec);
      if (ec) return;
    }
  }
  if (current->is_array()) {
    if (buffer.size() == 1 && buffer[0] == '-') {
      ...
      std::size_t index{0};
      auto result = jsoncons::detail::to_integer_decimal(
          buffer.data(), buffer.length(), index);
      if (!result) {
        ec = jsonpointer_errc::invalid_index;
        return;
      }
      ...
      if (index == current->size()) {
        ...
        auto it2 = current->insert(current->array_range().begin() + index,
                                   std::forward<T>(value));
...
typename std::enable_if<
    std::is_convertible<StringSource, jsoncons::basic_string_view<
                                          typename Json::char_type>>::value,
    void>::type
add(Json& root, const StringSource& location_str, T&& value,
    bool create_if_missing, std::error_code& ec) {
  auto jsonptr =
      basic_json_pointer<typename Json::char_type>::parse(location_str, ec);
  if (ec) {
...
void add(Json& root,
         const basic_json_pointer<typename Json::char_type>& location,
         T&& value, bool create_if_missing = false) {
  std::error_code ec;
...
typename std::enable_if<
    std::is_convertible<StringSource, jsoncons::basic_string_view<
                                          typename Json::char_type>>::value,
    void>::type
add_if_absent(Json& root, const StringSource& location_str, T&& value,
              bool create_if_missing, std::error_code& ec) {
  auto jsonptr =
      basic_json_pointer<typename Json::char_type>::parse(location_str, ec);
  if (ec) {
...
typename std::enable_if<
    std::is_convertible<StringSource, jsoncons::basic_string_view<
                                          typename Json::char_type>>::value,
    void>::type
add_if_absent(Json& root, const StringSource& location, T&& value,
              std::error_code& ec) {
...
add_if_absent(Json& root, const StringSource& location_str, T&& value,
              bool create_if_missing = false) {
  std::error_code ec;
  add_if_absent(root, location_str, std::forward<T>(value), create_if_missing,
                ec);
  if (ec) {
    JSONCONS_THROW(jsonpointer_error(ec));
  }
...
void remove(Json& root,
  ...
  while (it != end) {
    ...
    if (it != end) {
      current =
          jsoncons::jsonpointer::detail::resolve(current, buffer, false, ec);
      if (ec) return;
    }
  }
  if (current->is_array()) {
...
typename std::enable_if<
    std::is_convertible<StringSource, jsoncons::basic_string_view<
                                          typename Json::char_type>>::value,
    void>::type
replace(Json& root, const StringSource& location_str, T&& value,
        bool create_if_missing, std::error_code& ec) {
  auto jsonptr =
      basic_json_pointer<typename Json::char_type>::parse(location_str, ec);
  if (ec) {
...
typename std::enable_if<std::is_convertible<
    typename String::value_type, typename Result::value_type>::value>::type
...
void flatten_(const std::basic_string<typename Json::char_type>& parent_key,
              const Json& parent_value, Json& result) {
    ...
    case json_type::object_value: {
      if (parent_value.empty()) {
        ...
        for (const auto& item : parent_value.object_range()) {
          string_type key(parent_key);
          key.push_back('/');
          escape(jsoncons::basic_string_view<char_type>(item.key().data(),
                                                        item.key().size()),
                 key);
...
Json safe_unflatten(Json& value) {
  ...
  for (const auto& item : value.object_range()) {
    std::size_t n;
    auto r = jsoncons::detail::to_integer_decimal(item.key().data(),
                                                  item.key().size(), n);
    if (!r || (index++ != n)) {
      safe = false;
      break;
...
jsoncons::optional<Json> try_unflatten_array(const Json& value) {
  ...
  if (JSONCONS_UNLIKELY(!value.is_object())) {
    JSONCONS_THROW(
        jsonpointer_error(jsonpointer_errc::argument_to_unflatten_invalid));
  }
  Json result;
...
template <class Json>
Json unflatten_to_object(const Json& value,
                         unflatten_options options = unflatten_options::none) {
  using char_type = typename Json::char_type;
...
JSONCONS_DEPRECATED_MSG(
    "Instead, use add(Json&, const typename Json::string_view_type&, const "
    "Json&)")
void insert_or_assign(
    Json& root, const std::basic_string<typename Json::char_type>& location,
    const Json& value) {
...
void insert_or_assign(
    Json& root, const std::basic_string<typename Json::char_type>& location,
    const Json& value, std::error_code& ec) {

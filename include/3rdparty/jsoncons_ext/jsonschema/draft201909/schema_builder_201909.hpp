...
#include "../draft201909/schema_draft201909.hpp"
...
class schema_builder_201909 : public schema_builder<Json> {
 public:
  ...
  static const std::string& format_annotation_id() {
    static std::string id =
        "https://json-schema.org/draft/2019-09/format-annotation";
    return id;
  }
 ...
 public:
  schema_builder_201909(
      Json&& sch, const schema_builder_factory_type& builder_factory,
      ...
      : schema_builder<Json>(schema_version::draft201909(), std::move(sch),
...
*/
  ...
  schema_validator_type make_schema_validator(
    ...
    // std::cout << "make_schema_validator " << context.get_base_uri().string()
  ...
  schema_validator_type make_object_schema_validator(
    ...
    if (this->options().compatibility_mode()) {
      ...
      if (it != sch.object_range().end()) {
        for (const auto& def : it->value().object_range()) {
          ...
          defs.emplace(def.key(), make_schema_validator(context, def.value(),
    ...
    if (it != sch.object_range().end())  // this schema has a reference
    {
      ...
      auto orig = jsoncons::make_unique<recursive_ref_validator_type>(
          sch, ref.uri().base());
    ...
    if (include_applicator_) {
      ...
      if (it != sch.object_range().end()) {
        validators.emplace_back(this->make_property_names_validator(
            context, it->value(), sch, anchor_dict));
      }
      ...
      if (it != sch.object_range().end()) {
        ...
        if_validator =
            make_schema_validator(context, it->value(), sub_keys, anchor_dict);
      }
      ...
      if (it != sch.object_range().end()) {
        validators.emplace_back(this->make_additional_properties_validator(
            ...
            std::move(pattern_properties), anchor_dict));
      } else {
        if (properties) {
      ...
      if (it != sch.object_range().end()) {
        if (it->value().type() == json_type::array_value) {
          validators.emplace_back(this->make_prefix_items_validator_07(
              context, it->value(), sch, anchor_dict));
          ...
          validators.emplace_back(this->make_items_validator(
              "items", context, it->value(), sch, anchor_dict));
        }
      }
    }
    if (include_validation_) {
      for (const auto& key_value : sch.object_range()) {
        ...
        if (factory_it != validation_factory_map_.end()) {
          auto validator =
              factory_it->second(context, key_value.value(), sch, anchor_dict);
          if (validator) {
    ...
    if (sch.is_object()) {
      ...
      if (it != sch.object_range().end()) {
        ...
        if (relative.has_fragment()) {
          JSONCONS_THROW(
              schema_error("Draft 2019-09 does not allow $id with fragment"));
        }
        uri_wrapper new_uri =

...
#ifndef JSONCONS_JSONSCHEMA_DRAFT201909_DRAFT201909_SCHEMA_BUILDER_HPP
#define JSONCONS_JSONSCHEMA_DRAFT201909_DRAFT201909_SCHEMA_BUILDER_HPP
...
template <class Json>
class draft201909_schema_builder : public schema_builder<Json> {
 public:
 ...
 public:
  draft201909_schema_builder(const uri_resolver<Json>& resolver) noexcept
      : resolver_(resolver) {
    init();
  }

  draft201909_schema_builder(const draft201909_schema_builder&) = delete;
  draft201909_schema_builder& operator=(const draft201909_schema_builder&) =
      delete;
  draft201909_schema_builder(draft201909_schema_builder&&) = default;
  draft201909_schema_builder& operator=(draft201909_schema_builder&&) = default;
  ...
  void init() {
    ...
    keyword_factory_map_.emplace(
        "dependentRequired",
        [&](const compilation_context& context, const Json& sch, const Json&) {
          return this->make_dependent_required_validator(context, sch);
        });
    keyword_factory_map_.emplace(
        "dependentSchemas",
        [&](const compilation_context& context, const Json& sch, const Json&) {
          return this->make_dependent_schemas_validator(context, sch);
        });
    keyword_factory_map_.emplace(
        "unevaluatedProperties",
        [&](const compilation_context& context, const Json& sch, const Json&) {
          return this->make_unevaluated_properties_validator(context, sch);
        });
  }
  ...
  schema_validator_type make_object_schema_validator(
    ...
    if (it != sch.object_range().end())  // this schema has a reference
    {
      ...
      validators.push_back(
          jsoncons::make_unique<recursive_ref_validator_type>(id.uri()));
    }
  ...
  std::unique_ptr<items_object_validator<Json>> make_items_object_validator(
      const compilation_context& context, const Json& /* parent */,
      const Json& sch) {
    uri schema_path{context.make_schema_path_with("items")};

...
#include "../draft4/schema_draft4.hpp"
...
class schema_builder_4 : public schema_builder<Json> {
 ...
 public:
  schema_builder_4(Json&& sch,
      ...
      : schema_builder<Json>(schema_version::draft4(), std::move(sch),
  ...
  void init() {
    ...
    keyword_factory_map_.emplace(
        "maximum", [&](const compilation_context& context, const Json& sch,
                       const Json& parent, anchor_uri_map_type&) {
          return make_maximum_validator_4(context, sch, parent);
        });
    keyword_factory_map_.emplace(
        "minimum", [&](const compilation_context& context, const Json& sch,
                       const Json& parent, anchor_uri_map_type&) {
          return make_minimum_validator_4(context, sch, parent);
        });
    keyword_factory_map_.emplace(
  ...
  std::unique_ptr<keyword_validator<Json>> make_maximum_validator_4(
      const compilation_context& context, const Json& sch, const Json& parent) {
    ...
    if (is_exclusive) {
      ...
      return jsoncons::make_unique<maximum_validator<Json>>(
          parent, schema_location, sch);
    }
  }

  virtual std::unique_ptr<keyword_validator<Json>> make_minimum_validator_4(
      const compilation_context& context, const Json& sch, const Json& parent) {
    ...
    if (is_exclusive) {
      ...
      return jsoncons::make_unique<minimum_validator<Json>>(
          parent, schema_location, sch);
    }
  }

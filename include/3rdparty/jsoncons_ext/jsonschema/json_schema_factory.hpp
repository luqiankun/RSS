...
class schema_builder_factory {
 public:
  ...
  std::unique_ptr<schema_builder<Json>> operator()(
      Json sch, const evaluation_options& options,
    ...
    if (sch.is_object()) {
      ...
      if (it != sch.object_range().end()) {
        builder = get_builder(std::move(sch), it->value().as_string_view(),
        ...
        builder = get_default_schema_builder(
            std::move(sch), options, schema_store_ptr, resolvers, vocabulary);
      }
    } else if (sch.is_bool()) {
      builder = get_default_schema_builder(
          std::move(sch), options, schema_store_ptr, resolvers, vocabulary);
  ...
  std::unique_ptr<schema_builder<Json>> get_default_schema_builder(
      Json&& sch, const evaluation_options& options,
      schema_store_type* schema_store_ptr,
      const std::vector<schema_resolver<Json>>& resolvers,
      const std::unordered_map<std::string, bool>& vocabulary) const {
    if (options.default_version() == schema_version::draft202012()) {
      return jsoncons::make_unique<
          jsoncons::jsonschema::draft202012::schema_builder_202012<Json>>(
          std::move(sch), *this, options, schema_store_ptr, resolvers,
          vocabulary);
    } else if (options.default_version() == schema_version::draft201909()) {
      return jsoncons::make_unique<
          jsoncons::jsonschema::draft201909::schema_builder_201909<Json>>(
          std::move(sch), *this, options, schema_store_ptr, resolvers,
          vocabulary);
    } else if (options.default_version() == schema_version::draft7()) {
      return jsoncons::make_unique<
          jsoncons::jsonschema::draft7::schema_builder_7<Json>>(
          std::move(sch), *this, options, schema_store_ptr, resolvers);
    } else if (options.default_version() == schema_version::draft6()) {
      return jsoncons::make_unique<
          jsoncons::jsonschema::draft6::schema_builder_6<Json>>(
          std::move(sch), *this, options, schema_store_ptr, resolvers);
    } else if (options.default_version() == schema_version::draft4()) {
      return jsoncons::make_unique<
          jsoncons::jsonschema::draft4::schema_builder_4<Json>>(
          std::move(sch), *this, options, schema_store_ptr, resolvers);
    } else {
      JSONCONS_THROW(schema_error("Unsupported schema version " +
                                  options.default_version()));
    }
  }

  std::unique_ptr<schema_builder<Json>> get_builder(
      Json&& sch, const jsoncons::string_view& schema_id,
    ...
    if (schema_id == schema_version::draft202012()) {
      ...
      builder = get_builder_from_meta_schema(std::move(sch), schema_id, options,
                                             schema_store_ptr, resolvers);
    }
    return builder;
  }

  std::unique_ptr<schema_builder<Json>> get_builder_from_meta_schema(
      Json&& sch, const jsoncons::string_view& schema_id,
      const evaluation_options& options, schema_store_type* schema_store_ptr,
    ...
    for (auto it = resolvers.begin(); it != resolvers.end() && !found; ++it) {
      ...
      if (meta_sch.is_object()) {
        ...
        if (schema_it != meta_sch.object_range().end()) {
          builder =
              get_builder(std::move(sch), schema_it->value().as_string_view(),
                          options, schema_store_ptr, resolvers, vocabulary);
          found = true;
        }
      }
...
make_json_schema(Json sch, const std::string& retrieval_uri,
  ...
  std::unordered_map<std::string, bool> vocabulary{};
  std::vector<schema_resolver<Json>> resolvers = {
      {meta_resolver<Json>, resolver}};
  auto schema_builder = builder_factory(std::move(sch), options, &schema_store,
...
make_schema(Json sch, const std::string& retrieval_uri,
            const URIResolver& resolver) {
  using schema_store_type = std::map<jsoncons::uri, schema_validator<Json>*>;
  schema_store_type schema_store;
  schema_builder_factory<Json> builder_factory{};
  ...
  std::vector<schema_resolver<Json>> resolvers = {
      {meta_resolver<Json>, resolver}};
  auto schema_builder =
      builder_factory(std::move(sch),
                      jsonschema::evaluation_options{}.default_version(
                          jsonschema::schema_version::draft7()),
                      &schema_store, resolvers, vocabulary);

  schema_builder->build_schema(retrieval_uri);
  return std::make_shared<json_schema<Json>>(

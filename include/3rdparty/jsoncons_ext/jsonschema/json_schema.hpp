...
class throwing_error_listener : public error_reporter {
  walk_result do_error(const validation_message& msg) override {
    JSONCONS_THROW(validation_error(msg.instance_location().string() + ": " +
                                    msg.message()));
  }
};
...
using error_reporter_t =
    std::function<walk_result(const validation_message& msg)>;
...
class json_schema {
  ...
  using document_schema_validator_type =
      std::unique_ptr<document_schema_validator<Json>>;
 ...
 public:
  json_schema(document_schema_validator_type&& root) : root_(std::move(root)) {
    if (root_ == nullptr)
      JSONCONS_THROW(schema_error(
          "There is no root schema to validate an instance against"));
  }
  ...
  Json validate(const Json& instance) const {
    ...
    root_->validate(context, instance, instance_location, results, reporter,
                    patch);
    return patch;
  }
  ...
  validate(const Json& instance, const MsgReporter& reporter) const {
    ...
    evaluation_context<Json> context;
    evaluation_results results;
    root_->validate(context, instance, instance_location, results, adaptor,
                    patch);
  }
  ...
  void walk(const Json& instance, const WalkReporter& reporter) const {
    ...
    root_->walk(evaluation_context<Json>{}, instance, instance_location,
                reporter);
  }

 private:
  // Validate input JSON against a JSON Schema with a provided error reporter
  void validate2(const Json& instance, error_reporter& reporter,
                 Json& patch) const {
    jsonpointer::json_pointer instance_location{};
    patch = Json(json_array_arg);

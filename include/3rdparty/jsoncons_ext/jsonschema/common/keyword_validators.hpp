...
class recursive_ref_validator : public keyword_validator_base<Json>,
 ...
 public:
  ...
  uri get_base_uri() const { return this->schema_location(); }

  void set_referred_schema(const schema_validator<Json>* target) final {
    tentative_target_ = target;
  }
 ...
 private:
  walk_result do_validate(const evaluation_context<Json>& context,
                          ...
                          evaluation_results& results, error_reporter& reporter,
    ...
    if (schema_ptr == nullptr) {
      walk_result result = reporter.error(validation_message(
          this->keyword_name(), this_context.eval_path(),
          this->schema_location(), instance_location,
          "Unresolved schema reference " + this->schema_location().string()));
      return result;
    }

    return schema_ptr->validate(this_context, instance, instance_location,
                                results, reporter, patch);
  }

  walk_result do_walk(const evaluation_context<Json>& context,
                      const Json& instance,
                      const jsonpointer::json_pointer& instance_location,
                      const walk_reporter_type& reporter) const final {
    auto rit = context.dynamic_scope().rbegin();
    auto rend = context.dynamic_scope().rend();
    ...
    // std::cout << "recursive_ref_validator.do_validate " << "keywordLocation:
    // << " << this->schema_location().string() << ", instanceLocation:" <<
    ...
    evaluation_context<Json> this_context(context, this->keyword_name());
    return schema_ptr->walk(this_context, instance, instance_location,
                            reporter);
  }
};
...
class dynamic_ref_validator : public keyword_validator_base<Json>,
 ...
 public:
  dynamic_ref_validator(const Json& schema, const uri& schema_location,
                        const uri_wrapper& value)
      : keyword_validator_base<Json>("$dynamicRef", schema, schema_location),
        value_(value) {
    // std::cout << "dynamic_ref_validator path: " << schema_location.string()
    // << ", value: " << value.string() << "\n";
  }
 ...
 private:
  walk_result do_validate(const evaluation_context<Json>& context,
                          ...
                          evaluation_results& results, error_reporter& reporter,
                          Json& patch) const final {
    // std::cout << "dynamic_ref_validator [" << context.eval_path().string() <<
    // "," << this->schema_location().string() << "]"; std::cout <<
    ...
    if (value_.has_plain_name_fragment() && schema_ptr->dynamic_anchor()) {
      while (rit != rend) {
        auto p = (*rit)->get_schema_for_dynamic_anchor(
            schema_ptr->dynamic_anchor()->fragment());
        // std::cout << "  (2) [" << (*rit)->schema_location().string() << "] "
        // << ((*rit)->dynamic_anchor() ?
        // (*rit)->dynamic_anchor()->value().string() : "") << "\n";
    ...
    // std::cout << "dynamic_ref_validator.do_validate " << "keywordLocation: <<
    // " << this->schema_location().string() << ", instanceLocation:" <<
    // instance_location.string() << "\n";
...
class content_encoding_validator : public keyword_validator_base<Json> {
 ...
 public:
  content_encoding_validator(const Json& schema, const uri& schema_location,
                             const std::string& content_encoding)
      : keyword_validator_base<Json>("contentEncoding", schema,
                                     schema_location),
 ...
 private:
    ...
    if (content_encoding_ == "base64") {
      ...
      if (retval.ec != jsoncons::conv_errc::success) {
        walk_result result = reporter.error(
            validation_message(this->keyword_name(), this_context.eval_path(),
                               this->schema_location(), instance_location,
  ...
  walk_result do_walk(const evaluation_context<Json>& /*context*/,
                      ...
                      const jsonpointer::json_pointer& instance_location,
                      const walk_reporter_type& reporter) const final {
    return reporter(this->keyword_name(), this->schema(),
                    this->schema_location(), instance, instance_location);
  }
};
...
class content_media_type_validator : public keyword_validator_base<Json> {
 ...
 public:
  content_media_type_validator(const Json& schema, const uri& schema_location,
                               const std::string& content_media_type,
                               const std::string& content_encoding)
      : keyword_validator_base<Json>("contentMediaType", schema,
                                     schema_location),
...
class format_validator : public keyword_validator_base<Json> {
 ...
 public:
  format_validator(const Json& schema, const uri& schema_location,
                   const format_checker& format_check)
      : keyword_validator_base<Json>("format", schema, schema_location),
        format_check_(format_check) {}
 ...
 private:
  walk_result do_validate(const evaluation_context<Json>& context,
    ...
    if (format_check_ != nullptr) {
      ...
      walk_result result =
          format_check_(this_context.eval_path(), this->schema_location(),
                        instance_location, s, reporter);
      if (result == walk_result::abort) {
        return result;
      }
    }
 ...
 private:
  walk_result do_validate(const evaluation_context<Json>& context,
    ...
    if (!std::regex_search(s, regex_)) {
      ...
      walk_result result = reporter.error(validation_message(
          this->keyword_name(), this_context.eval_path(),
          this->schema_location(), instance_location, std::move(message)));
      if (result == walk_result::abort) {
        return result;
      }
    }
...
class pattern_validator : public keyword_validator_base<Json> {
 ...
 private:
  walk_result do_validate(const Json&, const jsonpointer::json_pointer&,
                          evaluation_results& /*results*/, error_reporter&,
                          Json& /*patch*/) const final {
    return walk_result::advance;
  }
...
class max_length_validator : public keyword_validator_base<Json> {
 ...
 public:
  max_length_validator(const Json& schema, const uri& schema_location,
                       std::size_t max_length)
      : keyword_validator_base<Json>("maxLength", schema, schema_location),
  ...
  walk_result do_validate(const evaluation_context<Json>& context,
    ...
    if (length > max_length_) {
      walk_result result = reporter.error(validation_message(
          this->keyword_name(), this_context.eval_path(),
          this->schema_location(), instance_location,
          std::string("Number of characters must be at most ") +
...
class max_items_validator : public keyword_validator_base<Json> {
 ...
 public:
  max_items_validator(const Json& schema, const uri& schema_location,
                      std::size_t max_items)
      : keyword_validator_base<Json>("maxItems", schema, schema_location),
 ...
 private:
  walk_result do_validate(const evaluation_context<Json>& context,
    ...
    if (instance.size() > max_items_) {
      std::string message("Maximum number of items is " +
...
class min_items_validator : public keyword_validator_base<Json> {
 ...
 public:
  min_items_validator(const Json& schema, const uri& schema_location,
                      std::size_t min_items)
      : keyword_validator_base<Json>("minItems", schema, schema_location),
 ...
 private:
  walk_result do_validate(const evaluation_context<Json>& context,
    ...
    if (instance.size() < min_items_) {
      std::string message("Minimum number of items is " +
                          std::to_string(min_items_));
      message.append(" but found " + std::to_string(instance.size()));
      walk_result result = reporter.error(validation_message(
          this->keyword_name(), this_context.eval_path(),
          this->schema_location(), instance_location, std::move(message)));
...
class items_validator : public keyword_validator_base<Json> {
 ...
 public:
  items_validator(const std::string& keyword_name, const Json& schema,
 ...
 private:
  walk_result do_validate(const evaluation_context<Json>& context,
    ...
    if (instance.size() > 0 && schema_val_) {
      if (schema_val_->always_fails()) {
        ...
        walk_result result = reporter.error(validation_message(
            this->keyword_name(), this_context.eval_path(),
            this->schema_location(), item_location,
        ...
        for (const auto& item : instance.array_range()) {
          ...
          walk_result result = schema_val_->validate(
              this_context, item, item_location, results, reporter, patch);
          if (result == walk_result::abort) {
            return result;
          }
  ...
  walk_result do_walk(const evaluation_context<Json>& context,
    ...
    walk_result result =
        reporter(this->keyword_name(), this->schema(), this->schema_location(),
...
class unique_items_validator : public keyword_validator_base<Json> {
 ...
 public:
  unique_items_validator(const Json& schema, const uri& schema_location,
                         bool are_unique)
      : keyword_validator_base<Json>("uniqueItems", schema, schema_location),
        are_unique_(are_unique) {}
...
class min_length_validator : public keyword_validator_base<Json> {
 ...
 public:
  min_length_validator(const Json& schema, const uri& schema_location,
                       std::size_t min_length)
      : keyword_validator_base<Json>("minLength", schema, schema_location),
  ...
  walk_result do_validate(const evaluation_context<Json>& context,
    ...
    if (length < min_length_) {
      walk_result result = reporter.error(validation_message(
          this->keyword_name(), this_context.eval_path(),
          this->schema_location(), instance_location,
          std::string("Number of characters must be at least ") +
...
class not_validator : public keyword_validator_base<Json> {
 ...
 private:
  walk_result do_validate(const evaluation_context<Json>& context,
    ...
    walk_result result =
        schema_val_->validate(this_context, instance, instance_location,
                              local_results, local_reporter, patch);
    if (result == walk_result::abort) {
      return result;
    }
...
class any_of_validator : public keyword_validator_base<Json> {
 ...
 private:
  walk_result do_validate(const evaluation_context<Json>& context,
                          ...
                          evaluation_results& results, error_reporter& reporter,
                          Json& patch) const final {
    // std::cout << "any_of_validator.do_validate " <<
    // context.eval_path().string() << ", " << instance << "\n";
    ...
    for (std::size_t i = 0; i < validators_.size(); ++i) {
      ...
      walk_result result =
          validators_[i]->validate(item_context, instance, instance_location,
                                   local_results2, local_reporter, patch);
      if (result == walk_result::abort) {
        return result;
      }
    ...
    if (count > 0) {
      ...
      walk_result result = reporter.error(
          validation_message(this->keyword_name(), this_context.eval_path(),
                             this->schema_location(), instance_location,
                             "Must be valid against at least one schema, but "
                             "found no matching schemas",
  ...
  walk_result do_walk(const evaluation_context<Json>& context,
    ...
    for (std::size_t i = 0; i < validators_.size(); ++i) {
      ...
      result = validators_[i]->walk(item_context, instance, instance_location,
                                    reporter);
      if (result == walk_result::abort) {
        return result;
      }
 ...
 private:
  walk_result do_validate(const evaluation_context<Json>& context,
    ...
    if (indices.size() == 1) {
      ...
      if (indices.size() == 0) {
        message =
            "Must be valid against exactly one schema, but found no matching "
            "schemas";
      } else {
        message = "Must be valid against exactly one schema, but found " +
                  std::to_string(indices.size()) +
                  " matching schemas at indices ";
...
class all_of_validator : public keyword_validator_base<Json> {
 ...
 private:
  walk_result do_validate(const evaluation_context<Json>& context,
                          ...
                          evaluation_results& results, error_reporter& reporter,
                          Json& patch) const final {
    // std::cout << this->keyword_name() << " [" << context.eval_path().string()
    // << ", " << this->schema_location().string() << "]\n";
...
class maximum_validator : public keyword_validator_base<Json> {
 ...
 public:
  maximum_validator(const Json& schema, const uri& schema_location,
                    const Json& value)
      : keyword_validator_base<Json>("maximum", schema, schema_location),
        value_(value),
        message_{"Maximum value is " + value.template as<std::string>() +
                 " but found"} {}
 ...
 private:
  walk_result do_validate(const evaluation_context<Json>& context,
    ...
    } else if (instance.is_string_view() &&
               instance.tag() == semantic_tag::bigint) {
...
class exclusive_maximum_validator : public keyword_validator_base<Json> {
 ...
 public:
  exclusive_maximum_validator(const Json& schema, const uri& schema_location,
                              const Json& value)
      : keyword_validator_base<Json>("exclusiveMaximum", schema,
                                     schema_location),
        value_(value),
        message_{"Exclusive maximum value is " +
                 value.template as<std::string>() + " but found "} {}
...
class minimum_validator : public keyword_validator_base<Json> {
 ...
 public:
  minimum_validator(const Json& schema, const uri& schema_location,
                    const Json& value)
      : keyword_validator_base<Json>("minimum", schema, schema_location),
        value_(value),
        message_{"Minimum value is " + value.template as<std::string>() +
                 " but found "} {}
...
class exclusive_minimum_validator : public keyword_validator_base<Json> {
 ...
 public:
  exclusive_minimum_validator(const Json& schema, const uri& schema_location,
                              const Json& value)
      : keyword_validator_base<Json>("exclusiveMinimum", schema,
                                     schema_location),
        value_(value),
        message_{"Exclusive minimum value is " +
                 value.template as<std::string>() + " but found "} {}
...
class multiple_of_validator : public keyword_validator_base<Json> {
 ...
 public:
  multiple_of_validator(const Json& schema, const uri& schema_location,
                        double value)
      : keyword_validator_base<Json>("multipleOf", schema, schema_location),
        value_(value) {}
    ...
    if (value != 0)  // Exclude zero
    {
      if (!is_multiple_of(value, static_cast<double>(value_))) {
        walk_result result = reporter.error(validation_message(
            this->keyword_name(), this_context.eval_path(),
            this->schema_location(), instance_location,
            instance.template as<std::string>() + " is not a multiple of " +
...
class required_validator : public keyword_validator_base<Json> {
 ...
 public:
  required_validator(const Json& schema, const uri& schema_location,
      ...
      : keyword_validator_base<Json>("required", schema, schema_location),
        items_(items) {}
...
class max_properties_validator : public keyword_validator_base<Json> {
 ...
 public:
  max_properties_validator(const Json& schema, const uri& schema_location,
                           std::size_t max_properties)
      : keyword_validator_base<Json>("maxProperties", schema, schema_location),
 ...
 private:
  walk_result do_validate(const evaluation_context<Json>& context,
    ...
    if (instance.size() > max_properties_) {
      ...
      std::string message("Maximum number of properties is " +
                          std::to_string(max_properties_));
      message.append(" but found " + std::to_string(instance.size()));
      walk_result result = reporter.error(validation_message(
          this->keyword_name(), this_context.eval_path(),
          this->schema_location(), instance_location, std::move(message)));
...
class min_properties_validator : public keyword_validator_base<Json> {
 ...
 public:
  min_properties_validator(const Json& schema, const uri& schema_location,
                           std::size_t min_properties)
      : keyword_validator_base<Json>("minProperties", schema, schema_location),
 ...
 private:
  walk_result do_validate(const evaluation_context<Json>& context,
    ...
    if (instance.size() < min_properties_) {
      ...
      std::string message("Minimum number of properties is " +
                          std::to_string(min_properties_));
      message.append(" but found " + std::to_string(instance.size()));
      walk_result result = reporter.error(validation_message(
          this->keyword_name(), this_context.eval_path(),
          this->schema_location(), instance_location, std::move(message)));
 ...
 private:
  walk_result do_validate(const evaluation_context<Json>& context,
    ...
    if (if_val_) {
      ...
      walk_result result =
          if_val_->validate(if_context, instance, instance_location,
                            local_results, local_reporter, patch);
      if (result == walk_result::abort) {
        return result;
      }
      ...
      if (local_reporter.errors.empty()) {
        ...
        if (then_val_) {
          ...
          result =
              then_val_->validate(then_context, instance, instance_location,
                                  results, reporter, patch);
          if (result == walk_result::abort) {
            return result;
          }
        ...
        if (else_val_) {
          ...
          result =
              else_val_->validate(else_context, instance, instance_location,
                                  results, reporter, patch);
          if (result == walk_result::abort) {
            return result;
          }
  ...
  walk_result do_walk(const evaluation_context<Json>& context,
    ...
    if (if_val_) {
      ...
      if (then_val_) {
        evaluation_context<Json> then_context(context, "then");
        result = then_val_->walk(then_context, instance, instance_location,
                                 reporter);
        if (result == walk_result::abort) {
          return result;
        }
      ...
      if (else_val_) {
        evaluation_context<Json> else_context(context, "else");
        result = else_val_->walk(else_context, instance, instance_location,
                                 reporter);
        if (result == walk_result::abort) {
          return result;
        }
...
class enum_validator : public keyword_validator_base<Json> {
 ...
 public:
  enum_validator(const Json& schema, const uri& schema_location,
                 const Json& sch)
      : keyword_validator_base<Json>("enum", schema, schema_location),
        value_(sch) {}
  ...
  walk_result do_validate(const evaluation_context<Json>& context,
    ...
    if (!in_range) {
      walk_result result = reporter.error(
          validation_message(this->keyword_name(), this_context.eval_path(),
                             this->schema_location(), instance_location,
                             "'" + instance.template as<std::string>() +
...
class const_validator : public keyword_validator_base<Json> {
 ...
 public:
  const_validator(const Json& schema, const uri& schema_location,
                  const Json& sch)
      : keyword_validator_base<Json>("const", schema, schema_location),
        value_(sch) {}
 ...
 private:
  walk_result do_validate(const evaluation_context<Json>& context,
    ...
    if (value_ != instance) {
      ...
      walk_result result = reporter.error(validation_message(
          this->keyword_name(), this_context.eval_path(),
          this->schema_location(), instance_location, "Instance is not const"));
      if (result == walk_result::abort) {
        return result;
      }
    }
...
inline std::string to_string(json_schema_type type) {
  switch (type) {
...
class type_validator : public keyword_validator_base<Json> {
 ...
 public:
  ...
  type_validator(const Json& schema, const uri& schema_location,
      ...
      : keyword_validator_base<Json>("type", schema,
                                     std::move(schema_location)),
 ...
 private:
  walk_result do_validate(const evaluation_context<Json>& context,
                          ...
                          error_reporter& reporter,
                          Json& /*patch*/) const final {
    // std::cout << "type_validator.do_validate " <<
    // context.eval_path().string() << instance << "\n"; for (auto& type :
    // expected_types_ )
    //{
      ...
      switch (*it) {
        ...
        case json_schema_type::integer:
          if (instance.is_int64() || instance.is_uint64()) {
            is_type_found = true;
          } else if (instance.is_double() &&
                     static_cast<double>(instance.template as<int64_t>()) ==
                         instance.template as<double>()) {
            is_type_found = true;
    ...
    if (!is_type_found) {
      ...
      return reporter.error(validation_message(
          this->keyword_name(), this_context.eval_path(),
          this->schema_location(), instance_location, message));
    }
    return walk_result::advance;
  }
...
class properties_validator : public keyword_validator_base<Json> {
 ...
 public:
  ...
  properties_validator(
      ...
      : keyword_validator_base<Json>("properties", schema,
                                     std::move(schema_location)),
        properties_(std::move(properties)) {}

  walk_result validate(
      const evaluation_context<Json>& context, const Json& instance,
      const jsonpointer::json_pointer& instance_location,
      evaluation_results& results, error_reporter& reporter, Json& patch,
      std::unordered_set<std::string>& allowed_properties) const {
    // std::cout << "properties_validator begin[" <<
    ...
    for (const auto& prop : instance.object_range()) {
      ...
      if (prop_it != properties_.end()) {
        evaluation_context<Json> prop_context{this_context, prop.key(),
                                              evaluation_flags{}};
        jsonpointer::json_pointer prop_location =
            instance_location / prop.key();

        std::size_t errors = reporter.error_count();
        walk_result result =
            prop_it->second->validate(prop_context, prop.value(), prop_location,
                                      results, reporter, patch);
        if (result == walk_result::abort) {
          return result;
        }
    ...
    // Any property that doesn't match any of the property names in the
    // properties keyword is ignored by this keyword.
    ...
    for (auto const& prop : properties_) {
      ...
      if (finding == instance.object_range().end()) {
        ...
        if (default_value) {
          ...
          jsonpointer::json_pointer prop_location =
              instance_location / prop.first;

          update_patch(patch, prop_location, std::move(*default_value));
        }
      }
    }
    // std::cout << "properties_validator end[" << context.eval_path().string()
    // << "," << this->schema_location().string() << "]"; std::cout <<
    // "results:\n"; for (const auto& s : results)
    //{
    //     std::cout << "    " << s << "\n";
  ...
  walk_result walk(const evaluation_context<Json>& context,
                   const Json& instance,
                   const jsonpointer::json_pointer& instance_location,
                   const walk_reporter_type& reporter,
                   std::unordered_set<std::string>& allowed_properties) const {
    if (!instance.is_object()) {
      return walk_result::advance;
    }
    ...
    for (const auto& prop : instance.object_range()) {
      ...
      if (prop_it != properties_.end()) {
        jsonpointer::json_pointer prop_location =
            instance_location / prop.key();
        result = prop_it->second->walk(context, prop.value(), prop_location,
                                       reporter);
        allowed_properties.insert(prop.key());
        if (result == walk_result::abort) {
          return result;
 ...
 private:
  walk_result do_validate(const evaluation_context<Json>& context,
    ...
    return validate(context, instance, instance_location, results, reporter,
                    patch, allowed_properties);
  }
  ...
  walk_result do_walk(const evaluation_context<Json>& context,
                      ...
                      const walk_reporter_type& reporter) const final {
    std::unordered_set<std::string> allowed_properties;
    return walk(context, instance, instance_location, reporter,
                allowed_properties);
  }

  void update_patch(Json& patch,
                    const jsonpointer::json_pointer& instance_location,
                    Json&& default_value) const {
    Json j;
 ...
 public:
  pattern_properties_validator(
      ...
      std::vector<std::pair<std::regex, schema_validator_type>>&&
          pattern_properties)
      : keyword_validator_base<Json>("patternProperties", schema,
                                     std::move(schema_location)),
    ...
    for (const auto& prop : instance.object_range()) {
      ...
      for (auto& schema_pp : pattern_properties_) {
        if (std::regex_search(prop.key(), schema_pp.first)) {
          allowed_properties.insert(prop.key());
          std::size_t errors = reporter.error_count();
          walk_result result = schema_pp.second->validate(
              prop_context, prop.value(), prop_location, results, reporter,
              patch);
          if (result == walk_result::abort) {
            return result;
          }
  ...
  walk_result walk(const evaluation_context<Json>& context,
    ...
    for (const auto& prop : instance.object_range()) {
      ...
      for (auto& schema_pp : pattern_properties_) {
        if (std::regex_search(prop.key(), schema_pp.first)) {
          allowed_properties.insert(prop.key());
          result = schema_pp.second->walk(prop_context, prop.value(),
...
class additional_properties_validator : public keyword_validator_base<Json> {
 ...
 public:
  additional_properties_validator(
      ...
      : keyword_validator_base<Json>("additionalProperties", schema,
                                     std::move(schema_location)),
        properties_(std::move(properties)),
        pattern_properties_(std::move(pattern_properties)),
 ...
 private:
  walk_result do_validate(const evaluation_context<Json>& context,
    ...
    if (properties_) {
      walk_result result =
          properties_->validate(context, instance, instance_location, results,
                                reporter, patch, allowed_properties);
      if (result == walk_result::abort) {
        return result;
      }
    }
    ...
    if (pattern_properties_) {
      walk_result result = pattern_properties_->validate(
          context, instance, instance_location, results, reporter, patch,
          allowed_properties);
      if (result == walk_result::abort) {
        return result;
      }
      ...
      if (additional_properties_->always_fails()) {
        for (const auto& prop : instance.object_range()) {
          ...
          if (prop_it == allowed_properties.end()) {
            walk_result result = reporter.error(validation_message(
                this->keyword_name(), prop_context.eval_path(),
                additional_properties_->schema_location(), prop_location,
                "Additional property '" + prop.key() +
        ...
        for (const auto& prop : instance.object_range()) {
          ...
          if (prop_it == allowed_properties.end()) {
            ...
            // std::cout << "additional_properties_validator
            // a_prop_or_pattern_matched " << a_prop_or_pattern_matched << ", "
            // << bool(additional_properties_);

            // std::cout << " !!!additionalProperties!!!";
            collecting_error_listener local_reporter;

            walk_result result = additional_properties_->validate(
                prop_context, prop.value(), prop_location, results,
                local_reporter, patch);
            if (result == walk_result::abort) {
              return result;
            }
            if (!local_reporter.errors.empty()) {
              result = reporter.error(validation_message(
                  this->keyword_name(), this_context.eval_path(),
  ...
  walk_result do_walk(const evaluation_context<Json>& context,
    ...
    std::unordered_set<std::string> allowed_properties;
    if (properties_) {
      result = properties_->walk(context, instance, instance_location, reporter,
                                 allowed_properties);
      if (result == walk_result::abort) {
        return result;
      }
    ...
    if (pattern_properties_) {
      result = pattern_properties_->walk(context, instance, instance_location,
                                         reporter, allowed_properties);
      if (result == walk_result::abort) {
        return result;
      }
    }
    ...
    if (additional_properties_) {
      ...
      for (const auto& prop : instance.object_range()) {
        ...
        if (prop_it == allowed_properties.end()) {
          ...
          result = additional_properties_->walk(prop_context, prop.value(),
                                                prop_location, reporter);
          if (result == walk_result::abort) {
            return result;
          }
...
class dependent_required_validator : public keyword_validator_base<Json> {
 ...
 public:
  dependent_required_validator(
      ...
      : keyword_validator_base<Json>("dependentRequired", schema,
                                     std::move(schema_location)),
 ...
 private:
  walk_result do_validate(const evaluation_context<Json>& context,
    ...
    for (const auto& dep : dependent_required_) {
      ...
      if (prop != instance.object_range().end()) {
        ...
        walk_result result =
            dep.second->validate(this_context, instance, prop_location, results,
                                 reporter, patch);  // validate
        if (result == walk_result::abort) {
          return result;
        }
      }
  ...
  walk_result do_walk(const evaluation_context<Json>& context,
    ...
    for (const auto& dep : dependent_required_) {
      ...
      if (prop != instance.object_range().end()) {
        // if dependency-prop is present in instance
        result = dep.second->walk(this_context, instance,
                                  instance_location / dep.first, reporter);
        if (result == walk_result::abort) {
          return result;
        }
      }
...
class dependent_schemas_validator : public keyword_validator_base<Json> {
 ...
 public:
  dependent_schemas_validator(
      const Json& schema, const uri& schema_location,
      std::map<std::string, schema_validator_type>&& dependent_schemas)
      : keyword_validator_base<Json>("dependentSchemas", schema,
                                     std::move(schema_location)),
...
class property_names_validator : public keyword_validator_base<Json> {
 ...
 private:
    ...
    if (instance.size() > 0 && schema_val_) {
      if (schema_val_->always_fails()) {
        ...
        return reporter.error(
            validation_message(this->keyword_name(), this_context.eval_path(),
                               this->schema_location(), item_location,
                               "Instance has properties but the schema does "
                               "not allow any property names."));
      } else if (schema_val_->always_succeeds()) {
        return walk_result::advance;
      } else {
        for (const auto& prop : instance.object_range()) {
          ...
          walk_result result =
              schema_val_->validate(this_context, prop.key(), instance_location,
                                    results, reporter, patch);
          if (result == walk_result::abort) {
            return result;
          }
  ...
  walk_result do_walk(const evaluation_context<Json>& context,
    ...
    if (instance.size() > 0 && schema_val_) {
      for (const auto& prop : instance.object_range()) {
        result = schema_val_->walk(this_context, prop.key(),
                                   instance_location / prop.key(), reporter);
        if (result == walk_result::abort) {
          return result;
        }
      }
...
class dependencies_validator : public keyword_validator_base<Json> {
 ...
 public:
  dependencies_validator(
      ...
      std::map<std::string, keyword_validator_type>&& dependent_required,
      std::map<std::string, schema_validator_type>&& dependent_schemas)
      : keyword_validator_base<Json>("dependencies", schema,
                                     std::move(schema_location)),
        dependent_required_(std::move(dependent_required)),
        dependent_schemas_(std::move(dependent_schemas)) {}
...
class max_contains_keyword : public keyword_base<Json> {
 ...
 public:
  max_contains_keyword(const Json& schema, const uri& schema_location,
                       std::size_t max_value)
      : keyword_base<Json>("maxContains", schema, schema_location),
        max_value_(max_value) {}

  walk_result validate(const evaluation_context<Json>& context,
                       const jsonpointer::json_pointer& instance_location,
                       std::size_t count, error_reporter& reporter) const {
    evaluation_context<Json> this_context(context, this->keyword_name());

    if (count > max_value_) {
      std::string message("A schema can match a contains constraint at most " +
                          std::to_string(max_value_) + " times");
  ...
  walk_result walk(const evaluation_context<Json>& /*context*/,
                   const Json& instance,
                   const jsonpointer::json_pointer& instance_location,
                   const walk_reporter_type& reporter) const {
    return reporter(this->keyword_name(), this->schema(),
                    this->schema_location(), instance, instance_location);
  }
};
...
class min_contains_keyword : public keyword_base<Json> {
 ...
 public:
  min_contains_keyword(const Json& schema, const uri& schema_location,
                       std::size_t min_value)
      : keyword_base<Json>("minContains", schema, schema_location),
  ...
  walk_result validate(const evaluation_context<Json>& context,
    ...
    if (count < min_value_) {
      std::string message(
          "A schema must match a contains constraint at least " +
          std::to_string(min_value_) + " times");
      message.append(" but it matched " + std::to_string(count) + " times.");
      walk_result result = reporter.error(validation_message(
          this->keyword_name(), this_context.eval_path(),
          this->schema_location(), instance_location, std::move(message)));
...
class contains_validator : public keyword_validator_base<Json> {
 ...
 public:
  contains_validator(const Json& schema, const uri& schema_location,
      ...
      : keyword_validator_base<Json>("contains", std::addressof(schema),
                                     std::move(schema_location)),
  ...
  contains_validator(const Json& schema, const uri& schema_location,
      ...
      : keyword_validator_base<Json>("contains", schema,
                                     std::move(schema_location)),
        schema_validator_(std::move(schema_validator)),
 ...
 private:
  walk_result do_validate(const evaluation_context<Json>& context,
    ...
    for (const auto& item : instance.array_range()) {
      std::size_t errors = local_reporter.errors.size();
      walk_result result = schema_validator_->validate(
          this_context, item, instance_location / index, results,
          local_reporter, patch);
      if (result == walk_result::abort) {
        return result;
      }
    ...
    if (max_contains_ || min_contains_) {
      if (max_contains_) {
        walk_result result = max_contains_->validate(
            this_context, instance_location, contains_count, reporter);
        if (result == walk_result::abort) {
          return result;
        }
      }
      if (min_contains_) {
        walk_result result = min_contains_->validate(
            this_context, instance_location, contains_count, reporter);
        if (result == walk_result::abort) {
          return result;
        }
  ...
  walk_result do_walk(const evaluation_context<Json>& context,
                      const Json& instance,
                      const jsonpointer::json_pointer& instance_location,
                      const walk_reporter_type& reporter) const override {
    if (!instance.is_array()) {
      return walk_result::advance;
    }
    ...
    for (std::size_t index = 0; index < instance.size(); ++index) {
      result = schema_validator_->walk(this_context, instance.at(index),
                                       instance_location / index, reporter);
      if (result == walk_result::abort) {
        return result;
      }
    }
    ...
    if (max_contains_) {
      result = max_contains_->walk(this_context, instance, instance_location,
                                   reporter);
      if (result == walk_result::abort) {
        return result;
      }
    ...
    if (min_contains_) {
      result = min_contains_->walk(this_context, instance, instance_location,
                                   reporter);
      if (result == walk_result::abort) {
        return result;
      }
...
class items_keyword : public keyword_base<Json> {
 ...
 public:
  items_keyword(const std::string& keyword_name, const Json& schema,
                const uri& schema_location, schema_validator_type&& items_val)
  ...
  walk_result validate(const evaluation_context<Json>& context,
                       ...
                       const jsonpointer::json_pointer& instance_location,
                       evaluation_results& results, error_reporter& reporter,
                       Json& patch, std::size_t data_index) const {
    if (!instance.is_array()) {
      return walk_result::advance;
    }
    if (data_index < instance.size() && items_val_) {
      ...
      if (items_val_->always_fails()) {
        jsonpointer::json_pointer item_location =
            instance_location / data_index;
        walk_result result = reporter.error(validation_message(
            this->keyword_name(), items_context.eval_path(),
            this->schema_location(), item_location,
            "Extra item at index '" + std::to_string(data_index) +
                "' but the schema does not allow extra items."));
        if (result == walk_result::abort) {
          return result;
        }
        ...
        for (; data_index < instance.size(); ++data_index) {
          ...
          std::size_t errors = reporter.error_count();
          walk_result result =
              items_val_->validate(items_context, instance[data_index],
                                   item_location, results, reporter, patch);
          if (result == walk_result::abort) {
            return result;
          }
          if (errors == reporter.error_count()) {
  ...
  walk_result walk(const evaluation_context<Json>& context,
    ...
    if (data_index < instance.size() && items_val_) {
      ...
      for (; data_index < instance.size(); ++data_index) {
        jsonpointer::json_pointer item_location =
            instance_location / data_index;
        result = items_val_->walk(items_context, instance[data_index],
                                  item_location, reporter);
        if (result == walk_result::abort) {
          return result;
        }
      }
...
class prefix_items_validator : public keyword_validator_base<Json> {
 ...
 public:
  prefix_items_validator(
      const std::string& keyword_name, const Json& schema,
      const uri& schema_location,
 ...
 private:
  walk_result do_validate(const evaluation_context<Json>& context,
    ...
    evaluation_context<Json> prefix_items_context(context,
                                                  this->keyword_name());

    size_t start = 0;
    size_t end = 0;
    for (std::size_t schema_index = 0;
         schema_index < prefix_item_validators_.size() &&
         data_index < instance.size();
         ++schema_index, ++data_index) {
      auto& val = prefix_item_validators_[schema_index];
      evaluation_context<Json> item_context{prefix_items_context, schema_index,
                                            evaluation_flags{}};
      jsonpointer::json_pointer item_location = instance_location / data_index;
      std::size_t errors = reporter.error_count();
      walk_result result =
          val->validate(item_context, instance[data_index], item_location,
                        results, reporter, patch);
      if (result == walk_result::abort) {
        return result;
      }
    ...
    if (data_index < instance.size() && items_val_) {
      walk_result result =
          items_val_->validate(context, instance, instance_location, results,
                               reporter, patch, data_index);
      if (result == walk_result::abort) {
        return result;
      }
    }
  ...
  walk_result do_walk(const evaluation_context<Json>& context,
    ...
    for (std::size_t schema_index = 0;
      ...
      evaluation_context<Json> item_context{prefix_items_context, schema_index,
                                            evaluation_flags{}};
      jsonpointer::json_pointer item_location = instance_location / data_index;
      result = val->walk(item_context, instance[data_index], item_location,
                         reporter);
      if (result == walk_result::abort) {
        return result;
      }
    ...
    if (data_index < instance.size() && items_val_) {
      items_val_->walk(context, instance, instance_location, reporter,
                       data_index);
    }
    return walk_result::advance;
  }
...
class unevaluated_properties_validator : public keyword_validator_base<Json> {
 ...
 public:
  unevaluated_properties_validator(const Json& schema,
                                   const uri& schema_location,
                                   schema_validator_type&& schema_val)
      : keyword_validator_base<Json>("unevaluatedProperties", schema,
                                     std::move(schema_location)),
        schema_val_(std::move(schema_val)) {}
 ...
 private:
  walk_result do_validate(const evaluation_context<Json>& context,
                          ...
                          evaluation_results& results, error_reporter& reporter,
                          Json& patch) const final {
    // std::cout << "unevaluated_properties_validator [" <<
    // context.eval_path().string() << "," << this->schema_location().string()
    // << "]"; std::cout << "results:\n"; for (const auto& s :
    // results.evaluated_properties_)
    //{
    //     std::cout << "    " << s << "\n";
    // }
    // std::cout << "\n";
      ...
      if (schema_val_->always_fails()) {
        for (const auto& prop : instance.object_range()) {
          ...
          if (prop_it == results.evaluated_properties.end()) {
            ...
            walk_result result = reporter.error(validation_message(
                this->keyword_name(), prop_context.eval_path(),
                this->schema_location(), prop_location,
                "Unevaluated property '" + prop.key() +
                    "' but the schema does not allow unevaluated properties."));
            if (result == walk_result::abort) {
              return result;
            }
            break;
        ...
        for (const auto& prop : instance.object_range()) {
          ...
          if (prop_it == results.evaluated_properties.end()) {
            ...
            walk_result result = schema_val_->validate(
                this_context, prop.value(), instance_location, results,
                reporter, patch);
            if (result == walk_result::abort) {
              return result;
            }
      ...
      if (schema_val_->always_fails()) {
        for (std::size_t index = 0; index < instance.size(); ++index) {
          ...
          if (!results.evaluated_items.contains(index)) {
            ...
            // "\n";
            walk_result result = reporter.error(validation_message(
                this->keyword_name(), item_context.eval_path(),
                this->schema_location(), item_location,
                "Unevaluated item at index '" + std::to_string(index) +
                    "' but the schema does not allow unevaluated items."));
            if (result == walk_result::abort) {
              return result;
            }
            break;
        ...
        for (const auto& item : instance.array_range()) {
          ...
          if (!results.evaluated_items.contains(index)) {
            ...
            std::size_t error_count = reporter.error_count();
            walk_result result = schema_val_->validate(
                item_context, item, item_location, results, reporter, patch);
            if (result == walk_result::abort) {
              return result;
            }
            if (reporter.error_count() == error_count) {

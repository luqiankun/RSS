...
class schema_builder {
 public:
  ...
  using schema_builder_factory_type =
      std::function<std::unique_ptr<schema_builder<Json>>(
          const Json&, const evaluation_options&, schema_store_type*,
          const std::vector<schema_resolver<Json>>&,
 ...
 public:
  schema_builder(const std::string& version, Json&& root_schema,
                 const schema_builder_factory_type& builder_factory,
                 evaluation_options options,
                 schema_store_type* schema_store_ptr,
                 const std::vector<schema_resolver<Json>>& resolvers)
      : spec_version_(version),
  ...
  const std::string& schema() const { return spec_version_; }

  void build_schema() {
    anchor_uri_map_type anchor_dict;

    root_ = make_schema_validator(compilation_context{}, *root_schema_, {},
                                  anchor_dict);
  }
  ...
  void build_schema(const std::string& retrieval_uri) {
    anchor_uri_map_type anchor_dict;
    root_ =
        make_schema_validator(compilation_context(uri_wrapper(retrieval_uri)),
                              *root_schema_, {}, anchor_dict);
  }

  evaluation_options options() const { return options_; }

  schema_validator_type make_boolean_schema(const compilation_context& context,
                                            const Json& sch) {
  ...
  std::unique_ptr<document_schema_validator<Json>> get_schema_validator() {
    ...
    for (std::size_t i = 0; i < unresolved_refs_.size(); ++i) {
      ...
      if (schema_store_ptr_->find(loc) ==
          schema_store_ptr_->end())  // registry for this file is empty
      {
        bool found = false;
        for (auto it = resolvers_.begin(); it != resolvers_.end() && !found;
             ++it) {
          Json external_sch = (*it)(loc.base());

          if (external_sch.is_object() || external_sch.is_bool()) {
            anchor_uri_map_type anchor_dict2;
            this->save_schema(make_cross_draft_schema_validator(
                compilation_context(uri_wrapper(loc.base())),
        ...
        if (found) {
          ...
          if (schema_store_ptr_->find(loc) == schema_store_ptr_->end()) {
            JSONCONS_THROW(jsonschema::schema_error("Unresolved reference '" +
                                                    loc.string() + "'"));
          }
        } else {
          JSONCONS_THROW(
              jsonschema::schema_error("Don't know how to load JSON Schema '" +
                                       loc.base().string() + "'"));
        }
      }
    }
    ...
    return jsoncons::make_unique<document_schema_validator<Json>>(
        std::move(root_schema_), std::move(root_),
        std::move(schema_validators_));
  }
  ...
  void resolve_references() {
    for (auto& ref : unresolved_refs_) {
      ...
      if (it == schema_store_ptr_->end()) {
        JSONCONS_THROW(
            schema_error("Undefined reference " + ref.first.string()));
      }
      if (it->second == nullptr) {
        JSONCONS_THROW(
            schema_error("Null referred schema " + ref.first.string()));
      }
  ...
  void insert_unknown_keyword(const uri_wrapper& uri, const std::string& key,
                              const Json& value) {
    ...
    if (new_uri.has_fragment() && !new_uri.has_plain_name_fragment()) {
      ...
      auto unresolved_refs = std::find_if(
          this->unresolved_refs_.begin(), this->unresolved_refs_.end(),
          [new_uri](const std::pair<jsoncons::uri, ref<Json>*>& pr) {
            return pr.first == new_uri.uri();
          });
      if (unresolved_refs != this->unresolved_refs_.end()) {
        anchor_uri_map_type anchor_dict2;
        this->save_schema(make_cross_draft_schema_validator(
            compilation_context(new_uri), value, {}, anchor_dict2));
  ...
  std::unique_ptr<ref_validator<Json>> get_or_create_reference(
      const Json& schema, const uri_wrapper& identifier) {
    // a schema already exists
    auto it = this->schema_store_ptr_->find(identifier.uri());
    if (it != this->schema_store_ptr_->end()) {
      return jsoncons::make_unique<ref_validator_type>(schema, identifier.uri(),
                                                       it->second);
    }
    ...
    if (identifier.has_fragment() && !identifier.has_plain_name_fragment()) {
      ...
      if (it2 != this->unknown_keywords_.end()) {
        ...
        anchor_uri_map_type anchor_dict2;
        auto s = make_cross_draft_schema_validator(
            compilation_context(identifier), subsch, {}, anchor_dict2);
        this->unknown_keywords_.erase(it2);
        auto orig = jsoncons::make_unique<ref_validator_type>(
            schema, identifier.uri(), s.get());
    ...
    auto orig =
        jsoncons::make_unique<ref_validator_type>(schema, identifier.uri());
  ...
  virtual compilation_context make_compilation_context(
      const compilation_context& parent, const Json& sch,
      jsoncons::span<const std::string> keys) const = 0;

  virtual schema_validator_type make_schema_validator(
      const compilation_context& context, const Json& sch,
      jsoncons::span<const std::string> keys,
      anchor_uri_map_type& anchor_dict) = 0;

  schema_validator_type make_cross_draft_schema_validator(
      const compilation_context& context, const Json& sch,
      jsoncons::span<const std::string> keys,
      anchor_uri_map_type& anchor_dict) {
      ...
      case json_type::object_value: {
        ...
        if (it != std::move(sch).object_range().end()) {
          if (it->value().as_string_view() == schema()) {
            return make_schema_validator(context, std::move(sch), keys,
                                         anchor_dict);
          } else {
            auto schema_builder =
                builder_factory_(std::move(sch), options_, schema_store_ptr_,
                                 resolvers_, vocabulary_);
  ...
  std::unique_ptr<properties_validator<Json>> make_properties_validator(
    ...
    for (const auto& prop : sch.object_range()) {
      ...
      properties.emplace(std::make_pair(
          prop.key(), this->make_cross_draft_schema_validator(
                          context, prop.value(), sub_keys, anchor_dict)));
    }

    return jsoncons::make_unique<properties_validator<Json>>(
        parent, std::move(schema_location), std::move(properties));
  }

  virtual std::unique_ptr<max_length_validator<Json>> make_max_length_validator(
      const compilation_context& context, const Json& sch, const Json& parent) {
    ...
    return jsoncons::make_unique<max_length_validator<Json>>(
        parent, schema_location, value);
  }

  virtual std::unique_ptr<min_length_validator<Json>> make_min_length_validator(
      const compilation_context& context, const Json& sch, const Json& parent) {
    ...
    auto value = sch.template as<std::size_t>();
    return jsoncons::make_unique<min_length_validator<Json>>(
        parent, schema_location, value);
  }

  virtual std::unique_ptr<not_validator<Json>> make_not_validator(
      const compilation_context& context, const Json& sch, const Json& parent,
      anchor_uri_map_type& anchor_dict) {
    uri schema_location = context.make_schema_location("not");
    std::string not_key[] = {"not"};
    return jsoncons::make_unique<not_validator<Json>>(
        parent, schema_location,
        make_cross_draft_schema_validator(context, sch, not_key, anchor_dict));
  }

  virtual std::unique_ptr<const_validator<Json>> make_const_validator(
      const compilation_context& context, const Json& sch, const Json& parent) {
  ...
  virtual std::unique_ptr<enum_validator<Json>> make_enum_validator(
      const compilation_context& context, const Json& sch, const Json& parent) {
    uri schema_location = context.make_schema_location("enum");
    return jsoncons::make_unique<enum_validator<Json>>(parent, schema_location,
                                                       sch);
  }

  virtual std::unique_ptr<required_validator<Json>> make_required_validator(
      const compilation_context& context, const Json& sch, const Json& parent) {
    uri schema_location = context.make_schema_location("required");
    return jsoncons::make_unique<required_validator<Json>>(
        parent, schema_location, sch.template as<std::vector<std::string>>());
  }

  virtual std::unique_ptr<maximum_validator<Json>> make_maximum_validator(
      const compilation_context& context, const Json& sch, const Json& parent) {
    ...
    return jsoncons::make_unique<maximum_validator<Json>>(parent,
                                                          schema_location, sch);
  }

  virtual std::unique_ptr<exclusive_maximum_validator<Json>>
  make_exclusive_maximum_validator(const compilation_context& context,
    ...
    return jsoncons::make_unique<exclusive_maximum_validator<Json>>(
        parent, schema_location, sch);
  }

  virtual std::unique_ptr<keyword_validator<Json>> make_minimum_validator(
      const compilation_context& context, const Json& sch, const Json& parent) {
    ...
    return jsoncons::make_unique<minimum_validator<Json>>(parent,
                                                          schema_location, sch);
  }

  virtual std::unique_ptr<exclusive_minimum_validator<Json>>
  make_exclusive_minimum_validator(const compilation_context& context,
                                   const Json& sch, const Json& parent) {
    ...
    return jsoncons::make_unique<exclusive_minimum_validator<Json>>(
        parent, schema_location, sch);
  }

  virtual std::unique_ptr<multiple_of_validator<Json>>
  make_multiple_of_validator(const compilation_context& context,
                             const Json& sch, const Json& parent) {
    ...
    return jsoncons::make_unique<multiple_of_validator<Json>>(
        parent, schema_location, value);
  }

  virtual std::unique_ptr<type_validator<Json>> make_type_validator(
      const compilation_context& context, const Json& sch, const Json& parent) {
    ...
    return jsoncons::make_unique<type_validator<Json>>(
        parent, std::move(schema_location), std::move(expected_types));
  }

  virtual std::unique_ptr<content_encoding_validator<Json>>
  make_content_encoding_validator(const compilation_context& context,
                                  const Json& sch, const Json& parent) {
    ...
    return jsoncons::make_unique<content_encoding_validator<Json>>(
        parent, schema_location, value);
  }

  virtual std::unique_ptr<content_media_type_validator<Json>>
  make_content_media_type_validator(const compilation_context& context,
                                    const Json& sch, const Json& parent) {
    ...
    auto value = sch.template as<std::string>();
    return jsoncons::make_unique<content_media_type_validator<Json>>(
        parent, schema_location, value, content_encoding);
  }

  virtual std::unique_ptr<format_validator<Json>> make_format_validator(
      const compilation_context& context, const Json& sch, const Json& parent) {
    ...
    return jsoncons::make_unique<format_validator<Json>>(
        parent, schema_location, format_check);
  }

  virtual std::unique_ptr<pattern_validator<Json>> make_pattern_validator(
      const compilation_context& context, const Json& sch, const Json& parent) {
    ...
    return jsoncons::make_unique<pattern_validator<Json>>(
        parent, schema_location, pattern_string, regex);
  }

  virtual std::unique_ptr<max_items_validator<Json>> make_max_items_validator(
      const compilation_context& context, const Json& sch, const Json& parent) {
    ...
    auto value = sch.template as<std::size_t>();
    return jsoncons::make_unique<max_items_validator<Json>>(
        parent, schema_location, value);
  }

  virtual std::unique_ptr<min_items_validator<Json>> make_min_items_validator(
      const compilation_context& context, const Json& sch, const Json& parent) {
    ...
    auto value = sch.template as<std::size_t>();
    return jsoncons::make_unique<min_items_validator<Json>>(
        parent, schema_location, value);
  }

  virtual std::unique_ptr<max_properties_validator<Json>>
  make_max_properties_validator(const compilation_context& context,
                                const Json& sch, const Json& parent) {
    ...
    auto value = sch.template as<std::size_t>();
    return jsoncons::make_unique<max_properties_validator<Json>>(
        parent, schema_location, value);
  }

  virtual std::unique_ptr<min_properties_validator<Json>>
  make_min_properties_validator(const compilation_context& context,
                                const Json& sch, const Json& parent) {
    ...
    auto value = sch.template as<std::size_t>();
    return jsoncons::make_unique<min_properties_validator<Json>>(
        parent, schema_location, value);
  }

  virtual std::unique_ptr<contains_validator<Json>> make_contains_validator(
      const compilation_context& context, const Json& sch, const Json& parent,
      anchor_uri_map_type& anchor_dict) {
    ...
    if (it != parent.object_range().end()) {
      ...
      max_contains = jsoncons::make_unique<max_contains_keyword<Json>>(
          parent, path, value);
    } else {
      uri path = context.make_schema_location("maxContains");
      max_contains = jsoncons::make_unique<max_contains_keyword<Json>>(
          parent, path, (std::numeric_limits<std::size_t>::max)());
    }
    ...
    if (it != parent.object_range().end()) {
      ...
      auto value = it->value().template as<std::size_t>();
      min_contains = jsoncons::make_unique<min_contains_keyword<Json>>(
          parent, path, value);
    } else {
      uri path = context.make_schema_location("minContains");
      min_contains =
          jsoncons::make_unique<min_contains_keyword<Json>>(parent, path, 1);
    }

    return jsoncons::make_unique<contains_validator<Json>>(
        parent, schema_location,
        make_cross_draft_schema_validator(context, sch, sub_keys, anchor_dict),
        std::move(max_contains), std::move(min_contains));
  }

  virtual std::unique_ptr<unique_items_validator<Json>>
  make_unique_items_validator(const compilation_context& context,
                              const Json& sch, const Json& parent) {
    uri schema_location = context.make_schema_location("uniqueItems");
    bool are_unique = sch.template as<bool>();
    return jsoncons::make_unique<unique_items_validator<Json>>(
        parent, schema_location, are_unique);
  }

  virtual std::unique_ptr<all_of_validator<Json>> make_all_of_validator(
      const compilation_context& context, const Json& sch, const Json& parent,
      anchor_uri_map_type& anchor_dict) {
    ...
    for (const auto& subsch : sch.array_range()) {
      ...
      subschemas.emplace_back(make_cross_draft_schema_validator(
          context, subsch, sub_keys, anchor_dict));
    }
    return jsoncons::make_unique<all_of_validator<Json>>(
        parent, std::move(schema_location), std::move(subschemas));
  }

  virtual std::unique_ptr<any_of_validator<Json>> make_any_of_validator(
      const compilation_context& context, const Json& sch, const Json& parent,
      anchor_uri_map_type& anchor_dict) {
    ...
    return jsoncons::make_unique<any_of_validator<Json>>(
        parent, std::move(schema_location), std::move(subschemas));
  }

  virtual std::unique_ptr<one_of_validator<Json>> make_one_of_validator(
      const compilation_context& context, const Json& sch, const Json& parent,
      anchor_uri_map_type& anchor_dict) {
    ...
    return jsoncons::make_unique<one_of_validator<Json>>(
        parent, std::move(schema_location), std::move(subschemas));
  }

  virtual std::unique_ptr<dependencies_validator<Json>>
  make_dependencies_validator(const compilation_context& context,
                              const Json& sch, const Json& parent,
                              anchor_uri_map_type& anchor_dict) {
    uri schema_location = context.get_base_uri();
    ...
    for (const auto& dep : sch.object_range()) {
      switch (dep.value().type()) {
        case json_type::array_value: {
          ...
          dependent_required.emplace(
              dep.key(), this->make_required_validator(
                             compilation_context(std::vector<uri_wrapper>{
                                 {uri_wrapper{location}}}),
                             dep.value(), sch));
          break;
        }
        case json_type::bool_value:
        case json_type::object_value: {
          ...
          dependent_schemas.emplace(
              dep.key(), make_cross_draft_schema_validator(
                             context, dep.value(), sub_keys, anchor_dict));
          break;
        }
        default: {
          break;
    ...
    return jsoncons::make_unique<dependencies_validator<Json>>(
        parent, std::move(schema_location), std::move(dependent_required),
        std::move(dependent_schemas));
  }

  virtual std::unique_ptr<property_names_validator<Json>>
  make_property_names_validator(const compilation_context& context,
                                const Json& sch, const Json& parent,
                                anchor_uri_map_type& anchor_dict) {
    uri schema_location = context.get_base_uri();
    ...
    property_names_schema_validator =
        make_cross_draft_schema_validator(context, sch, sub_keys, anchor_dict);

    return jsoncons::make_unique<property_names_validator<Json>>(
        parent, std::move(schema_location),
        std::move(property_names_schema_validator));
  }

  // 201909 and later

  virtual std::unique_ptr<dependent_required_validator<Json>>
  make_dependent_required_validator(const compilation_context& context,
                                    const Json& sch, const Json& parent) {
    uri schema_location = context.get_base_uri();
    std::map<std::string, keyword_validator_type> dependent_required;
    ...
    return jsoncons::make_unique<dependent_required_validator<Json>>(
        parent, std::move(schema_location), std::move(dependent_required));
  }

  virtual std::unique_ptr<dependent_schemas_validator<Json>>
  make_dependent_schemas_validator(const compilation_context& context,
                                   const Json& sch, const Json& parent,
                                   anchor_uri_map_type& anchor_dict) {
    uri schema_location = context.get_base_uri();
    std::map<std::string, schema_validator_type> dependent_schemas;
    ...
    return jsoncons::make_unique<dependent_schemas_validator<Json>>(
        parent, std::move(schema_location), std::move(dependent_schemas));
  }

  std::unique_ptr<prefix_items_validator<Json>> make_prefix_items_validator_07(
      const compilation_context& context, const Json& sch, const Json& parent,
      anchor_uri_map_type& anchor_dict) {
    ...
    if (sch.type() == json_type::array_value) {
      ...
      for (const auto& subsch : sch.array_range()) {
        ...
        prefix_item_validators.emplace_back(
            this->make_cross_draft_schema_validator(context, subsch, sub_keys,
                                                    anchor_dict));
      }
      ...
      if (it != parent.object_range().end()) {
        ...
        items_val = jsoncons::make_unique<items_keyword<Json>>(
            "additionalItems", parent, items_location,
            this->make_cross_draft_schema_validator(context, it->value(),
                                                    sub_keys, anchor_dict));
      }
    }

    return jsoncons::make_unique<prefix_items_validator<Json>>(
        "items", parent, schema_location, std::move(prefix_item_validators),
        std::move(items_val));
  }

  std::unique_ptr<items_validator<Json>> make_items_validator(
      const std::string& keyword_name, const compilation_context& context,
    ...
    return jsoncons::make_unique<items_validator<Json>>(
        keyword_name, parent, schema_location,
        this->make_cross_draft_schema_validator(context, sch, sub_keys,
                                                anchor_dict));
  }

  virtual std::unique_ptr<unevaluated_properties_validator<Json>>
  make_unevaluated_properties_validator(const compilation_context& context,
                                        const Json& sch, const Json& parent,
                                        anchor_uri_map_type& anchor_dict) {
    uri schema_location = context.get_base_uri();
    ...
    return jsoncons::make_unique<unevaluated_properties_validator<Json>>(
        parent, std::move(schema_location),
        make_cross_draft_schema_validator(context, sch, sub_keys, anchor_dict));
  }

  virtual std::unique_ptr<unevaluated_items_validator<Json>>
  make_unevaluated_items_validator(const compilation_context& context,
                                   const Json& sch, const Json& parent,
                                   anchor_uri_map_type& anchor_dict) {
    uri schema_location = context.get_base_uri();
    ...
    return jsoncons::make_unique<unevaluated_items_validator<Json>>(
        parent, std::move(schema_location),
        make_cross_draft_schema_validator(context, sch, sub_keys, anchor_dict));
  }
  ...
  make_additional_properties_validator(
    ...
    additional_properties = this->make_cross_draft_schema_validator(
        context, sch, sub_keys, anchor_dict);

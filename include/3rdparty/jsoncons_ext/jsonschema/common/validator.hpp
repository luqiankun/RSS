...
struct json_schema_traits {
  using walk_reporter_type = std::function<walk_result(
      ...
      const uri& schema_location, const Json& instance,
...
class range_collection {
 ...
 public:
  ...
  range operator[](std::size_t index) const { return ranges_[index]; }

  const_iterator begin() const { return ranges_.cbegin(); }

  const_iterator end() const { return ranges_.cend(); }

  void insert(range index_range) { ranges_.push_back(index_range); }
...
class validator_base {
 public:
  ...
  walk_result validate(const evaluation_context<Json>& context,
                       ...
                       Json& patch) const {
    return do_validate(context, instance, instance_location, results, reporter,
                       patch);
  }
 ...
 private:
  virtual walk_result do_validate(
      const evaluation_context<Json>& context, const Json& instance,
      const jsonpointer::json_pointer& instance_location,
      evaluation_results& results, error_reporter& reporter,
      Json& patch) const = 0;

  virtual walk_result do_walk(
      const evaluation_context<Json>& /*context*/, const Json& /*instance*/,
      const jsonpointer::json_pointer& /*instance_location*/,
...
class keyword_validator : public validator_base<Json> {
 public:
  ...
  virtual bool always_fails() const { return false; }
...
class keyword_validator_base : public keyword_validator<Json> {
 ...
 public:
  ...
  keyword_validator_base(const std::string& keyword_name, const Json& schema,
                         const uri& schema_location)
      : keyword_name_(keyword_name),
        schema_ptr_(std::addressof(schema)),
  ...
  const std::string& keyword_name() const final { return keyword_name_; }
...
class ref_validator : public keyword_validator_base<Json>,
 ...
 private:
  walk_result do_validate(const evaluation_context<Json>& context,
    ...
    return referred_schema_->validate(this_context, instance, instance_location,
  ...
  walk_result do_walk(const evaluation_context<Json>& context,
    ...
    evaluation_context<Json> this_context(context, this->keyword_name());
    return referred_schema_->walk(this_context, instance, instance_location,
                                  reporter);
  }
};
...
class keyword_base {
 ...
 public:
  keyword_base(const std::string& keyword_name, const Json& schema,
               const uri& schema_location)
      : keyword_name_(keyword_name),
        schema_ptr_(std::addressof(schema)),
        schema_location_(schema_location) {}
  ...
  const std::string& keyword_name() const { return keyword_name_; }

  const Json& schema() const { return *schema_ptr_; }

  const uri& schema_location() const { return schema_location_; }
...
class schema_validator : public validator_base<Json> {
 ...
 public:
  ...
  virtual const schema_validator<Json>* get_schema_for_dynamic_anchor(
      const std::string& anchor) const = 0;

...
enum class evaluation_flags : uint32_t {
  require_evaluated_properties = 1,
  require_evaluated_items = 2
};
...
class evaluation_context {
 ...
 public:
  evaluation_context() : flags_{} {}

  evaluation_context(const evaluation_context& other)
      : dynamic_scope_{other.dynamic_scope_},
  ...
  evaluation_context(const evaluation_context& parent,
                     const schema_validator<Json>* validator)
      : dynamic_scope_{parent.dynamic_scope_},
        eval_path_{parent.eval_path_},
  ...
  evaluation_flags eval_flags() const { return flags_; }

  bool require_evaluated_properties() const {
    return (flags_ & evaluation_flags::require_evaluated_properties) ==
           evaluation_flags::require_evaluated_properties;
  }

  bool require_evaluated_items() const {
    return (flags_ & evaluation_flags::require_evaluated_items) ==

...
class document_schema_validator : public schema_validator<Json> {
 ...
 public:
  document_schema_validator(std::unique_ptr<Json>&& root_schema,
                            schema_validator_type&& schema_val,
                            std::vector<schema_validator_type>&& schemas)
      : root_schema_(std::move(root_schema)),
        schema_val_(std::move(schema_val)),
        schemas_(std::move(schemas)) {
    if (schema_val_ == nullptr)
      JSONCONS_THROW(
          schema_error("There is no schema to validate an instance against"));
  }
  ...
  document_schema_validator& operator=(const document_schema_validator&) =
      delete;
  ...
  const schema_validator<Json>* get_schema_for_dynamic_anchor(
      const std::string& anchor) const final {
    return schema_val_->get_schema_for_dynamic_anchor(anchor);
  }

  bool always_fails() const final { return schema_val_->always_fails(); }

  bool always_succeeds() const final { return schema_val_->always_succeeds(); }
 ...
 private:
  walk_result do_validate(const evaluation_context<Json>& context,
    ...
    return schema_val_->validate(context, instance, instance_location, results,
                                 reporter, patch);
  }
...
class boolean_schema_validator : public schema_validator<Json> {
 ...
 public:
  ...
  bool recursive_anchor() const final { return false; }

  const jsoncons::optional<jsoncons::uri>& id() const final { return id_; }
  ...
  bool always_fails() const final { return !value_; }

  bool always_succeeds() const final { return value_; }
 ...
 private:
  walk_result do_validate(const evaluation_context<Json>& context, const Json&,
    ...
    if (!value_) {
      reporter.error(validation_message(
          "false", context.eval_path(), this->schema_location(),
          instance_location, "False schema always fails"));
    }
 ...
 public:
  ...
  object_schema_validator(const uri& schema_location,
      ...
      : schema_location_(schema_location),
        ...
        always_succeeds_(false),
        always_fails_(false) {
    init();
  }
  object_schema_validator(
      const uri& schema_location, const jsoncons::optional<jsoncons::uri>& id,
      std::vector<keyword_validator_type>&& validators,
      std::unique_ptr<unevaluated_properties_validator<Json>>&&
          unevaluated_properties_val,
      std::unique_ptr<unevaluated_items_validator<Json>>&&
          unevaluated_items_val,
      std::map<std::string, schema_validator_type>&& defs, Json&& default_value,
  ...
  const uri& schema_location() const override { return schema_location_; }

  bool recursive_anchor() const final { return recursive_anchor_; }
  ...
  bool always_fails() const final { return always_fails_; }

  bool always_succeeds() const final { return always_succeeds_; }
 ...
 private:
  void init() {
    if (!(unevaluated_properties_val_ || unevaluated_items_val_)) {
      ...
      always_succeeds_ = always_succeeds_count ==
                         validators_.size();  // empty schema always succeeds
      always_fails_ =
          validators_.size() > 0 && (always_fails_count == validators_.size());
    }
  }
  ...
  walk_result do_validate(const evaluation_context<Json>& context,
                          ...
                          evaluation_results& results, error_reporter& reporter,
                          Json& patch) const final {
    // std::cout << "object_schema_validator begin[" <<
    ...
    for (auto& val : validators_) {
      ...
      walk_result result =
          val->validate(this_context, instance, instance_location,
                        local_results, reporter, patch);
    ...
    if (unevaluated_properties_val_) {
      walk_result result = unevaluated_properties_val_->validate(
          this_context, instance, instance_location, local_results, reporter,
          patch);
      if (result == walk_result::abort) {
        return result;
      }
    ...
    if (unevaluated_items_val_) {
      walk_result result = unevaluated_items_val_->validate(
          this_context, instance, instance_location, local_results, reporter,
          patch);
      if (result == walk_result::abort) {
        return result;
    ...
    // std::cout << "object_schema_validator end[" <<
    // context.eval_path().string() << "," << this->schema_location().string()
    // << "]"; std::cout << "results:\n"; for (const auto& s : results)
    //{
    //     std::cout << "    " << s << "\n";
  ...
  walk_result do_walk(const evaluation_context<Json>& context,
    ...
    if (unevaluated_properties_val_) {
      walk_result result = unevaluated_properties_val_->walk(
          this_context, instance, instance_location, reporter);
      if (result == walk_result::abort) {
        return result;
      }
    }
    ...
    if (unevaluated_items_val_) {
      walk_result result = unevaluated_items_val_->walk(
          this_context, instance, instance_location, reporter);
      if (result == walk_result::abort) {
        return result;
      }

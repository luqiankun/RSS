...
class schema_identifier {
 ...
 public:
  ...
  explicit schema_identifier(const uri& uri) : uri_{uri} {
    uri_ = jsoncons::uri(uri);
    identifier_ = uri_.fragment();
  }

  jsoncons::uri uri() const { return uri_; }
  ...
  friend bool operator==(const schema_identifier& lhs,
                         const schema_identifier& rhs) {

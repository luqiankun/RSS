...
class validator_base {
 public:
  ...
  void validate(const evaluation_context<Json>& eval_context,
    ...
    do_validate(eval_context, instance, instance_location, evaluated_properties,
                reporter, patch);
  }

 private:
  virtual void do_validate(
      const evaluation_context<Json>& eval_context, const Json& instance,
      const jsonpointer::json_pointer& instance_location,
      std::unordered_set<std::string>& evaluated_properties,
...
class keyword_validator_base : public keyword_validator<Json> {
 ...
 public:
  ...
  keyword_validator_base(const std::string& keyword_name,
  ...
  const std::string& keyword_name() const override { return keyword_name_; }

  const uri& schema_path() const override { return schema_path_; }
};
...
class boolean_schema_validator : public schema_validator<Json> {
 ...
 public:
  ...
  bool is_recursive_anchor() const final { return false; }
  ...
  void do_validate(const evaluation_context<Json>& eval_context, const Json&,
    ...
    if (!value_) {
      reporter.error(validation_output(
          "false", eval_context.eval_path(), this->schema_path(),
          instance_location.to_string(), "False schema always fails"));
    }
  }
};
...
class object_schema_validator : public schema_validator<Json> {
 ...
 public:
  ...
  bool is_recursive_anchor() const final { return is_recursive_anchor_; }
 ...
 private:
  void do_validate(const evaluation_context<Json>& eval_context,
    ...
    // std::cout << "object_schema_validator: " <<
    ...
    for (auto& validator : validators_) {
      ...
      validator->validate(this_context, instance, instance_location,
                          local_evaluated_properties, reporter, patch);

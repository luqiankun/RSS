...
typename std::enable_if<extension_traits::is_unary_function_object_exact<
                        ...
                        std::unique_ptr<schema_builder<Json>>>::type
make_schema_validator_factory(const Json& sch, const URIResolver& resolver) {
  ...
  if (sch.is_object()) {
    ...
    if (it != sch.object_range().end()) {
      if (it->value() == "https://json-schema.org/draft/2019-09/schema") {
        validator_factory_ptr =
            jsoncons::make_unique<jsoncons::jsonschema::draft201909::
                                      draft201909_schema_builder<Json>>(
                resolver);
      } else if (it->value() == "http://json-schema.org/draft-07/schema#") {
        validator_factory_ptr = jsoncons::make_unique<
            jsoncons::jsonschema::draft7::draft7_schema_builder<Json>>(
            resolver);
      } else {
      ...
      validator_factory_ptr = jsoncons::make_unique<
          jsoncons::jsonschema::draft7::draft7_schema_builder<Json>>(resolver);
    }
  } else if (sch.is_bool()) {
    validator_factory_ptr = jsoncons::make_unique<
        jsoncons::jsonschema::draft7::draft7_schema_builder<Json>>(resolver);
...
struct default_uri_resolver {
  Json operator()(const jsoncons::uri& uri) {
    ...
    JSONCONS_THROW(jsonschema::schema_error(
        "Don't know how to load JSON Schema " + uri.string()));
  }
};
...
make_schema(const Json& sch, const std::string& retrieval_uri,
...
std::shared_ptr<json_schema<Json>> make_schema(
    const Json& sch, const std::string& retrieval_uri) {
  auto schema_builder =
      make_schema_validator_factory(sch, default_uri_resolver<Json>{});

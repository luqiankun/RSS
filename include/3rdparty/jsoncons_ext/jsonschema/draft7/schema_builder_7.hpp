...
#include "../draft7/schema_draft7.hpp"
...
class schema_builder_7 : public schema_builder<Json> {
 ...
 public:
  schema_builder_7(Json&& sch,
      ...
      : schema_builder<Json>(schema_version::draft7(), std::move(sch),
  ...
  void init() {
    ...
    keyword_factory_map_.emplace(
        ...
        [&](const compilation_context& context, const Json& sch,
            const Json& parent, anchor_uri_map_type&) {
          return this->make_content_media_type_validator(context, sch, parent);
        });
    if (this->options().require_format_validation()) {
      keyword_factory_map_.emplace(
          "format", [&](const compilation_context& context, const Json& sch,
                        const Json& parent, anchor_uri_map_type&) {
            return this->make_format_validator(context, sch, parent);
          });
    }
#if defined(JSONCONS_HAS_STD_REGEX)
    keyword_factory_map_.emplace(
    ...
    keyword_factory_map_.emplace(
        ...
        [&](const compilation_context& context, const Json& sch,
            const Json& parent, anchor_uri_map_type& anchor_dict) {
          return this->make_property_names_validator(context, sch, parent,
                                                     anchor_dict);
        });
    keyword_factory_map_.emplace(
        ...
        if (it != sch.object_range().end())  // this schema is a reference
        {
          ...
          if (it2 != sch.object_range().end()) {
            for (const auto& def : it2->value().object_range()) {
              ...
              defs.emplace(def.key(),
                           make_schema_validator(context, def.value(), sub_keys,
                                                 anchor_dict));
            }
          }
          ...
          schema_validator_ptr =
              jsoncons::make_unique<object_schema_validator<Json>>(
                  ...
                  std::move(validators), std::move(defs),
  ...
  schema_validator_type make_object_schema_validator(
    ...
    return jsoncons::make_unique<object_schema_validator<Json>>(
        ...
        std::move(defs), std::move(default_value));
  }
 ...
 private:
  static const std::unordered_set<std::string>& known_keywords() {
    static std::unordered_set<std::string> keywords{"$id",
                                                    "$ref",
                                                    "additionalItems",

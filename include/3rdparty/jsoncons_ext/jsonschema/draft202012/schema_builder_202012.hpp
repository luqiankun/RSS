...
class schema_builder_202012 : public schema_builder<Json> {
 public:
  ...
  using schema_builder_factory_type =
      typename schema_builder<Json>::schema_builder_factory_type;
  ...
  static const std::string& format_annotation_id() {
    static std::string id =
        "https://json-schema.org/draft/2020-12/format-annotation";
    return id;
  }
 ...
 public:
  schema_builder_202012(Json&& sch,
      ...
      : schema_builder<Json>(schema_version::draft202012(), std::move(sch),
                             builder_factory, options, schema_store_ptr,
                             resolvers, vocabulary),
        include_applicator_(true),
        include_unevaluated_(true),
        include_validation_(true),
        include_format_(true) {
    if (!vocabulary.empty()) {
  ...
  void init() {
    // validation
    validation_factory_map_.emplace(
        "type", [&](const compilation_context& context, const Json& sch,
                    const Json& parent, anchor_uri_map_type&) {
          return this->make_type_validator(context, sch, parent);
        });
/*
            validation_factory_map_.emplace("contentEncoding",
                [&](const compilation_context& context, const Json& sch, const
   Json& parent, anchor_uri_map_type&){return
   this->make_content_encoding_validator(context, sch, parent);});
            validation_factory_map_.emplace("contentMediaType",
                [&](const compilation_context& context, const Json& sch, const
   Json& parent, anchor_uri_map_type&){return
   this->make_content_media_type_validator(context, sch, parent);});
*/
#if defined(JSONCONS_HAS_STD_REGEX)
    validation_factory_map_.emplace(
        "pattern", [&](const compilation_context& context, const Json& sch,
                       const Json& parent, anchor_uri_map_type&) {
          return this->make_pattern_validator(context, sch, parent);
        });
#endif
    validation_factory_map_.emplace(
        "maxItems", [&](const compilation_context& context, const Json& sch,
                        const Json& parent, anchor_uri_map_type&) {
          return this->make_max_items_validator(context, sch, parent);
        });
    validation_factory_map_.emplace(
        "minItems", [&](const compilation_context& context, const Json& sch,
                        const Json& parent, anchor_uri_map_type&) {
          return this->make_min_items_validator(context, sch, parent);
        });
    validation_factory_map_.emplace(
        "maxProperties",
        [&](const compilation_context& context, const Json& sch,
            const Json& parent, anchor_uri_map_type&) {
          return this->make_max_properties_validator(context, sch, parent);
        });
    validation_factory_map_.emplace(
        "minProperties",
        [&](const compilation_context& context, const Json& sch,
            const Json& parent, anchor_uri_map_type&) {
          return this->make_min_properties_validator(context, sch, parent);
        });
    validation_factory_map_.emplace(
        "contains", [&](const compilation_context& context, const Json& sch,
                        const Json& parent, anchor_uri_map_type& anchor_dict) {
          return this->make_contains_validator(context, sch, parent,
                                               anchor_dict);
        });
    validation_factory_map_.emplace(
        "uniqueItems", [&](const compilation_context& context, const Json& sch,
                           const Json& parent, anchor_uri_map_type&) {
          return this->make_unique_items_validator(context, sch, parent);
        });
    validation_factory_map_.emplace(
        "maxLength", [&](const compilation_context& context, const Json& sch,
                         const Json& parent, anchor_uri_map_type&) {
          return this->make_max_length_validator(context, sch, parent);
        });
    validation_factory_map_.emplace(
        "minLength", [&](const compilation_context& context, const Json& sch,
                         const Json& parent, anchor_uri_map_type&) {
          return this->make_min_length_validator(context, sch, parent);
        });
    validation_factory_map_.emplace(
        "not", [&](const compilation_context& context, const Json& sch,
                   const Json& parent, anchor_uri_map_type& anchor_dict) {
          return this->make_not_validator(context, sch, parent, anchor_dict);
        });
    validation_factory_map_.emplace(
        "maximum", [&](const compilation_context& context, const Json& sch,
                       const Json& parent, anchor_uri_map_type&) {
          return this->make_maximum_validator(context, sch, parent);
        });
    validation_factory_map_.emplace(
        "exclusiveMaximum",
        [&](const compilation_context& context, const Json& sch,
            const Json& parent, anchor_uri_map_type&) {
          return this->make_exclusive_maximum_validator(context, sch, parent);
        });
    validation_factory_map_.emplace(
        "minimum", [&](const compilation_context& context, const Json& sch,
                       const Json& parent, anchor_uri_map_type&) {
          return this->make_minimum_validator(context, sch, parent);
        });
    validation_factory_map_.emplace(
        "exclusiveMinimum",
        [&](const compilation_context& context, const Json& sch,
            const Json& parent, anchor_uri_map_type&) {
          return this->make_exclusive_minimum_validator(context, sch, parent);
        });
    validation_factory_map_.emplace(
        "multipleOf", [&](const compilation_context& context, const Json& sch,
                          const Json& parent, anchor_uri_map_type&) {
          return this->make_multiple_of_validator(context, sch, parent);
        });
    validation_factory_map_.emplace(
        "const", [&](const compilation_context& context, const Json& sch,
                     const Json& parent, anchor_uri_map_type&) {
          return this->make_const_validator(context, sch, parent);
        });
    validation_factory_map_.emplace(
        "enum", [&](const compilation_context& context, const Json& sch,
                    const Json& parent, anchor_uri_map_type&) {
          return this->make_enum_validator(context, sch, parent);
        });
    validation_factory_map_.emplace(
        "allOf", [&](const compilation_context& context, const Json& sch,
                     const Json& parent, anchor_uri_map_type& anchor_dict) {
          return this->make_all_of_validator(context, sch, parent, anchor_dict);
        });
    validation_factory_map_.emplace(
        "anyOf", [&](const compilation_context& context, const Json& sch,
                     const Json& parent, anchor_uri_map_type& anchor_dict) {
          return this->make_any_of_validator(context, sch, parent, anchor_dict);
        });
    validation_factory_map_.emplace(
        "oneOf", [&](const compilation_context& context, const Json& sch,
                     const Json& parent, anchor_uri_map_type& anchor_dict) {
          return this->make_one_of_validator(context, sch, parent, anchor_dict);
        });
    if (this->options().compatibility_mode()) {
      validation_factory_map_.emplace(
          ...
          [&](const compilation_context& context, const Json& sch,
              const Json& parent, anchor_uri_map_type& anchor_dict) {
            return this->make_dependencies_validator(context, sch, parent,
                                                     anchor_dict);
          });
    }
    validation_factory_map_.emplace(
        "required", [&](const compilation_context& context, const Json& sch,
                        const Json& parent, anchor_uri_map_type&) {
          return this->make_required_validator(context, sch, parent);
        });
    validation_factory_map_.emplace(
        "dependentRequired",
        [&](const compilation_context& context, const Json& sch,
            const Json& parent, anchor_uri_map_type&) {
          return this->make_dependent_required_validator(context, sch, parent);
        });
  }
  ...
  schema_validator_type make_schema_validator(
    ...
    // std::cout << "this->make_cross_draft_schema_validator " <<
    // context.get_base_uri().string() << ", " <<
    // new_context.get_base_uri().string() << "\n\n";
    ...
    switch (sch.type()) {
      ...
      case json_type::object_value: {
        schema_validator_ptr =
            make_object_schema_validator(new_context, sch, anchor_dict);
        schema_validator<Json>* p = schema_validator_ptr.get();
        for (const auto& uri : new_context.uris()) {
          this->insert_schema(uri, p);
          /*for (const auto& item : sch.object_range())
          {
              ...
              {
                  std::cout << "  " << item.key() << "\n";
                  this->insert_unknown_keyword(uri, item.key(), item.value());
          // save unknown keywords for later reference
              }
          }*/
        }
        break;
  ...
  schema_validator_type make_object_schema_validator(
    ...
    std::unique_ptr<unevaluated_properties_validator<Json>>
        unevaluated_properties_val;
    ...
    if (this->options().compatibility_mode()) {
      ...
      if (it != sch.object_range().end()) {
        for (const auto& def : it->value().object_range()) {
          ...
          defs.emplace(def.key(),
                       this->make_cross_draft_schema_validator(
                           context, def.value(), sub_keys, local_anchor_dict));
        }
      }
    }
    it = sch.find("$defs");
    ...
    if (it != sch.object_range().end())  // this schema has a reference
    {
      ...
      auto ref = relative.resolve(uri_wrapper{context.get_base_uri()});
      auto orig = jsoncons::make_unique<dynamic_ref_validator_type>(
          sch, ref.uri().base(), ref);
    ...
    if (include_applicator_) {
      ...
      if (it != sch.object_range().end()) {
        validators.emplace_back(this->make_property_names_validator(
            context, it->value(), sch, local_anchor_dict));
      }

      it = sch.find("dependentSchemas");
      if (it != sch.object_range().end()) {
        validators.emplace_back(this->make_dependent_schemas_validator(
            context, it->value(), sch, local_anchor_dict));
      }
      ...
      if (it != sch.object_range().end()) {
        ...
        if_validator = this->make_cross_draft_schema_validator(
            context, it->value(), sub_keys, local_anchor_dict);
      }

      it = sch.find("then");
      if (it != sch.object_range().end()) {
        std::string sub_keys[] = {"then"};
        then_validator = this->make_cross_draft_schema_validator(
            context, it->value(), sub_keys, local_anchor_dict);
      }
      ...
      if (if_validator || then_validator || else_validator) {
        validators.emplace_back(
            jsoncons::make_unique<conditional_validator<Json>>(
                sch, context.get_base_uri(), std::move(if_validator),
      ...
      if (it != sch.object_range().end()) {
        properties = this->make_properties_validator(context, it->value(), sch,
                                                     local_anchor_dict);
      }
      ...
      if (it != sch.object_range().end()) {
        pattern_properties = make_pattern_properties_validator(
            context, it->value(), sch, local_anchor_dict);
      }
#endif
      ...
      if (it != sch.object_range().end()) {
        validators.emplace_back(this->make_additional_properties_validator(
            context, it->value(), sch, std::move(properties),
            std::move(pattern_properties), local_anchor_dict));
      } else {
        if (properties) {
      ...
      if (it != sch.object_range().end()) {
        if (it->value().type() == json_type::array_value) {
          validators.emplace_back(make_prefix_items_validator(
              context, it->value(), sch, local_anchor_dict));
        }
      } else {
        it = sch.find("items");
        if (it != sch.object_range().end()) {
          if (it->value().type() == json_type::object_value ||
              it->value().type() == json_type::bool_value) {
            validators.emplace_back(this->make_items_validator(
                "items", context, it->value(), sch, local_anchor_dict));
          }
        }
      }
    }
    ...
    if (include_format_) {
      if (this->options().require_format_validation()) {
        ...
        if (it != sch.object_range().end()) {
          validators.emplace_back(
              this->make_format_validator(context, it->value(), sch));
        }
      }
    }
    ...
    if (include_unevaluated_) {
      ...
      if (it != sch.object_range().end()) {
        unevaluated_properties_val =
            this->make_unevaluated_properties_validator(context, it->value(),
                                                        sch, local_anchor_dict);
      }
      it = sch.find("unevaluatedItems");
      if (it != sch.object_range().end()) {
        unevaluated_items_val = this->make_unevaluated_items_validator(
            context, it->value(), sch, local_anchor_dict);
      }
    }
    ...
    std::unordered_map<std::string, std::unique_ptr<ref_validator<Json>>>
        anchor_schema_map;
    for (const auto& member : local_anchor_dict) {
      anchor_schema_map.emplace(
          member.first, this->get_or_create_reference(sch, member.second));
    }
    return jsoncons::make_unique<object_schema_validator<Json>>(
        context.get_base_uri(), std::move(id), std::move(validators),
        std::move(unevaluated_properties_val), std::move(unevaluated_items_val),
        std::move(defs), std::move(default_value), std::move(dynamic_anchor),
        std::move(anchor_schema_map));
  }

  std::unique_ptr<prefix_items_validator<Json>> make_prefix_items_validator(
      const compilation_context& context, const Json& sch, const Json& parent,
      anchor_uri_map_type& anchor_dict) {
    ...
    return jsoncons::make_unique<prefix_items_validator<Json>>(
        "prefixItems", parent, schema_location,
  ...
  make_pattern_properties_validator(const compilation_context& context,
    ...
    for (const auto& prop : sch.object_range()) {
      ...
      pattern_properties.emplace_back(
          std::make_pair(std::regex(prop.key(), std::regex::ECMAScript),
                         this->make_cross_draft_schema_validator(
                             context, prop.value(), sub_keys, anchor_dict)));
    }

    return jsoncons::make_unique<pattern_properties_validator<Json>>(
        parent, std::move(schema_location), std::move(pattern_properties));
  }
#endif
    ...
    if (sch.is_object()) {
      ...
      if (it != sch.object_range().end()) {
        ...
        if (relative.has_fragment()) {
          JSONCONS_THROW(schema_error(
              str + ": Draft 2019-09 does not allow $id with fragment"));
        }
        uri_wrapper new_uri =
            relative.resolve(uri_wrapper{parent.get_base_uri()});
        id = new_uri.uri();
      ...
      if (it != sch.object_range().end()) {
        ...
        auto uri =
            !new_uris.empty() ? new_uris.back().uri() : jsoncons::uri{"#"};
        jsoncons::uri new_uri(uri, uri_fragment_part, anchor);
        uri_wrapper identifier{new_uri};
        if (std::find(new_uris.begin(), new_uris.end(), identifier) ==
            new_uris.end()) {
 ...
 private:
  static const std::unordered_set<std::string>& known_keywords() {
    static std::unordered_set<std::string> keywords{"$anchor",
                                                    "$dynamicAnchor",
                                                    "$dynamicRef",
                                                    "$id",
                                                    "$ref",
